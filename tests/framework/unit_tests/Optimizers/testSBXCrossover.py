...
"""
  Testing for the Simulated Binary Crossover (sbxCrossover) method.
  Validates output shape, bound-respect, identical-parent preservation, and
  reproducibility under a fixed RNG seed.
...
# bounded continuous distributions for two decision variables
...
def makeParents(rows):
  """
    Build a parents DataArray from a list of gene rows.
    @ In, rows, list(list(float)), parent gene values
    @ Out, da, xr.DataArray, parents in (chromosome, Gene) layout
  """
  rows = np.array(rows, dtype=float)
  return xr.DataArray(rows, dims=['chromosome', 'Gene'],
                      coords={'chromosome': np.arange(rows.shape[0]), 'Gene': optVars})

sbxCrossover = returnInstance('tester', 'sbxCrossover')

# ---- shape and bound-respect ----
randomUtils.randomSeed(42)
parents = makeParents([[2.0, 1.0], [8.0, -3.0], [5.0, 4.0]])
children = sbxCrossover(parents=parents, variables=optVars, crossoverProb=0.9, points=None, distDict=distDict)
arr = children.values
checkTrue('SBX output shape is 2*C(nParents,2) x nGenes', arr.shape == (6, 2))
checkTrue('SBX respects x bounds [0,10]', np.all((arr[:, 0] >= 0.0) & (arr[:, 0] <= 10.0)))
checkTrue('SBX respects y bounds [-5,5]', np.all((arr[:, 1] >= -5.0) & (arr[:, 1] <= 5.0)))
checkTrue('SBX produces no NaNs', not np.any(np.isnan(arr)))

# ---- identical parents are preserved (nothing to recombine) ----
identical = makeParents([[3.0, 2.0], [3.0, 2.0]])
childrenId = sbxCrossover(parents=identical, variables=optVars, crossoverProb=1.0, points=None, distDict=distDict).values
checkTrue('SBX preserves identical parents', np.allclose(childrenId, [[3.0, 2.0], [3.0, 2.0]]))

# ---- reproducibility under fixed seed ----
randomUtils.randomSeed(7)
runA = sbxCrossover(parents=parents, variables=optVars, crossoverProb=0.9, points=None, distDict=distDict).values
randomUtils.randomSeed(7)
runB = sbxCrossover(parents=parents, variables=optVars, crossoverProb=0.9, points=None, distDict=distDict).values
checkTrue('SBX is reproducible under a fixed seed', np.allclose(runA, runB))
