...
# Test fit
diff1 = createDifferencing(targets=targets, order=1)
params = diff1.fit(signals, pivot, targets, {})
checkFloat('first-order differencing fit', params['A']['initValues'], signals[:1], 1e-10)

# Test forward transformation
transformed1 = diff1.getResidual(signals, params, pivot, {})
transformed1True = np.r_[0, np.diff(signals, axis=0).ravel()].reshape(-1, 1)
checkArray('first-order differencing getResidual', transformed1, transformed1True, float)

# Test inverse transformation
inverse1 = diff1.getComposite(transformed1True, params, pivot, {})
...
# Test fit
diff2 = createDifferencing(targets=targets, order=2)
params = diff2.fit(signals, pivot, targets, {})
checkFloat('second-order differencing fit', params['B']['initValues'], signals[:2], 1e-10)

# Test forward transformation
transformed2 = diff2.getResidual(signals, params, pivot, {})
transformed2True = np.r_[0, 0, np.diff(signals, n=2, axis=0).ravel()].reshape(-1, 1)
checkArray('second-order differencing getResidual', transformed2, transformed2True, float)

# Test inverse transformation
inverse2 = diff2.getComposite(transformed2True, params, pivot, {})
