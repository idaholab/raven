...
# plt.scatter(test2D[:,0], test2D[:,1])
# for i in range(test2D.shape[0]):
#   plt.text(test2D[i,0], test2D[i,1], str(i))
# plt.show()
...
### Compare with hand-calculation result: testing non-dominated sorting & crowding distances ###

testHand = np.array([[0.913, 2.348],
                     [0.599, 3.092],
                     [0.139, 2.138],
                     [0.867, 1.753],
                     [0.885, 1.455],
...
# plt.scatter(testHand[:,0], testHand[:,1])
# for i in range(testHand.shape[0]):
#   plt.text(testHand[i,0], testHand[i,1], str(i))
# plt.show()

ranktestHandFront = frontUtils.rankNonDominatedFrontiers(testHand)
indexestestHand = frontUtils.crowdingDistance(rank=ranktestHandFront, popSize=len(ranktestHandFront), objectives=testHand)

answerranktestHandFront = np.array([3, 2, 1, 2, 1, 2, 2, 1])
answerindexestestHand = np.array([np.inf, np.inf, np.inf, np.inf, np.inf, 1.1137376968778216, 1.4176401413396051, 2.0])

checkArray('2D Hand calculation: ranks of NonDominated Frontiers', ranktestHandFront, answerranktestHandFront.tolist())
checkArray('2D Hand calculation: crowding distance', indexestestHand.tolist(), answerindexestestHand.tolist())
