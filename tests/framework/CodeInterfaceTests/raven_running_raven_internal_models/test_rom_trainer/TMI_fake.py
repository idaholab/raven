...
def run(self, Input):
  ...
  print 'DEBUGG run DTA:',DeltaTimeScramToAux,type(DeltaTimeScramToAux)
  print 'DEBUGG run DG1:',DG1recoveryTime,type(DG1recoveryTime)
  ...
  print 'OUTGOING: self.CladTempThreshold:'
  for i in range(len(self.time)):
    print ' ...',i,self.time[i],self.CladTempThreshold[i]
