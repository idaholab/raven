...
def evaluate(self):
  ...
  """
  model = None
  left = self.leftTemperature > 860 and self.leftTemperature < 1340
  right = self.rightTemperature > 740 and self.rightTemperature < 1460
  if left and right:
    model = 'svr'
  else:
    model = 'thermalConductivityComputation'
