...
def control_function(monitored, controlled, auxiliary):
    ...
    if auxiliary.crew1DG1:
        if monitored.time_step == 1:
            auxiliary.DG1recoveryTime = monitored.time
            auxiliary.DG1_time_ratio = 0.0
        else: 
...
def dynamic_event_tree(monitored, controlled, auxiliary):    
    ...
    if distcont.checkCdf('crew1DG1', monitored.time - auxiliary.scram_start_time - 100.0) and (not auxiliary.CladDamaged) and (not auxiliary.crew1DG1):
        ...
        return
    if auxiliary.crew1DG1 and distcont.checkCdf('crew1DG2CoupledDG1', auxiliary.DG1_time_ratio) and (not auxiliary.CladDamaged) and (not auxiliary.AuxSystemUp):   
        auxiliary.AuxSystemUp = True
        return
    if distcont.checkCdf('crewSecPG', monitored.time - auxiliary.scram_start_time - 400.0) and (not auxiliary.CladDamaged) and (not auxiliary.AuxSystemUp):   
        auxiliary.AuxSystemUp = True
        return        
    if distcont.checkCdf('PrimPGrecovery', monitored.time - auxiliary.scram_start_time) and (not auxiliary.CladDamaged) and (not auxiliary.AuxSystemUp):   
        auxiliary.AuxSystemUp = True
        return     
    return
