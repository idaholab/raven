...
class BatemanClass:
  ...
  def __init__(self,initializationDict):
    ...
    Constructor
    """
    if "totalTime" not in initializationDict.keys():
      raise IOError("not total time day specified in the input")
    self.totaltime = initializationDict["totalTime"]
    if "powerHistory" not in initializationDict.keys():
      raise IOError("not power history specified in the input")
    self.PowerHistory = initializationDict["powerHistory"]
    self.numberMacroSteps = len(self.PowerHistory)
    if "flux" not in initializationDict.keys():
      raise IOError("not flux specified in the input")
    self.Flux = initializationDict["flux"]
    if len(self.Flux) != self.numberMacroSteps:
      raise IOError("number of steps in Flux different then the one in power history")
    if "timeSteps" not in initializationDict.keys():
      raise IOError("not timeSteps specified in the input")
    self.timesteps = [int(elm) for elm in initializationDict["timeSteps"]]
    if len(self.timesteps) != self.numberMacroSteps:
      raise IOError("number of steps in timeSteps different then the one in power history")
    if "stepDays" not in initializationDict.keys():
      raise IOError("not stepDays specified in the input")
    stepDays = initializationDict["stepDays"]
    if len(stepDays)-1 != self.numberMacroSteps:
      raise IOError("number of steps in stepDays must be equal to the one in power history + 1")
    self.lowbound = []
    self.upbound  = []
    ...
    """
      Nuclides
    """
    if "nuclides" not in initializationDict.keys():
      raise IOError("no nuclide specified in the input")
...
#        self.IsoData.append(('D','N4',1,0.00000008,5,230.0))

#        self.IsoData.append(('E','N1',1,0.05,90,230.0))
#        self.IsoData.append(('F','N2',1,0.05,90,230.0))
#        self.IsoData.append(('G','N3',1,0.05,90,230.0))
#        self.IsoData.append(('H','N4',1,0.05,90,230.0))
#
#        self.IsoData.append(('I','N1',1,0.05,90,230.0))
#        self.IsoData.append(('L','N2',1,0.05,90,230.0))
#        self.IsoData.append(('M','N3',1,0.05,90,230.0))
#        self.IsoData.append(('N','N4',1,0.05,90,230.0))
#
#        self.IsoData.append(('O','N1',1,0.05,90,230.0))
#        self.IsoData.append(('P','N2',1,0.05,90,230.0))
#        self.IsoData.append(('Q','N3',1,0.05,90,230.0))
#        self.IsoData.append(('R','N4',1,0.05,90,230.0))
#
#        self.IsoData.append(('S','N1',1,0.05,90,230.0))
#        self.IsoData.append(('T','N2',1,0.05,90,230.0))
#        self.IsoData.append(('U','N3',1,0.05,90,230.0))
#        self.IsoData.append(('V','N4',1,0.05,90,230.0))
    ...
    #self.IsoGroups.append([self.IsoData[4],self.IsoData[5],self.IsoData[6],self.IsoData[7]])
    #self.IsoGroups.append([self.IsoData[8],self.IsoData[9],self.IsoData[10],self.IsoData[11]])
    #self.IsoGroups.append([self.IsoData[12],self.IsoData[13],self.IsoData[14],self.IsoData[15]])
    #self.IsoGroups.append([self.IsoData[16],self.IsoData[17],self.IsoData[18],self.IsoData[19]])
  ...
  def N2solution(self,N01,N02,A1,A2,sigma1,sigma2,flux,t):
    flux = flux * self.barn
    if (-A1+A2-flux*sigma1+flux*sigma2) == 0.0:
      dividend = 0.000000001
    else:
      dividend = (-A1+A2-flux*sigma1+flux*sigma2)
  ...
  def N3solution(self,N01,N02,N03,A1,A2,A3,sigma1,sigma2,flux,t):
    flux = flux * self.barn
    if ((-A1+A3-flux*sigma1)*(-A2+A3-flux*sigma2)*(-A1+A2-flux*sigma1+flux*sigma2)) !=0.0:
      dividend  = ((-A1+A3-flux*sigma1)*(-A2+A3-flux*sigma2)*(-A1+A2-flux*sigma1+flux*sigma2))
    else:
      dividend = 0.000000001
  ...
  def N4solution(self,N01,N02,N03,N04,A1,A2,A3,A4,sigma1,sigma2,flux,t):
    flux = flux * self.barn
    if ((A1+flux*sigma1)*(A1-A3+flux*sigma1)*(A2+flux*sigma2)*(A2-A3+flux*sigma2)*(-A1+A2-flux*sigma1+flux*sigma2)) !=0.0:
      dividend  = ((A1+flux*sigma1)*(A1-A3+flux*sigma1)*(A2+flux*sigma2)*(A2-A3+flux*sigma2)*(-A1+A2-flux*sigma1+flux*sigma2))
    else:
      dividend = 0.000000001
  ...
  def printResults(self,outputFileName="results.csv"):
    ...
    f.write(header + '\n')
    for ts in range(len(self.time_to_run)):
      row = str(self.result[key][0][ts])
      for key in self.result.keys():
        row = row + ',' + str(self.result[key][1][ts])
      f.write(row + '\n')
