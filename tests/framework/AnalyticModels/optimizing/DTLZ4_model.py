...
"""
Analytic implementation of the three-objective DTLZ4 benchmark problem with the
standard alpha=100 biasing on the first M-1 decision variables.
"""
...
def _extract_decision_vectors(inputs):
  ...
  if not decision_keys:
    raise ValueError('DTLZ4_model requires decision variables named x1 ... xN.')
...
def _dtlz4_objectives(vector, num_objectives, alpha=_ALPHA):
  num_decisions = len(vector)
  k = num_decisions - num_objectives + 1
  if k <= 0:
    raise ValueError('DTLZ4 requires at least num_objectives - 1 free variables.')
  tail = vector[-k:]
  g = sum((value - 0.5) ** 2 for value in tail)
  transformed = vector[:]
  for idx in range(num_objectives - 1):
    transformed[idx] = math.pow(transformed[idx], alpha)
  objectives = []
  for m in range(1, num_objectives + 1):
    prefix = 1.0 + g
    for i in range(1, num_objectives - m + 1):
      prefix *= math.cos(0.5 * math.pi * transformed[i - 1])
    if m > 1:
      prefix *= math.sin(0.5 * math.pi * transformed[num_objectives - m])
...
def evaluate(inputs):
  if not inputs:
    raise ValueError('DTLZ4_model.evaluate received no decision variables.')
  ...
  for decision_vector in decision_vectors:
    objective_vals = _dtlz4_objectives(decision_vector, _NUM_OBJECTIVES)
...
def run(self, inputs):
  ...
  if not data:
    raise ValueError('DTLZ4_model.run received no decision variables.')
