  ...
  def modifySimulation(self, runInfoDict):
    ...
    if self.__in_pbs:
      ...
      if newBatchsize != oldBatchsize:
        #XX
        runInfoDict['batchSize'] = newBatchsize
        print("changing batchsize from "+ str(oldBatchsize)+" to " +str(newBatchsize))
      print("Using Nodefile to set batchSize:"+str(runInfoDict['batchSize']))
      #Add pbsdsh command to run.  pbsdsh runs a command remotely with pbs
      print('DEBUG precommand',runInfoDict['precommand'])
      #XXX
      runInfoDict['precommand'] = "pbsdsh -v -n %INDEX1% -- %BASE_WORKING_DIR%/pbsdsh_custom/raven_remote.sh out_%CURRENT_ID% %WORKING_DIR% "+ str(runInfoDict['logfileBuffer'])+" "+runInfoDict['precommand']
      #XXX
      runInfoDict['logfilePBS'] = 'out_%CURRENT_ID%'
      ...
      #print('DEBUG precommand',runInfoDict['precommand'],'postcommand',runInfoDict['postcommand'])
