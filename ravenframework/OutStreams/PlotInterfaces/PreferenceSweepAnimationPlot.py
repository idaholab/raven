...
class PreferenceSweepAnimationPlot(PlotInterface):
  ...
  def handleInput(self, spec):
    ...
    if fmtNode is not None and fmtNode.value is not None:
      parsed = self._parseFormats(fmtNode.value)
      if parsed:
        self.formats = parsed
    ...
    if framesMaxNode is not None and framesMaxNode.value is not None:
      self.framesMax = max(1, int(framesMaxNode.value))
  ...
  def run(self):
    ...
    if df.empty:
      ...
      return
    subset = df.copy()
    objCols = self._getObjectiveColumns()
    for col in objCols:
    ...
    subset = subset.dropna(subset=objCols)
    if subset.empty:
    ...
    objectivesArray = subset[objCols].to_numpy(dtype=float)
    objectivesArray = self._toMinimizationSpace(objectivesArray)
    if self.normalize:
      objectivesArray = self._minmaxScale(objectivesArray)
    bestIndices = []
    scores = []
    for w in weights:
      coeffs = np.array([w, 1.0 - w], dtype=float)
      scalar = objectivesArray @ coeffs
      idx = int(np.argmin(scalar))
      bestIndices.append(idx)
      scores.append(float(scalar[idx]))
    bestIndices = np.asarray(bestIndices, dtype=int)
    scores = np.asarray(scores, dtype=float)

    xy = subset[objCols].to_numpy(dtype=float)
    fig, ax = plt.subplots(figsize=(6.4, 5.2))
    if 'gif' in self.formats:
      self._writeGif(weights, xy, bestIndices, scores)
    if 'html' in self.formats:
      self._writeHtml(weights, xy, bestIndices, scores)
    if self.saveFrames:
      self._writeFrames(weights, xy, bestIndices, scores)
    plt.close(fig)

  def _writeGif(self, weights, xy, bestIndices, scores):
    ...
    with imageio.get_writer(filename, mode='I', duration=duration, loop=0) as writer:
      for frameIdx, w in enumerate(weights):
        fig = self._renderFrame(w, xy, bestIndices[frameIdx], scores[frameIdx])
        buffer = io.BytesIO()
  ...
  def _writeHtml(self, weights, xy, bestIndices, scores):
    filename = self._createFilename(defaultName=f'{self.name}.html')
    fig, ax = self._createBasePlot()

    def init():
      self._drawFrame(ax, weights[0], xy, bestIndices[0], scores[0])
      fig.tight_layout(rect=(0.05, 0.05, 0.95, 0.95))
      return fig.axes

    def update(frameIdx):
      self._drawFrame(ax, weights[frameIdx], xy, bestIndices[frameIdx], scores[frameIdx])
      fig.tight_layout(rect=(0.05, 0.05, 0.95, 0.95))
      return fig.axes
  ...
  def _writeFrames(self, weights, xy, bestIndices, scores):
    count = min(self.framesMax, len(weights))
    ...
    for idx in indices:
      fig = self._renderFrame(weights[idx], xy, bestIndices[idx], scores[idx])
      fig.tight_layout(rect=(0.05, 0.05, 0.95, 0.95))
      fig.savefig(template.format(idx=idx), dpi=150)
      plt.close(fig)

  def _renderFrame(self, weight, xy, bestIdx, score):
    fig, ax = self._createBasePlot()
    self._drawFrame(ax, weight, xy, bestIdx, score)
    fig.tight_layout(rect=(0.05, 0.05, 0.95, 0.95))
    return fig

  def _drawFrame(self, ax, weight, xy, bestIdx, score):
    ax.clear()
    ax.scatter(xy[:, 0], xy[:, 1], c='#bbbbbb', edgecolor='k', linewidths=0.1, alpha=0.6, s=28, label='Samples')
    ax.scatter([xy[bestIdx, 0]], [xy[bestIdx, 1]], c='tab:red', s=80,
    ...
    legendX = 1.02
    ax.legend(loc='upper left', bbox_to_anchor=(legendX, 1.0), borderaxespad=0.0, frameon=True)
    ax.text(legendX, 0.0,
            f'w = {weight:0.2f}\nscore = {score:0.4g}',
