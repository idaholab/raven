...
class LowerConfidenceBound(AcquisitionFunction):
  ...
  def _converged(self, bayesianOptimizer):
    ...
    if self._optValue is None:
      converged = False
      return converged
    minObjValDiff = np.absolute(-1*self._optValue - bayesianOptimizer._optPointHistory[0][-1][0][bayesianOptimizer._objectiveVar[0]])
    minObjValDiff /= np.absolute(bayesianOptimizer._optPointHistory[0][-1][0][bayesianOptimizer._objectiveVar[0]])
    if minObjValDiff <= bayesianOptimizer._acquisitionConv:
      converged = True
    else:
      converged = False
    return converged
