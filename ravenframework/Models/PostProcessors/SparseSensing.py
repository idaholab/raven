...
class SparseSensing(PostProcessorReadyInterface):
  ...
  def _handleInput(self, paramInput):
    ...
    for child in paramInput.subparts:
      if child.getName() == 'reconstruction':
        self.SparseSensingGoal = 'reconstruction'
      ...
      elif child.getName() == 'classification':
        self.SparseSensingGoal = 'reconstruction'
        self.nSensors = child.findFirst('nSensors').value
        self.nModes = child.findFirst('nModes').value
        self.basis = child.findFirst('basis').value
        ...
        self.raiseAnError(IOError, '{} is not a recognized option, allowed options are [\'reconstruction\',\'classification\']]'.format(child.getName()))
    # checks
