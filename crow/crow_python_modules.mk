...
setup_py3_intermediate : $(CROW_DIR)/crow_modules/distribution1Dpy3.i $(CROW_DIR)/crow_modules/interpolationNDpy3.i $(CROW_DIR)/crow_modules/randomENGpy3.i $(DISTRIBUTION_SOURCE) $(INTERPOLATION_SOURCE) $(RANDOM_CLASS_SOURCE)
	(cd $(CROW_DIR) && unset CXX CC && if test `uname` = Darwin; then MACOSX_DEPLOYMENT_TARGET=10.12.6; export MACOSX_DEPLOYMENT_TARGET; fi && if test $NO_CONDA = 0; then . ../scripts/establish_conda_env.sh --load; fi && python3 ./setup3.py build_ext build install --install-platlib=./install)
