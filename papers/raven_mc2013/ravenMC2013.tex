...
\begin{quote}
\begin{small}
RAVEN is a software tool under development at the Idaho National Laboratory (INL), that acts as the control logic driver and post-processing framework for the newly developed Thermo-Hydraylic code RELAP-7. The aim of this paper is to show the software structure of the code and its utilization in connection with RELAP-7, presenting a short overview of the mathematical framework behind the code and tis main capabilities (e.g. on-line controlling and monitoring, Monte-Carlo sampling, etc.).A demo of a Station Black Out PRA analysis of a simplified Pressurized Water Reactor (PWR) model is shown, in order to demonstrate the Monte-Carlo and clustering capabilities.
...
\end{itemize}
Since the aim of this demo is showing the capabilities contained in RAVEN and the RELAP-7 CPU time is still an issue, the transient has been accelerated in order to simulate a maximum of 300 seconds. 
...
\begin{subfigure}{.5\textwidth}
  ...
  \caption{Probabilistic distribution function for Monte-Carlo sampling.}
  \label{fig:runs_temp_dist}
\end{subfigure}%
\begin{subfigure}{.5\textwidth}
  \centering
  \includegraphics[width=1\linewidth]{figures/PRA_dist2.PNG}
  \caption{Monte-carlo histogram results. }
  \label{fig:pdf_temp}
\end{subfigure}
\caption{PRA resutls. Give a title here}
\label{fig:test}
\end{figure}
\begin{figure}[H]
   \centering
    \includegraphics[width=0.8\textwidth]{figures/PRA_limitSurface.PNG}
    \caption{Limit Surface Monte-Carlo sampling.}
