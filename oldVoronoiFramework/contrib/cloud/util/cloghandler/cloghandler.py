...
# Fall back to regular rotating handler if we can't go concurrent
try:
    from portalocker import lock, unlock, LOCK_EX, LOCK_NB, LockException
    concurrent_error = None
except Exception, e: # on import failure; disable concurrent logging
    concurrent_error = e
    def lock(file, flags):
        pass
    def unlock(file):
        pass
    LOCK_EX = LOCK_NB = LockException = None
...
class ConcurrentRotatingFileHandler(BaseRotatingHandler):
    ...
    def release(self):
        ...
        try:
            ...
            try:
                #self.stream_lock.write(str(os.getpid()) + ' rel\n')
                unlock(self.stream_lock)
