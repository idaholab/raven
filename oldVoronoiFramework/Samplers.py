  ...
  def localGenerateInput(self,model,oldInput):
      ...
      if self.batchStrategy == 'none':
        ...
        if maxDistance > 0.0:
          for varIndex, _ in enumerate([key.replace('<distribution>','') for key in self.axisName]):
            ...
            # self.inputInfo['SampledVarsCdf'][self.axisName[varIndex]] = self.distDict[self.axisName[varIndex]].cdf(self.values[self.axisName[varIndex]])
          varSet=True
        else:
    ...
    if not varSet:
      ...
      for key in self.distDict.keys():
        ...
        print(self.inputInfo.keys())
        print(self.inputInfo['SampledVarsCdf'])
    ...
    for key in self.distDict:
      ...
      if totDim == 1:
        for var in self.distributions2variablesMapping[dist]:
          ...
          self.inputInfo['SampledVarsCdf'][key] = self.distDict[key].cdf(rvsnum)
        ...
        if dim == 1:
          ...
          if "pca" not in self.transformationMethod.values() and len(self.transformationMethod.values())==1:
            self.inputInfo['SampledVarsCdf'][key] = self.distDict[key].cdf(coordinate)
          else:
...
class Grid(Sampler):
  ...
  def localGenerateInput(self,model,myInput):
    ...
    while not found:
      ...
      for i in range(len(self.axisName)):
        ...
        # compute the SampledVarsPb for N-D distribution
            ...
            if self.variables2distributionsMapping[varName]['dim']==1:    # to avoid double count;
              ...
              # self.inputInfo['SampledVarsCdf'][varName] = self.distDict[varName].cdf(NDcoordinate)
      ...
      # self.inputInfo['GridInfo'] = [[i,self.gridInfo[i]] for i in self.gridInfo]
    ...
    for varName in self.axisName:
        ...
        if self.variables2distributionsMapping[varName]['totDim']>1 and self.variables2distributionsMapping[varName]['dim'] == 1:    # to avoid double count of weight for ND distribution; I need to count only one variable instaed of N
          if self.variablesTransformationDict:
            ...
            if self.gridInfo[varName] == 'CDF':
              ...
              self.inputInfo['SampledVarsCdf'][varName] = self.distDict[varName].cdf(np.atleast_1d(gridCoordinate).tolist())
      ...
      if ("<distribution>" in varName) or self.variables2distributionsMapping[varName]['totDim']==1:   # 1D variable
        ...
        if self.gridInfo[varName] =='CDF':
          ...
          self.inputInfo['SampledVarsCdf'][varName] = self.distDict[varName].cdf(ppfValue)
          ...
          self.inputInfo['SampledVarsCdf'][varName] = self.distDict[varName].cdf(coordinate)    #@jougcj : added for the computation of the voronoi in the probability space. (if equally spaced distribution)
  ...
  def localGenerateInput(self,model,myInput):
    ...
    while not found:
      ...
      for v,varName in enumerate(self.distDict.keys()):
        ...
        self.inputInfo['SampledVarsCdf'][varName] = self.distDict[varName].cdf(self.values[varName])
        self.inputInfo['ProbabilityWeight-'+varName.replace(",","-")] = self.inputInfo['SampledVarsPb'][varName]
      self.inputInfo['PointProbability'] = reduce(mul,self.inputInfo['SampledVarsPb'].values())
