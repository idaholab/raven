...
path. \maljdan{May need a better description.}
...
\begin{itemize}
  ...
  which is \xmlNode{scaling} (see below). \maljdan{Which article?}
  %
...
There are two nodes that are used in the post-processor.
...
\item \xmlNode{compare}: specifies the data to use for comparison.
  ...
  \item \xmlNode{reference}: Specifies that a normal distribution will
...
\xmlAttr{subType} attribute to be ``External'': 
\xmlNode{PostProcessor \xmlAttr{subType}=\xmlString{External}}.
% 
