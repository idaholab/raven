...
Currently this model is only able to share information (i.e. data) using both \textbf{PointSet}.
...
\begin{itemize}
  ...
  \item \xmlAttr{class}, \xmlDesc{required string attribute}, the class of this sub-model (e.g. Models)
  %
  \item \xmlAttr{type}, \xmlDesc{required string attribute}, the sub-type of this Model (e.g. ROM)
  %
\end{itemize}
\vspace{-5mm}
...
\begin{itemize}
  ...
  \item \xmlAttr{class}, \xmlDesc{required string attribute}, the class of this sub-model (e.g. Models)
  %
  \item \xmlAttr{type}, \xmlDesc{required string attribute}, the sub-type of this Model (e.g. PostProcessor)
  %
\end{itemize}
\vspace{-5mm}
  ...
  %
    ...
    \nb currently only accept data object wih type ``PointSet``. The \xmlNode{TargetEvaluation} is primary used for
...
\begin{itemize}
  \itemsep0em
  \item \xmlAttr{class}, \xmlDesc{required string attribute}, the class of the DataObjects.
  %
  \item \xmlAttr{type}, \xmlDesc{required string attribute}, the sub-type of the DataObjects (e.g. ``PointSet``)
  %
\end{itemize}
\vspace{-5mm}
  %
...
\begin{itemize}
  ...
  %
  \begin{itemize}
     \item \xmlNode{initialTrainSize}, \xmlDesc{integer, optional field}, the initial number of high-fidelity
       model runs before starting train the ROMs.
       \default{10};
     \item \xmlNode{tolerance}, \xmlDesc{float, optional field}, ROMs convergence criterion. This tolerance will
       be compared with the results from the cross validation to determine whether the ROMs are converged or not.
        \default{0.01};
     \item \xmlNode{trainStep}, \xmlDesc{XML node, optional field}, the number of additional high-fidelity model
       runs before performing cross validation on the ROMs.
       \default{1}
     \item \xmlNode{maxTrainSize}, \xmlDesc{XML node, optional field}, the maximum size of training set of ROMs.
       \default{1.0E6}
     \item \xmlNode{selectionMethod}, \xmlDesc{XML node, optional field}, the validity methods that are used to
       determine which model to run (i.e. ROMs or high-fidelity Model).
       \nb Currently, only one method is available, ie. ``CrowdingDistance``.
       \default{``CrowdingDistance``}
  \end{itemize}
\end{itemize}
  ...
  <Models>
    ...
    <HybridModel name="hybrid" subType="">
        ...
        <settings>
            ...
            <selectionMethod>CrowdingDistance</selectionMethod>
        </settings>
    </HybridModel>
  </Models>
  ...
