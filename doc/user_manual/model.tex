...
\newcommand{\verSVMDescription}[1]
{
  ...
  that, if enabled, may not work properly in a multi-threaded context.
}
...
\begin{itemize}
  ...
  \begin{itemize}
   ...
   \item \xmlAttr{csv} \xmlDesc{bool, optional field} specifies if a csv file needs to be 
...
\begin{itemize}
  ...
  \item \xmlAttr{subType}, \xmlDesc{required string attribute}, external model type. It must be kept empty, except in the following cases:
  \begin{itemize}
    \item  \textbf{\textit{pluginType}}, in this case the  \xmlAttr{subType} is the spefic plugin to use (e.g. \textbf{TEAL.CashFlow})
  ...
  with its absolute or relative path. This attribute is specified just for Generic ExternalModel (i.e.  empty \xmlAttr{subType})
  %
...
\begin{itemize}
  \item \xmlNode{variables}, \xmlDesc{string, optional parameter}.
  %
  Comma-separated list of variable names.
  %
  Each variable name needs to match a variable used/defined in the external python
  model.
  \nb \textbf{This node (\xmlNode{variables}) is deprecated and will be removed in RAVEN 3.0 in favor of the two following nodes (\xmlNode{inputs}, \xmlNode{outputs}) }
  %
...
\paragraph{\texttt{def initialize}}
...
\paragraph{Method: \texttt{def createNewInput}}
...
\paragraph{Method: \texttt{def run}}
...
When this is the case, a \xmlNode{ExternalModel} with subtype ’pickledModel’ is used to hold the place of the
...
raise an error. An \xmlNode{IOStep} is used to load the ExternalModel from file, at which point the ExternalModel will
...
\xmlNode{Files}, the model is defined in \xmlNode{Models}, and the model loaded in \xmlNode{Steps}.
