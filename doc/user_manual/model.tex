...
\begin{itemize}
  \item \xmlString{mean}, the mean value of the ROM within the input space it was trained,
  \item \xmlString{variance}, the variance of the ROM within the input space it was trained,
...
When Printing this ROM via an OutStreamManager (see \ref{sec:printing}), the available metrics are
\xmlString{mean}, \xmlString{variance}, \xmlString{numRuns}, and \xmlString{indices}.  In the latter
case, the total calculated variance is printed, followed by the percent of contributing variance for
each existing subset of the input domain (the Sobol indices).
