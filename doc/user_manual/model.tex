...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \item \xmlAttr{cdf}, \xmlDesc{boolean, option field},
      indicates that the code should use the CDF of the distribution instead of the distribution
      itself (see note for \xmlAttr{quad}).  Default is \xmlString{false} for the four distributions
      in Table \ref{tab:gpcCompatible} and \xmlString{true} otherwise.
      \default{``false'' if listed on Table \ref{tab:gpcCompatible}, ``true'' otherwise.}
  \end{itemize}
  %
\end{itemize}
\begin{table}[htb]
