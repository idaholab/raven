...
\begin{itemize}
  ...
  \begin{itemize}
     ...
     \item \xmlNode{tolerance}, \xmlDesc{float, optional field}, ROMs convergence criterion. This tolerance will
       ...
       if \textit{explained\_variance\_score} and \textit{r2\_error} are used, the best possible score is ``1.0``, and
       lower values are worse. The \xmlNode{tolerance} should be selected close to ``1.0``.
       if \textit{mean\_squared\_error}, \textit{mean\_absolute\_error} and \textit{median\_absolute\_error}
       are used, the best possible score is ``0.0``, larger values are worse. Thus, the \xmlNode{tolerance} should be
       selected close to ``0.0``.
       \default{0.01};
