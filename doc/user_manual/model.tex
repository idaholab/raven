...
\item \xmlNode{compare}: specifies the data to use for comparison.
  ...
  \item \xmlNode{reference}: This specifies a reference distribution
    ...
    % DEPRECATED: Specifies that a normal distribution will
...
can generate either a CSV file or a TimePointSet.
...
The inputted Limit Surface needs to be, in the  \textbf{PostProcess} step, of type  \textbf{TimePointSet} and needs to contain 
both boundary sides (-1.0, +1.0).
\\ The \textbf{LimitSurfaceIntegral} post-processor accepts as outputs both files (CSV) and/or  \textbf{TimePointSet}s.
  ...
  \end{itemize}
    ...
    \item  \xmlNode{tolerance}, \xmlDesc{float, optional field}, specifies the tolerance for
     ...
     \item  \xmlNode{target}, \xmlDesc{string, optional field}, specifies the target name that represents
                ...
                \default{last output found in the inputted TimePointSet}  
\end{itemize}
...
\begin{itemize}
  ...
  connected graph.
  \default{-1}
  \item \xmlNode{normalization}, \xmlDesc{string, optional field}, an optional 
  string specifying whether the inputs/output should be scaled before computing.
  Currently, two modes are supported 'zscore' and 'feature'. 'zscore' will ensure
  the data has a mean of zero and a standard deviation of 1 by subtracting the
