...
\item Try to shorten the gradient vector so that it satisfies the constraints.
\item if by shorternning the gradient constraint cannot be satisfied, then find a random vector orthogonal to gradient that satisfy constraint, and rotate it towards the constraints. When doing so, the length of new vector is taken care of as well.
\item otherwise do not update the variables and the spsa will conclude the optimal (which may not be true optimal).
