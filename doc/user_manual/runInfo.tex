...
Essentially, the SimulationMode class allows modifications to the way
RAVEN runs in two ways.  First of all, it allows modifying the various
run info parameters before the code is run.  Secondly, it allows
overriding how Simulation runs. This second bit can be used if for
example RAVEN needs to be run on a different machine such as a head
node.  In this case, the runOverride can be done to somehow call RAVEN
to be run on the head node, and then on the head node, it will run
RAVEN normally.
...
run type that is `parallel', but not for `serial' codes.

To help with these commands, there are several variables that are
substituted in before running the command.  These are:

\begin{description}
\item[\%INDEX\%]  0 based index in list of running jobs.  Note that this is stable for the life of the job.
\item[\%INDEX1\%] 1 based index in the list of running jobs, same as \%INDEX\%+1
\item[\%CURRENT\_ID\%]  0 based id for the job handler.  This starts as 0, and increases for each job the job handler starts.
\item[\%CURRENT\_ID1\%] 1 based id for the job handler, same as \%CURRENT\_ID\%+1
