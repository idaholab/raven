...
\begin{itemize}
%%%%%% WORKING DIR
\item \xmlNode{WorkingDir}, \xmlDesc{string, required field}, specifies
...
where RAVEN looks for the files specified in the block \xmlNode{Files}.
%
\default{None}

%%%%%% RELATIVE WORKING DIR
\item \xmlNode{RelativeWorkingDir}, \xmlDesc{string, optional field}, specifies
the relative (with respect to the location where the xml file is located)
path to a directory that will store all the results of the calculations and
where RAVEN looks for the files specified in the block \xmlNode{Files}.
%
\default{None}
...
%%%%%% Files
\item \xmlNode{Files}, \xmlDesc{comma separated string, required field}, lists
the paths to any files required by the code.
%string from the \emph{WorkingDir}.

%%%%%% BATCH SIZE
\item \xmlNode{batchSize}, \xmlDesc{integer, required field}, specifies the
number of parallel runs executed simultaneously (e.g., the number of driven
...
%%%%%% NUMBER OF NODES
\item \xmlNode{NumNode}, \xmlDesc{integer, optional field}, specifies
...
(mpi mode)}, specifies the time the whole calculation is expected to
last.
%
