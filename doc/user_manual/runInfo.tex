...
\item \xmlNode{MPIExec}, \xmlDesc{string, optional field}, specifies the command used to run mpi.  This will be followed by the \xmlNode{NodeParameter} and then the node file and then the code command.
  ...
  %%%%%% N THREADS
\item \xmlNode{threadCommand}, \xmlDesc{string, optional field}, specifies the command used to set the
    number of threads. The "%#%" is a wildcard that will be replaced by the number of threads
    ...
    For example, $--my-nthreads=%#%$ (e.g. $--my-nthreads=10$). In other cases, the command can be
    inputted explicetely adding the blank space. For example, $-omp %#%$ (e.g. $-omp 10$).
...
customized via the node \xmlNode{threadCommand}.
%
