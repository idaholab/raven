...
hdf5, so those may need to be manually installed.  It probably will be
useful to use a virtual environment with PIP:
...
method.  If you want to do development on RAVEN and RAVEN is the only
MOOSE application you are using or you don't mind having duplicate
MOOSE directories, go to Section \ref{submodule_git}. If you are using
several MOOSE applications and want to share the MOOSE directory go to
Section \ref{parallel_directory_git}.  If you don't need to do RAVEN
development but want to keep up with the latest version of RAVEN, the
RAVEN Whole repository can be used as described in Section
...
\ref{raven_whole_stable}.  If git is not used, the Source Code Package
in Section \ref{raven_source_package} can be used.
...
This should only be done when the user is using a secure connection to a known
