...
\textbf{V}alue (\textbf{CSV}) file for \textbf{Datas} objects, and \textbf{XML}
...
\textit{Data} from scratch, as well as access advanced features of particular
reduced order models.
%
\subsubsection{\textbf{Datas} Printing}
The XML code, that is reported below, shows different ways to request a
\textit{Print} OutStream for \textbf{Datas} objects.
%
...
\begin{itemize}
  ...
  %
  \nb Only \textbf{csv} is currently available for Datas objects
...
While all \textbf{ROM}s in RAVEN are designed to be used as a surrogate model,
...
\begin{itemize}
  ...
  \item \xmlNode{source}, the \textit{Rom} name (one of the \textit{ROM} 
  ...
  keyward \xmlString{all} can be provided to request all available metrics, if any.
\end{itemize}
\vspace{-5mm}
Addtionally, when printing ROMs one optional nodes are available,
\vspace{-5mm}
\begin{itemize}
  \itemsep0em
...
\end{itemize}
\vspace{-5mm}
ROM printing uses the same naming conventions as Datas printing.
Examples:
\begin{lstlisting}[style=XML]
<OutStreamManager>
