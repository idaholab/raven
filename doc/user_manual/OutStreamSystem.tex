...
\begin{itemize}
  ...
  \item \xmlNode{color\_mode}, \xmlDesc{optional, string}, controls infeasible coloring when constraints are provided. Options are \xmlString{feasibility} (default),
        \xmlString{violation} (color encodes violation magnitude), or \xmlString{none}.
  \item \xmlNode{thickness\_mode}, \xmlDesc{optional, string}, controls linewidth when constraints are provided. Options are \xmlString{none} (default) or \xmlString{violation}.
  \item \xmlNode{violation\_metric}, \xmlDesc{optional, string}, reduces multiple constraint violations into a scalar (for \xmlNode{color\_mode} or \xmlNode{thickness\_mode}): \xmlString{sum} (default), \xmlString{max}, or \xmlString{l2}.
  \item \xmlNode{linewidth\_bounds}, \xmlDesc{optional, comma-separated floats}, two values giving min/max linewidth when \xmlNode{thickness\_mode} is \xmlString{violation}.
  \item \xmlNode{feasible\_color}, \xmlDesc{optional, string}, color for feasible paths (default \xmlString{\#2e7d32}).
  \item \xmlNode{infeasible\_color}, \xmlDesc{optional, string}, color for infeasible paths when \xmlNode{color\_mode} is \xmlString{feasibility} (default \xmlString{\#d32f2f}).
  \item \xmlNode{infeasible\_cmap}, \xmlDesc{optional, string}, Matplotlib colormap used for infeasible paths when \xmlNode{color\_mode} is \xmlString{violation} (default \xmlString{Reds}).
  \item \xmlNode{show\_infeasible}, \xmlDesc{optional, boolean}, if false, infeasible paths are hidden (default true).
\end{itemize}
...
\begin{itemize}
  \item \xmlNode{rows\_per\_fig}, \xmlDesc{optional, integer}, splits a large matrix into multiple output images.
  \item \xmlNode{min\_slice\_points}, \xmlDesc{optional, integer}, minimum slice size before fallback selection is used.
  \item \xmlNode{fallback\_points}, \xmlDesc{optional, integer}, number of nearest-to-median samples used as fallback.
\end{itemize}
...
Optional nodes include \xmlNode{top\_n} (and \xmlNode{include\_other}) to limit displayed categories, plus \xmlNode{index}/\xmlNode{generation}/\xmlNode{use\_all\_generations} for generation filtering.
...
Provide \xmlNode{objectives} (two entries). Choose epsilon either directly with \xmlNode{epsilon} or indirectly with \xmlNode{search\_extent} (epsilon = 1/search\_extent).
Optionally provide \xmlNode{constraints} to mark infeasible samples, and \xmlNode{goals} to indicate min/max directions.
The \xmlNode{search\_extent} is a convenience control that sets the number of grid divisions per axis in normalized space; increasing it decreases epsilon and increases resolution.
If both \xmlNode{epsilon} and \xmlNode{search\_extent} are provided, \xmlNode{epsilon} takes precedence.
In this plot, normalization is used to compute epsilon boxes (so \xmlNode{epsilon} is interpreted in normalized objective space); \xmlNode{axis\_space} only controls how the axes are displayed.
For improved readability when solutions occupy only a small region of the unit square, enable \xmlNode{auto\_limits} (default) to zoom to the occupied region,
...
When normalized objectives appear overly compressed (e.g., most points clustered near 0 or 1 due to outliers), use \xmlNode{normalize\_mode}=\xmlString{quantile}
with \xmlNode{normalize\_quantiles} to scale using robust percentile bounds instead of raw min/max.
If you prefer to label axes in the original objective units instead of \([0,1]\), set \xmlNode{axis\_space}=\xmlString{objective}. In this mode the epsilon grid and boxes
are still computed in normalized space but mapped back onto the objective axes for display.
Note that objective axes can differ by orders of magnitude (e.g., efficiency vs cost); if \xmlNode{axis\_space}=\xmlString{objective} and \xmlNode{aspect} is not explicitly provided,
...
Optional nodes:
\begin{itemize}
  \item \xmlNode{include\_failed}, \xmlDesc{optional, boolean}, include samples marked failed in the source. Defaults to \xmlString{false} to prevent invalid sentinel values from distorting limits.
  \item \xmlNode{layout\_top}, \xmlDesc{optional, float}, top boundary passed to matplotlib \texttt{tight\_layout(rect=...)}. Use values below 1.0 to leave additional headroom above the title (default 0.96).
  \item \xmlNode{show\_info}, \xmlDesc{optional, boolean}, if \xmlString{true} (default) prints ``if-cues'' under the legend: a \emph{Good signs} box and a \emph{Needs attention} box with quick interpretation guidance (e.g., gaps in blue suggest under-exploration; many points in one box suggests epsilon too large).
  \item \xmlNode{info\_fontsize}, \xmlDesc{optional, float}, font size for the if-cues boxes (default 9).
  ...
  \item \xmlNode{index}, \xmlDesc{optional, string}, generation identifier column; if present and \xmlNode{use\_all\_generations} is \xmlString{false}, uses the last generation unless \xmlNode{generation} is supplied.
  \item \xmlNode{generation}, \xmlDesc{optional, float}, specific generation to plot when \xmlNode{index} is provided.
  \item \xmlNode{use\_all\_generations}, \xmlDesc{optional, boolean}, when \xmlNode{index} is present, include all generations (default \xmlString{true}).
  ...
  \item \xmlNode{normalize\_mode}, \xmlDesc{optional, string}, \xmlString{minmax} (default) or \xmlString{quantile}.
  \item \xmlNode{normalize\_quantiles}, \xmlDesc{optional, list}, two quantiles (e.g., 0.02,0.98) used when \xmlNode{normalize\_mode}=\xmlString{quantile}; values are clipped to \([0,1]\) after scaling.
  \item \xmlNode{search\_extent}, \xmlDesc{optional, integer}, sets epsilon as \(1/\)search\_extent and draws a search\_extent\(\times\)search\_extent grid in normalized space.
  \item \xmlNode{epsilon}, \xmlDesc{optional, float}, epsilon size in normalized space (overrides \xmlNode{search\_extent}).
  \item \xmlNode{show\_grid}, \xmlDesc{optional, boolean}, show epsilon grid lines (default \xmlString{true}).
  \item \xmlNode{show\_boxes}, \xmlDesc{optional, boolean}, show highlighted epsilon boxes for epsilon-efficient representatives (default \xmlString{true}).
  \item \xmlNode{show\_all}, \xmlDesc{optional, boolean}, show all samples (default \xmlString{true}).
  \item \xmlNode{show\_infeasible}, \xmlDesc{optional, boolean}, mark infeasible samples with red \texttt{x} when \xmlNode{constraints} are provided (default \xmlString{true}).
  \item \xmlNode{auto\_limits}, \xmlDesc{optional, boolean}, zoom axes to the occupied region plus padding (default \xmlString{true}).
  \item \xmlNode{limits\_padding}, \xmlDesc{optional, float}, fractional padding around occupied region when \xmlNode{auto\_limits} is enabled.
  ...
  \item \xmlNode{aspect}, \xmlDesc{optional, string}, \xmlString{equal} or \xmlString{auto}; when \xmlNode{axis\_space}=\xmlString{objective} the default is \xmlString{auto} unless explicitly set.
  \item \xmlNode{axis\_space}, \xmlDesc{optional, string}, \xmlString{normalized} (default) or \xmlString{objective}.
\end{itemize}
