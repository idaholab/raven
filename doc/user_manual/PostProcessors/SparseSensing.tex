...
\begin{itemize}
  ...
  \nb{Currently the algorithms can handle a single target}.
	...
	\item \xmlNode{optimizer},  \xmlDesc{string, optional field}, the optimizer used to find the sensors: \xmlString{QR}, for the unconstrained case.
        ...
        \item \xmlNode{reshape}, \xmlDesc{string, optional field}, how to flatten a time-dependent tensor before sensor selection. \xmlString{snapshot} (default) stacks samples and time steps as rows so sensor selection still returns physical spatial locations.
\end{itemize}
\end{itemize}
