...
\begin{itemize}
  ...
  \nb{Currently the algorithms can handle a single target}.
	\item \xmlNode{basis} , \xmlDesc{string, optional field}, the type of basis onto which the data are projected: \xmlString{Identity}, \xmlString{SVD}, \xmlString{RandomProjection}. The legacy misspelling \xmlString{RandomProjetion} is still accepted for backward compatibility. \default{SVD}
	...
	\item \xmlNode{optimizer},  \xmlDesc{string, optional field}, the optimizer used to find the sensors: \xmlString{QR} for the unconstrained case or \xmlString{CCQR} when sensor costs are supplied. \default{QR}
...
\end{itemize}
\item \xmlNode{Metric}, \xmlDesc{string, optional field}, the name of a RAVEN \xmlNode{Metric} object with attributes \xmlAttr{class=\xmlString{Metrics}} and \xmlAttr{type=\xmlString{Metric}}. These metrics are evaluated on the reconstructed steady-state field and written as scalar outputs.
\end{itemize}
