...
\label{EconomicRatio}
The \xmlNode{EconomicRatio} PostProcessor provides the economic metrics from the percent change
...
\begin{itemize}
  ...
  specify this quantity with a parameter \xmlAttr{threshold='X'}, where the \xmlAttr{X} represents
  the requested $\alpha$ value (a floating point value between 0.0 and 1.0). The user can also specify
  the parameter \xmlAttr{interpolation='Y'}, where the \xmlAttr{Y} represents the interpolation method
  to be used (\xmlAttr{linear} (default) or \xmlAttr{midpoint}). \xmlAttr{linear} uses linear
  interpolation and \xmlAttr{midpoint} uses the midpoint or average between data points. It is
