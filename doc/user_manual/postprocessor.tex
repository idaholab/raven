...
\begin{itemize}
  ...
  %
  %
  In addition, the user can request the transformation matrix (or inverse transformation matrix) from this post processor. The transformation matrix
  ...
  matrix is used to map the manifest variables to the latent variables in the transformed space. In order to let RAVEN to output the transformation matrix
  (or inverse transformation matrix), the user need to specify \xmlString{transformation} or \xmlString{InverseTransformation} in \xmlNode{what}. In addition,
  both  \xmlNode{manifest} and \xmlNode{latent} subnodes should be defined in node \xmlNode{features}. 
  In the following, we will show the user how to request the transformation matrix or the inverse transformation matrix.
  %
  %
  \item \xmlNode{features}, \textit{\textbf{XML node, required parameter}}, used to specify the information for the input variables.
...
\end{itemize}
  %
  %
  Moreover, if both \xmlNode{manifest} and \xmlNode{latent} are provided, the user can input \xmlString{ManifestSensitivity} in \xmlNode{what}
