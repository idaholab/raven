...
\begin{itemize}
  ...
  \default{False}
  %
  \item \xmlNode{multipleLinear}, \xmlDesc(boolean, optional field), this node can be used when the users do not want
    to compute the multiple linear sensitivities. This method can be used when the input features depend on each
    other. \nb this node only affects the calculations of metrics such as \xmlNode{sensitivity}, \xmlNode{VarianceDependentSensitivity}
    and \xmlNode{NormalizedSensitivity}.
  \default{False}
\end{itemize}
