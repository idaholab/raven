...
\begin{itemize}
  ...
  If all the quantities need to be computed, the user can input in the body of
  \xmlNode{what} the string ``all''.
  In addition, the user can request the transformation matrix from this post processor. The transformation matrix
  is used to map the latent variables in the transformed space to the manifest variables in the original input space.
  In the following, we will show the user how to request the transformation matrix.
  %
  %
  ...
  In addition, the user can also specify the \xmlAttr{type} for \xmlNode{features}:
    \begin{itemize}
      \item \xmlAttr{type}, \xmlDesc{optional string attribute}, specifies the type of input variables
      the default \xmlAttr{type} is \xmlString{''}. Possible values are \xmlString{''} and \xmlString{latent}.
      \nb \xmlString{latent} indicates the input variables are from the transformed space, i.e. the latent variables.
    \end{itemize}
  \nb the \xmlAttr{type} need to be specified only if the transformation matrix is requested. 
  %
  \item \xmlNode{targets}, \xmlDesc{comma separated string, required field},
  lists output responses on which previous operations need to be applied. In addition, this node contains one
  attribute:
    \begin{itemize}
      \item \xmlAttr{type}, \xmlDesc{optional string attribute}, specifies the type of target variables
      the default \xmlAttr{type} is \xmlString{''}. Possible values are \xmlString{''} and \xmlString{manifest}.
      \nb \xmlString{manifest} indicates the target variables are from the input space, i.e. the manifest variables.
    \end{itemize}
  \nb the \xmlAttr{type} need to be specified only if the transformation matrix is requested. 
  %
  ...
  %
  \item \xmlNode{targetDimensions}, \xmlDesc{comma separated integer, optional field},
  lists the  dimensions corresponding to \xmlNode{targets}. This information will be used if the transformation matrix
  is requested, otherwise ignore this node.
  %
...
In oder to output the transformation matrix, the user need to input the body of \xmlNode{what} the string
`transformation'. In addition, the \xmlAttr{type} of \xmlNode{targets} should be \xmlString{manifest}, and the
\xmlAttr{type} of \xmlNode{features} should be \xmlString{latent}. If \xmlNode{dimensions} and \xmlNode{targetDimensions}
are not specified, the default order indices of \xmlNode{features} and \xmlNode{targets} will be used, respectively.
Here is the example:
...
<Simulation>
  ...
  <Models>
    ...
    <PostProcessor name='aUserDefinedName' subType='ImportanceRank'>
      <what>transformation</what>
      <features type='latent'>x1,x2</features>
      <targets type='manifest'>y1,y2</targets>
      <dimensions>1,2<dimensions>
      <targetDimensions>1,2</targetDimensions>
      <mvnDistribution>MVN</mvnDistribution>
    </PostProcessor>
    ...
  </Models>
