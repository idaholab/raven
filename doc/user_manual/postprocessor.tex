...
\label{ETimporterPP}
The \textbf{ETimporter} post-processor has been designed to import Event-Tree (ETs) object into
RAVEN. This is performed by saving the structure of the ET (from file) as a \textbf{PointSet} only. 
...
%
\begin{itemize}
  \item \xmlNode{OpenPSA}, \xmlDesc{string, required field}, specifies the format of the file that contains the ET structure (supported format: OpenPSA).
\end{itemize}
...
\begin{itemize}
  \item \xmlNode{n}, \xmlDesc{integer, required field}, total number of elements
...
In addition to this XML node, several others are available:
\begin{itemize}
  \item \xmlNode{n}, \xmlDesc{integer, required field}, total number of elements in dataset.
\end{itemize}
