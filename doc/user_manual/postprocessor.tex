...
Note that when the \xmlNode{samplingType} type is set to uniform and \xmlNode{interpolation} is set to intervalAverage, the input historySet shall be evenly spaced in time. If this is not the case, use Post-Processor method \textbf{HistorySetSync} to synchronize the data before running this method.
