...
A clustering algorithm associate a unique cluster label to each element of the dataObject (PointSet or HistorySet). 
This cluster label is a natural number ranging from $0$ to $N$ where $N$ is the number of obtained clusters. 
...
\begin{itemize}
   \item \xmlNode{clusterIDs}, \xmlDesc{string, required field}, ID of the clustering label
   \item \xmlNode{label}, \xmlDesc{integers, required field}, IDs of the chosen clustering labels. Note that more than one label can be provided as input
\end{itemize}
