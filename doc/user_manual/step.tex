...
recovered in other simulations or used to evaluate the response of a physical system
 in a Python program by the implementing of the Pickle module.
  \item export a \textit{ROM} or  \textit{ExternalModel} to an external $FMI/FMU$  \textit{File} using the RAVEN native
  $FMI/FMU$ exporting capability
\end{itemize}
...
\begin{table}[h!]
  ...
  \begin{tabular}{l|l|l}
    ...
    ExternalModel     & File       & Serialize ExternalModel to Disk \\
                                & File        & If $ext$ is $fmu$ ($myOut.fmu$) Serialize ExternalModel to FMU
  \end{tabular}
