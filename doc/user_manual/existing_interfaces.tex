...
\begin{itemize}
  ...
  $301 \:\:\: p \:\:\: 345010000$ will be named in the converted CSVs as $p\_345010000$).In addition, if a variable contains spaces, the trailing spaces are going to be removed and internal spaces are replaced with underscores (e.g. $HTTEMP 1131008 12$ will become $HTTEMP\_1131008\_12$}.
\end{itemize}
}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%% MAMMOTH INTERFACE %%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{MAMMOTH Interfaces}
%
This section covers the input specification for runing MAMMOTH (couple Rattlesnake with Bison) through RAVEN. 
It is important to notice that this short explanation assumes that the reader already knows how to use MAMMOTH.
The interface can be used to perturb the Bison, Rattlesnake YAML input files as well as the Yak multigroup
cross section libraries XML input files.
%
...
name for the file contained in the node, and \xmlAttr{type} identifies which base-level
interface the file is used within.  \xmlNode{type} should only be specified for inputs
that RAVEN will perturb.  For MAMMOTH input files, \xmlNode{type} should be \xmlString{MAMMOTHInput|RattlesnakeInput}, 
for Yak multigroup cross section libraries, the \xmlNode{type} should be \xmlString{YakXSInput}, and for the Yak 
alias files that are used to perturb the Yak multigroup cross section libraries, the \xmlNode{type} should be
\xmlString{YakXSAliasInput}. For Bison input files, the \xmlNode{type} should be \xmlString{BisonInput}. In addition, 
The \xmlNode{type}s for Rattlesnake mesh input file and Bison mesh input file are \xmlString{Bison|Mesh} and 
\xmlString{Rattlesnake|Mesh},respectively. 
...
%
The alias files are employed to define the variables that will be used to perturb Yak multigroup cross section
libraries. Please see the section \textbf{Rattlesnake Interface} for the example.  
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{Models}
A user provides paths to executables and aliases for sampled variables within the
...
their distributions are identified in the \xmlNode{variable} blocks.
The name attribute in the \xmlNode{variable} block must either be the
full MooseBasedApp model variable name, the alias name specifed in
\xmlNode{Models}, or the variable name specified in the provided alias files.  
...
input file and the poissons ratio defined in Bison input files.
%
\begin{lstlisting}[style=XML]
<Samplers>
  <Grid name="Grid_sampling">
    ...
    <variable name="AuxVariables|Temp|initial_condition:Rattlesnake">
      <distribution>uniform</distribution>
      <grid type="value" construction="custom">3.0 6.0</grid>
    </variable>
    <variable name="Materials|fuel_solid_mechanics_elastic|poissons_ratio:Bison">
...
\end{lstlisting}
%
In order to make the input variables of Rattlesnake be distinct from the input variables of Bison, \textbf{:Rattlesnake}
or \textbf{:Bison} is used at the end of each full MooseBasedApp model variable name. 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{Steps}
For a MAMMOTH interface, the \xmlNode{MultiRun} step type will most likely be used. First, the step needs
