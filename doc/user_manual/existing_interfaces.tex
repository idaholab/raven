...
as its \xmlAttr{type}. Some files may be necessary for codes to run, but need no perturbed variables from
RAVEN; they only need to be copied to each perturbed run directory.  In this case, one can use
the \xmlAttr{type} \xmlString{AncillaryInput} to specify that it is a part of the code run, but only needs to 
be copied (with no changes) to each run directory.  For Yak multigroup cross section libraries, 
...
the app's name followed by the '@' symbol is prepended to the variable name (e.g. \xmlString{appName@varName}).
...
variables itself.  Notice that the clad inner and outer diameter sampled parameters have \xmlString{Dummy}
specified as their destination app.  As these parameters are exclusively utilized to sample inputs
for the clad thickness and gap width variables in BISON, the \xmlString{Dummy} tag
allows passthrough of variables that aren't necessarily used in any app's input.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{Steps}
