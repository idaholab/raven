...
\label{subsec:genericCodeInterface}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Rattlesnake Interfaces}
%
...
The interface can be used to perturb the Rattlesnake MOOSE-based input file as well as the Yak multigroup
cross section libraries XML input files.
%
...
name for the file contained in the node, and \xmlAttr{type} identifies which base-level
interface the file is used within.  \xmlNode{type} should only be specified for inputs
that RAVEN will perturb.  For Rattlesnake input files, \xmlNode{type} should be \xmlString{RattlesnakeInput}; 
for Yak multigroup cross section libraries, the \xmlNode{type} should be \xmlString{YakXSInput}; and for the Yak 
alias files that are used to perturb the Yak multigroup cross section libraries, the \xmlNode{type} should be
\xmlString{YakXSAliasInput}. The node should contain the path to the file from the working directory.  
...
\end{lstlisting}
%
This node defines an alias with name \xmlString{f11} used to represent the fission cross section at energy group '1'
for material with name 'pseudo-seed1' at grid index '1' in the Yak multigroup cross section library input files.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{OutStreams,DataObjects}
This interface's, \xmlNode{OutStreams}, and
\xmlNode{DataObjects} blocks do not deviate significantly from
other interfaces' respective nodes.  Please refer to previous
entries for these blocks if needed.
...
the consequent RAVEN input adjustments and the modifications of the MAAP5 input 
...
files defining the different plotfils and the other files which could
...
<Files>
  ...
  <Input name="rep900-502a2.par" type="">rep900-502a2.par</Input>
</Files>
\end{lstlisting}
All the files here mentioned in this section
 need, then, to be put into the working directory specified 
by the \xmlNode{RunInfo} block.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\paragraph{Models}
The \xmlNode{Models} block contains the name of the executable file of MAAP5 (with the path, if necessary), 
and the name of the interface (e.g. MAAP5\_GenericV7).
In the specific case of MAAP5 interface, this block has also some additional required nodes:
\begin{itemize}
  \item \xmlNode{boolMaapOutputVariables}: containing the number of the MAAP5 $IEVNT$ corresponding to the boolean events of 
  interest;
  ...
  \item \xmlNode{stopSimulation}: this node is required only in case of DET-based sampling strategy. The user needs to specify
  if the MAAP5 simulation run stops due to the reached $END TIME$ ('mission\_time') or due to the occurrence of a specific event, 
  whose $IEVNT$ number needs to be inputted (e.g. ``core\_uncovery'' i.e. $IEVNT(691)$). 
  
\end{itemize}
A \xmlNode{Models} block is show as example below:
\begin{lstlisting}[style=XML]
<Models>
  <Code name="MyMAAP" subType="MAAP5\_GenericV7">
    <executable>MP502a2LINUX_opt.exe</executable>         
...
\begin{itemize}
  ...
  \item MAAP5 plotfil definition files
\end{itemize}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\paragraph{MAAP5 include file}
The adjustments explained in this section are required only in case of a DET analysis.
In this case, in fact, one of the include files needs to contain the definition of the
different timers, one for each variable that causes a branching.
...
need to be added into the include file (that usually contains a section for
the definition of all the timers to be used into the simulation) for the set of the timer corresponding
...
also for the event 'core\_uncovery', if this is the stop condition
for the MAAP5 run:
\begin{lstlisting}[style=XML]
...
(to the default value) and set within the include file into the user-defined variables section.
...
Only for the DET analysis, then, the occurrence of a branch is identified by a comment before. This comment is
...
one branching exists.
Within the block, the occurrence of the branching leads the value of a variable 
...
the value of these kind of variables (initialized within the user-defined
variables into the include file) of all the branches.
Therefore following the example, if we want that, when the diesels failure occurs it leads
...
Finally, also in this case only for DET analysis, a stop simulation condition (provided by the comment 
...
condition, even including that one of the event that stops the simulation (e.g. core\_uncovery), 
if any.
\begin{lstlisting}[style=XML]
...
be used by the two following branch to run the two branches simulation.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\paragraph{MAAP5 plotfil definition file}
These files in MAAP5 are used to define all the plotfil output (.csv) in terms of variables included in each plotfil. 
It is important that the MAAP5 csv output files contain the evolution of:
\begin{itemize}
  \item RAVEN sampled variables (e.g. DIESEL) (both for Forward and DET sampling)
  \item the variables whose value is modified by the occurrence of one of the branches, either continuous or boolean (e.g. 
  $IEVNT(225)$)
  \item the variables of interest defined within \xmlNode{boolMaapOutputVariables} and \xmlNode{contMaapOutputVariables} blocks (both for Forward and DET sampling)
\end{itemize}
If one of these variables won't be contained into one of csv files, RAVEN will give an error.
