...
\xmlNode{MultivariateNormal} distribution, and the values for \xmlNode{mu} should be \xmlString{ones}. In the other case, if 
the user choose \xmlAttr{Type="abs"}, the cross sections will be perturbed absolutely (i.e. perturbed by values), and 
the user needs to provide an absolute covariance matrix and specify \xmlString{zeros} for \xmlNode{mu} in \xmlNode{MultivariateNormal}
distribution.
