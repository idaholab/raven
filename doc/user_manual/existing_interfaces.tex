...
The \xmlNode{mrtauStandAlone} node informs whether or not MrTau is ran in standalone mode. The \xmlNode{mrtauStandAlone} accepts only a boolean entry (\xmlString{true}, \xmlString{t}, \xmlString{false},\xmlString{f}). It is case insensitive. 
Default: false. 
...
The \xmlNode{phisicsRelap} node is a boolean entry that indicates if PHISICS is used in standalone mode (\xmlString{False}), or in coupled PHISICS/RELAP5 mode (\xmlString{True}). The entry is case insensitive. Default: False. 

The \xmlNode{printSpatialRR} node indicates if the spatial reaction rates computed by PHISICS are to be included in the RAVEN csv output. The entry is case insensitive. 
...
The user enters the tabulation parameters and corresponding tabulation values with the \xmlNode{tabulation}, using the sub-node \xmlNode{tab}. 

\item [$\cdot$]If the user perturbs cross sections, an XML file 'scaled\_xs.xml' will be generated at each perturbation in the output folder. 
The file 'XS.xml' is automatically created from the cross section variables defined in the RAVEN \xmlNode{Sampler} block (see \ref{XSPara}). The cross section file 'XS.xml has the following format: 
...
The sub-node \xmlNode{XS} (where XS the perturbable type of cross sections are listed in \ref{XSPara}) provides the cross section information. 
...
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{Output Files Conversion} \label{outputFileConversion}

The PHISICS output available for RAVEN post-processing are described in this section. The PHISICS output are by convention separated by '$\vert$' if they are contained in a matrix form such as group-wise or region-wise values. 
In the PHISICS mode (i.e. \xmlNode{mrtauStandalone} is \xmlString{False}) The variables available for RAVEN post-processing are:
\begin{enumerate}
\item [$-$]the MrTau time; 
...
The variables template is provided in Table \ref{VariableTemplateTable}. In the table, the region number is taken equal to 4, the group number is taken equal to 7, the cell number equal to 2 and the material number equal to 3. 
...
$\nu$                       & neutron$\vert$gr7$\vert$reg4         & only if \xmlNode{printSpatialRR} is \xmlString{True})    \\
...
\end{tabular}
\end{table}
Where $\nu$ is the average number of neutrons generated after fission. Note that the material number in the neutron flux by material corresponds to the material ID number in the PHISICS csv output, 
while the material string ID in the decay heat corresponds to the material name given by the user in the xml material file. Hence, the neutron flux by material will always have the format matX, where X is an interger. 
The decay heat material is user-defined in the xml material file in the attribute \xmlAttr{id} of the \xmlNode{mat} node.  
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
For the files parsed by the PHISICS interface or the PHISICS interface's parsers, the \xmlAttr{type} attributes are hardcoded. 
...
The necessary RELAP5 files for use in the coupled PHISICS/RELAP mode within RAVEN are given in \ref{PhisicsRelap5TypeTable}. The files associated 
...
<File>
  ...
  <Input name="lic"       type="relapLicence" perturbable="False">license.bib</Input>
</File>
\end{lstlisting}
%
