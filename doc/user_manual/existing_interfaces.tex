...
Remeber also that a RELAP5 simulation run is considered succesfull (i.e., the simualtion did not crash) if it terminates with the following message:
...
Hence, it is strongly recommended to set up the RELAP5 input file so that the simualtion exiting conditions are set through control logic trip variables 
...
name for the file contained in the node, and \xmlAttr{type} identifies which base-level
interface the file is used within.  \xmlNode{type} should only be specified for inputs
that RAVEN will perturb.  For MAMMOTH input files, \xmlNode{type} should be \xmlString{MAMMOTHInput|appNameInput}, 
...
\end{lstlisting}
%
In order to make the input variables of one application distinct from the input variabels of another,
the app's name followed by the '@' symbol is prepended to the variable name (or alias) (e.g. \xmlString{appName@varName}).
Each variable sampled in the MAMMOTH interface is required to have a destination app specified.
All variables utilizing Rattlesnake's executable (whether they are in the
Rattlesnake input file or not) are listed as Rattlesnake variables
