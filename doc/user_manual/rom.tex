...
%
In addition, the temporal vairable for a \textbf{HistorySet} \xmlString{SciKitLearn}
is specified in \xmlNode{pivotParameter}:
\begin{itemize}
  \item \xmlNode{pivotParameter}, \xmlDesc{string, optional parameter}, specifies the pivot
    variable (e.g. time, etc) in the input HistorySet.
    \default{time}
\end{itemize}
\nb for HistorySet's \xmlString{SciKitLearn} performs the task given in \xmlNode{SKLtype} for
each time step. and so only synchronized HistorySet can be used as input to this ROM. For
