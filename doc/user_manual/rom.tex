  ...
  \item \xmlNode{Multiyear}, \xmlDesc{node, optional}, indicates that each sample of the ARMA should yield
    ...
    \begin{itemize}
      ...
      \item \xmlNode{growth}, \xmlDesc{float, optional field}, if provided then the histories produced by
        the ARMA will be multiplied by the growth factor for successive years; that is, the scaling factor
        is the value given here raised to the power of the year. For example, with a growth factor of 1.5,
        the first year would not be scaled, the second year would be multiplied by 1.5, the third year by 2.25,
        and so forth. This simulates growth in the mean value and proportional signal fluctuations each year.
    \end{itemize}
    \romClusterOption
\end{itemize}
