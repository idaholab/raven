      ...
      \item \xmlNode{evalMode}, \xmlDesc{string, optional field}, one of \xmlString{truncated},
        ...
        \begin{itemize}
          ...
          \item \xmlString{clustered}, reproduce a N-dimensional object with the variable
            \texttt{_ROM_cluster} as one of the indexes for the ROM's sampled variables. Note that
            in order to use the option, the receiving \xmlNode{DataObject} should be of type
            \xmlNode{DataSet} with one of the indices being \texttt{_ROM_cluster}.
        \end{itemize}
    \end{itemize}
}
...
If \xmlNode{InternalParallel} is \xmlString{False} and \xmlNode{batchSize} is more than 1, it is not possible to guarantee the order of RNG usage by the separate processes, so it is not possible to guarantee reproducible histories are generated.        \default{True}
  ...
  \item \xmlNode{SpecificFourier}, \xmlDesc{node, optional}, provides a means to specify different Fourier
    ...
    \end{itemize}
  \item \xmlNode{Multiyear}, \xmlDesc{node, optional}, indicates that each sample of the ARMA should yield
    multiple sequential samples. For example, if an ARMA model is trained to produce a year's worth of data,
    enabling \xmlNode{Multiyear} causes it to produce several successive years of data. Multiyear sampling
    is independent of ROM training, and only changes how samples of the ARMA are created.

    \nb The output of a multiyear ARMA must be stored in a \xmlNode{DataSet}, as the targets will depend
      on both the \xmlNode{pivotParameter} as well as the year, \xmlString{Year}. The year is a second
      \xmlNode{Index} that all targets should depend on, with variable name \xmlString{Year}.

    The \xmlNode{Multiyear} node accepts the following subnodes:
    \begin{itemize}
      \item \xmlNode{years}, \xmlDesc{integer, required field}, the number of years the ARMA should produce
        each time it yields a sample.
      \item \xmlNode{growth}, \xmlDesc{float, optional field}, if provided then the histories produced by
        the ARMA will be increased by the growth factor for successive years. This node can be added
        ...
        Table~\ref{tab:arma multiyear growth}, where \emph{Growth factor} is the value used in the RAVEN
        ...
        \begin{table}[h!]
          ...
          \begin{tabular}{r c l}
            ...
            50 & 1.5 & growing by 50\% each year \\
            -50 & 0.5 & shrinking by 50\% each year \\
            150 & 2.5 & growing by 150\% each year \\
          \end{tabular}
          \caption{ARMA Growth Factor Examples}
          \label{tab:arma multiyear growth}
        \end{table}
        ...
        \begin{itemize}
          ...
          \item \xmlAttr{mode}, \xmlDesc{string, required field}, either \xmlString{linear} or
            ...
            where $y$ is the year after the first and $g$ is the provided scaling factor.
        \end{itemize}
    \end{itemize}
    \romClusterOption
\end{itemize}
...
\begin{itemize}
  ...
  \item \xmlNode{Multiyear}, \xmlDesc{node, optional}, as \xmlNode{MultiYear} above, allows resetting
    a growth factor and number of years to sample.
...
\end{itemize}
the \xmlNode{seed}
as well as the \xmlNode{Multiyear} nodes can be added to change the behavior of the ARMA ROM.
