...
\begin{itemize}
  ...
  \item \xmlString{DMDC}, for time-dependent regression of linear system with Control
...
\begin{itemize}
  \item \xmlNode{dmdType}, \xmlDesc{string, optional field}, the type of Dynamic Mode Decomposition to apply. Available are:
    ...
    \end{itemize}
  \default{dmd}
  \item \xmlNode{pivotParameter}, \xmlDesc{string, required field}, defines the pivot variable (e.g., time) that represents the
  independent monotonic variable
  \default{time}
  ...
  listed in \xmlNode{StateVariables} must be listed in the  \xmlNode{Target} node too.
  
  \item \xmlNode{Target}, \xmlDesc{comma separated string, required field}, defines output FOMs that are going to be predicted.
  The pivot variable in \xmlNode{pivotParameter} is required in \xmlNode{Target}. In addition, it is optional to list each system 
  output variables (y1, y2, etc.).
    \begin{itemize} 
      \item \textit{pivotParameter}, for A and B matrices calculations only;
      \item \textit{pivotParameter, y(s)}, for A, B and C matrices calculations.
    \end{itemize}  
  ...
  \item \xmlNode{Actuators}, \xmlDesc{comma separated string, required field}, defines the actuators (i.e. system input parameters)
  of this model. Each actuator variables (u1, u2, etc.) needs to be listed here.
  \item \xmlNode{Features}, \xmlDesc{comma separated string, required field}, defines the features (i.e. system input and state 
  parameters) of this model. The pivot variables in \xmlNode{Actuators}, and each system state variable (x1, x2, etc.) are required in \xmlNode{Features}.
  \item \xmlNode{rankSVD}, \xmlDesc{integer, optional field}, defines the truncation rank to be used for the SVD.
     Available options are:
     \begin{itemize}
     \item \textit{-1}, no truncation is performed
...
<Simulation>
  ...
  <Models>
   ...
   <ROM name="DMDrom" subType="DMDC">
      ...
      <Features>u1,u2,x1,x2,x3</Features> 
      <rankSVD>-1</rankSVD>
...
\begin{itemize}
  ...
  \item \xmlNode{acturatorsCount}, XML node containing the number of actuators
  ...
  \item \xmlNode{statesCount}, XML node containing the number of state variables
  ...
  \item \xmlNode{outputsCount}, XML node containing the number of output variables
