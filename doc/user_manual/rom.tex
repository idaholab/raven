...
If \xmlNode{InternalParallel} is \xmlString{False} and \xmlNode{batchSize} is more than 1, it is not possible to guarantee the order of RNG usage by the separate processes, so it is not possible to guarantee reproducible histories are generated.        \default{True}
  ...
  \default{None}
   ...
   \item \xmlNode{Peaks}, \xmlDesc{node, optional}, designed to estimate the peaks in signal who has fundamental frequency, usually a data. The \xmlNode{Peaks} node has the following attributes:  
  	\begin{itemize}
    		\item \xmlAttr{target}, \xmlDesc{required string attribute}, defines the name of the target 
		 (not pivot parameter) expected to have periodic peaks.
    		\item \xmlAttr{threshold}, \xmlDesc{required float attribute}, user-defined minimal required 
		height of peaks.
	...
	\end{lstlisting}
	This means the 'speed' signal is a daily signal, and the minimal height required to define a peak is 0.
    Additionally, the \xmlNode{Peaks} requires the sub-node:
    	\begin{itemize}
      		 \item \xmlNode{window}, \xmlDesc{comma-separated floats, required field}, lists the
		 certain region in the periodic time for the peaks, user need to define the lower domain boundary and 
		 upper domain boundary for the window in signal unit. The first element is interpreted as the lower bound
		 and the second as the upper bound of the window. User can define the lower bound to be a negative
		 number if the window occupation passes through one side of one period. For example, if the period is 24
		 hours, the window can be -2,2 which is equivalent to 22, 2.
		 This node has one attribute, \xmlAttr{width}, \xmlDesc{required float attribute} which is the 
