...
characteristics as when it was pickled in a previous RAVEN run.  Specifically, if it was trained before
pickling, then it will be trained when it is loaded.
...
\begin{itemize}
  \item \xmlNode{pivotParameter}, \xmlDesc{string, optional field}, defines the pivot variable (e.g., time) that is non-decreasing in the input HistorySet.
  \default{Time}
  \item \xmlNode{Features}, \xmlDesc{string, required field}, defines the features (e.g., scaling). Note that only one feature is allowed for \xmlString{ARMA} and in current implementation this is used for evaluation only.
  \item \xmlNode{Target}, \xmlDesc{string, required field}, defines the variables of the time series.
