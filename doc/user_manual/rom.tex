...
this ROMs can be employed by other RAVEN entities to perform uncertainty quantification, model opimization and data analysis.
...
As shown in above example, in addition to the common subnodes \xmlNode{Target} and \xmlNode{Features}, the \xmlNode{ROM} can be initialized with
...
\begin{itemize}
  \item \xmlNode{loss}, \xmlDesc{string or comma separated string, optional field}, if the model has multiple outputs, you can use a different
    loss on each output by passing a list of losses. The loss value that will be minimized by the model will then
    be the sum of all individual losses. Available loss functions include \textit{mean\_squared\_error},
  ...
  \item \xmlNode{metrics}, \xmlDesc{string or comma separated string, optional field}, list of metrics to be evaluated by
    ...
    \textit{top\_k\_categorical\_accuracy}, 
    %
  ...
  \item \xmlNode{optimizerSetting}, \xmlDesc{optional field}, including several subnode depending on the type of
    ...
    \end{itemize}
    \default{Adam}
    \nb The users can also choose different optimizers to train the ROM. The default algorithm is \textit{Adam}. Other
    available optimizers include:
    \textit{SGD}, \textit{RMSprop}, \textit{Adagrad}, \textit{Adadelta}, \textit{Adamx}, \textit{Nadam}. The users
    can use \xmlNode{optimizerSetting} to set the parameters of the optimizer. 
    \begin{itemize}
...
In addition, most core layers will accept the \xmlNode{activation} subnode, and the optional input for this
subnode could be:
\begin{itemize}
  ...
  \item \textit{selu}, scaled exponential linear unit.
  ...
  \item \textit{hard\_sigmoid}
  \item \textit{linear}
  \item \textit{softmax}, softmax activation function.
\end{itemize}
...
\begin{itemize}
  \item \xmlNode{hidden\_layer\_sizes}, \xmlDesc{comma separated integers, required field}, the sizes for each
    hidden layer, the ith element represents the number of neurons in the ith hidden layer.
  \item \xmlNode{hidden\_layer\_activations}, \xmlDesc{comma separated strings, optional field}, 
    {`relu', `tanh', `elu', `selu', `softplus', `softsign', `sigmoid', `hard\_sigmoid', `linear', `softmax'}.
  \default{`relu'}
  \item \xmlNode{output\_layer\_activation}, \xmlDesc{string, optional field}.
    i.e. {`relu', `tanh', `elu', `selu', `softplus', `softsign', `sigmoid', `hard\_sigmoid', `linear', `softmax'}.
  \default{softmax}
  \item \xmlNode{hidden\_layer\_dropout}, \xmlDesc{integer or comma separated integers, optional field}, applies dropout to the input.
