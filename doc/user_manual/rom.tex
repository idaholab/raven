  ...
  \item \xmlNode{Multiyear}, \xmlDesc{node, optional}, indicates that each sample of the ARMA should yield
    ...
    \begin{itemize}
      ...
      \item \xmlNode{growth}, \xmlDesc{float, optional field}, if provided then the histories produced by
        ...
        The text of this node is the growth factor. For example,
        if the growth factor is 0.5, the scaling factor for the signal produced is 1.5, corresponding to a
        50\% growth rate.

        This node takes the following attributes as settings:
        \begin{itemize}
          ...
          \item \xmlAttr{mode}, \xmlDesc{string, required field}, either \xmlString{linear} or
            ...
            If \xmlString{linear}, then the scaling factor is $y\cdot (1+g)$;

            if \xmlString{exponential}, then the scaling factor is $(1+g)^y$;
...
Note that when loading the ARMA model from a serialized file as a \xmlNode{pickledROM}, the \xmlNode{seed}
