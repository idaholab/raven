...
\newcommand{\romClusterOption}[0]
{
  \item \xmlNode{Cluster}, \xmlDesc{node, optional}, provides an alternative way to build the ROM. When
    this mode is enabled, the subspace of the ROM (usually ``time'') will be divided into segments as
    ...
    \begin{itemize}
      \item \xmlNode{subspace}, \xmlDesc{string, required field} designates the subspace to divide. This
        should be the pivot parameter (often ``time'') for the ROM. This node also requires one of the two
        following attributes, used to determine how the subspace is divided:
        \begin{itemize}
