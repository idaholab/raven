...
\begin{itemize}
  ...
  \item \xmlNode{seed}, \xmlDesc{integer, optional field}, provides seed for VARMA and ARMA sampling. 
   Must be provided before training; it cannot be changed once trained. If no seed is assigned, 
   then use random number instead.
   
   \item \xmlNode{reseedCopies}, \xmlDesc{boolean, optional field}, if \xmlString{True} then whenever the ARMA is copied, a
    random reseeding will be performed to ensure different histories. \nb If same histories are expected for an unpickled arma,
    user should set the \xmlNode{reseedCopies} to \xmlString{False}, also needs to make sure the \xmlNode{batchSize} is 1 and  
    \xmlNode{internalParallel} is \xmlString{False} in \xmlNode{RunInfo} block for both the arma before pickled and after unpickled.
    When \xmlNode{internalParallel} is \xmlString{True}, arma can only generate one set of history, regardless of how many sets of
    histories are expected. While \xmlNode{batchSize} is larger than 1 will cause the genrated values out of order.
        \default{True}
