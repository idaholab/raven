...
\subsection{GeneticAlgorithm}
  ...
  evolutionary computation 6.2 (2002): 182-197.\\\\                              Internally, GA and
  NSGA-II store \texttt{minObjVals} as objective values transformed into RAVEN's minimization
  convention.                             Objectives declared as \texttt{min} are unchanged, while
  objectives declared as \texttt{max} are multiplied by -1 before Pareto ranking,
  crowding-distance calculation, convergence checks, and fitness calculations. User-facing objective
  values should be interpreted                             through the requested minimization or
  maximization convention when exported.\\                             The implementation keeps
  \texttt{minObjVals}, \texttt{externalObjVals}, and \texttt{fitVals} separate: \texttt{minObjVals}
  drives algorithmic ranking, crowding distance, convergence, and fitness calculation;
  \texttt{externalObjVals} is used only when converting back to user-facing signs for exports or
  implicit constraints; and \texttt{fitVals} is the scalar or per-objective fitness container used
  by parent and survivor selection.\\                              GA in RAVEN supports for both
        ...
        \item \xmlNode{reproduction}:
          ...
          \begin{itemize}
            ...
            \item \xmlNode{crossover}: \xmlDesc{string},
              ...
              corresponding positions.                     \item \textit{sbxCrossover} - Simulated
              Binary Crossover (Deb \& Agrawal, 1995) for real-valued variables; produces offspring
              distributed around the parents within the variable bounds, controlled by a
              distribution index. Recommended for continuous multi-objective problems.
              \end{itemize}
              The \xmlNode{crossover} node recognizes the following parameters:
                \begin{itemize}
                  \item \xmlAttr{type}: \xmlDesc{[onePointCrossover, twoPointsCrossover, uniformCrossover, sbxCrossover], required},
            ...
            \item \xmlNode{mutation}: \xmlDesc{string},
              ...
              mutates the gene.                   \item \textit{polynomialMutator} - Polynomial
              mutation (Deb \& Goyal, 1996) for real-valued variables; perturbs a gene by a bounded,
              polynomial-distributed step controlled by a distribution index. Recommended for
              continuous multi-objective problems, paired with sbxCrossover.
              \end{itemize}
              The \xmlNode{mutation} node recognizes the following parameters:
                \begin{itemize}
                  \item \xmlAttr{type}: \xmlDesc{[swapMutator, scrambleMutator, inversionMutator, randomMutator, polynomialMutator], required},
...
\vspace{7pt} \\When used as part of a \xmlNode{MultiRun} step, this entity provides
  ...
  The \xmlNode{MultiObjectiveGeneticAlgorithm} node recognizes the following parameters:
    \begin{itemize}
      ...
      \item \xmlAttr{verbosity}: \xmlDesc{[silent, quiet, all, debug], optional},
        Desired verbosity of messages coming from this entity
      \item \xmlAttr{type}: \xmlDesc{[NSGA-II], optional},
        selects the concrete multi-objective genetic algorithm to run. Currently \xmlString{NSGA-II}
        (Non-dominated Sorting Genetic Algorithm II) is available. Defaults to \xmlString{NSGA-II}. \default{NSGA-II}
  \end{itemize}
  ...
  \begin{itemize}
    ...
    \item \xmlNode{convergence}:
      ...
      \begin{itemize}
        ...
        \item \xmlNode{spread}: \xmlDesc{float},
          provides the maximum allowable value of Deb's spread metric ($\Delta$) measuring the
          distance                        between extreme and intermediate Pareto points. Once the
          spread drops below this value                        the algorithm is considered
          converged.
        ...
        \item \xmlNode{hypervolume}: \xmlDesc{float},
          sets the relative-change tolerance for the Pareto-front hypervolume indicator
    ...
    \item \xmlNode{crowdingDistanceNormalization}: \xmlDesc{[front, population]},
      selects how objective gaps are normalized when computing the NSGA-II crowding distance.
  ...
  \default{front}
    ...
    \item \xmlNode{paretoArchive}: \xmlDesc{[True, Yes, 1, False, No, 0, t, y, 1, f, n, 0]},
      if True, maintain an external archive of the non-dominated (rank-1) solutions found
  ...
  \default{False}
      The \xmlNode{paretoArchive} node recognizes the following parameters:
        \begin{itemize}
          \item \xmlAttr{maxSize}: \xmlDesc{integer, optional},
            optional cap on the number of solutions retained in the archive. When the archive
    ...
    \item \xmlNode{constraintEpsilon}: \xmlDesc{float},
      enables epsilon-constrained dominance (Takahama \& Sato) in the non-dominated sorting:
  ...
  \default{0.0}
    ...
    \item \xmlNode{adaptiveMutation}: \xmlDesc{[True, Yes, 1, False, No, 0, t, y, 1, f, n, 0]},
      if True, anneal the mutation probability linearly across the generation budget, from the
  ...
  \default{False}
      The \xmlNode{adaptiveMutation} node recognizes the following parameters:
        \begin{itemize}
          \item \xmlAttr{final}: \xmlDesc{float, optional},
            the mutation probability reached at the final generation when \xmlNode{adaptiveMutation}
            is True. If omitted it defaults to 1/(number of decision variables), the customary
            per-gene NSGA-II mutation rate.
      \end{itemize}

    \item \xmlNode{stochasticRanking}: \xmlDesc{[True, Yes, 1, False, No, 0, t, y, 1, f, n, 0]},
      if True, use stochastic ranking (Runarsson \& Yao) for constraint handling: each
      ...
      to False.
  \default{False}
      The \xmlNode{stochasticRanking} node recognizes the following parameters:
        \begin{itemize}
          \item \xmlAttr{pf}: \xmlDesc{float, optional},
            probability of ranking by objectives only (ignoring constraints) in a given generation
