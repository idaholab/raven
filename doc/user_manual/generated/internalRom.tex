...
\subsubsection{SyntheticHistory}
  ...
  \default{time}
    ...
    \item \xmlNode{saveResiduals}: \xmlDesc{[True, Yes, 1, False, No, 0, t, y, 1, f, n, 0]},
  ...
  \default{False}
    ...
    \item \xmlNode{zerofilter}:
      masks values that are near zero. The masked values are replaced with a fill     value
      (defaults to NaN). Caution should be used when using this algorithm because not all algorithms
      can handle NaN values! A warning will be issued if NaN values are detected in the input of an
      ...
      The \xmlNode{zerofilter} node recognizes the following parameters:
        \begin{itemize}
          ...
          \item \xmlAttr{global}: \xmlDesc{[True, Yes, 1, False, No, 0, t, y, 1, f, n, 0], optional},
            ...
            which case the this 'global' parameter should be set to True for better
            fitting. \default{False}
          \item \xmlAttr{fill}: \xmlDesc{float or string, optional},
            fill strategy for masked values. May be one of 'drop' or a float value.
            If 'drop', the masked values are dropped and replaced with NaN. If a float value,
            that value is used to fill the masked values. \default{drop}
          \item \xmlAttr{tol}: \xmlDesc{float, optional},
            absolute tolerance about zero for which to apply the filter \default{1e-08}
      \end{itemize}
    ...
    \item \xmlNode{bounddomain}:
      applies the transformation function       \begin{align}         f(x) = \begin{cases}
      L & x < L \\           x & L \leq x \leq U \\           U & x > U         \end{cases}
      \end{align}     for both the forward and inverse transformations to limit the training
      signal and samples to the interval $[L, U]$.
      The \xmlNode{bounddomain} node recognizes the following parameters:
        \begin{itemize}
          \item \xmlAttr{target}: \xmlDesc{comma-separated strings, required},
            indicates the variables for which this algorithm will be used for characterization.
          \item \xmlAttr{seed}: \xmlDesc{integer, optional},
      ...
      The \xmlNode{bounddomain} node recognizes the following subnodes:
      \begin{itemize}
        \item \xmlNode{lowerBound}: \xmlDesc{float},
          Bounding interval lower bound $L$
  \default{-inf}

        \item \xmlNode{upperBound}: \xmlDesc{float},
          Bounding interval upper bound $U$
  \default{inf}
      \end{itemize}
...
\subsubsection{MultiResolutionTSA}
  ...
  In order to use this Reduced Order Model, the \xmlNode{ROM} attribute \xmlAttr{subType} needs to
  be       \xmlString{MultiResolutionTSA}. It must also have a SegmentROM node of \xmlAttr{subType}
  \xmlString{decomposition}
