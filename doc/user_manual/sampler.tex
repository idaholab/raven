...
Because of the tight coupling between the Sampler and the ROM in stochastic collocation for generalized polynomial chaos, the Sampler needs access to the ROM via the assembler do determine the polynomials, quadratures, and importance weights to use in each dimension (see \ref{subsubsec:GaussPolynomialRom}).  It also needs access to the TimePointSet where solutions are being stored, so it knows when to inform the ROM.  \talbpaul{I wonder if this could be done after Sampler is initiated instead of waiting until the sampling is done...} Both the TimePointSet and the ROM are specified through an Assembler tag: \talbpaul{TODO I recognize the Assembler node is dead; however, until assembly is done in the BaseClass, I'm still making use of it.}
\begin{itemize}
