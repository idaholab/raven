...
%\talbpaul{This seems really specific to the MooseBasedApp interface.  Do we want it here?}
...
In addition, \xmlNode{sampler\textunderscore init} might contains the information regarding the random generator function for each $N$-Dimensional distribution (specified in the \xmlNode{dist\textunderscore init} node):
\begin{itemize}
...
In the \xmlNode{sampler\textunderscore init}  node it is possible to add also the subnode \xmlNode{global\_grid}.
The \xmlNode{global\_grid} can be used in two cases:
\begin{itemize}
\item 1D distributions: an identical grid that is associated to several distributions
\item ND distribution: a grid associated to a single ND distribution. This is the case when a stratified sampling is performed on the CDF of an ND distribution: the  \xmlNode{global\_grid} is  shared among the variables associated to the Nd distribution
\end{itemize}
...
\begin{itemize}
\item \xmlNode{sampler\textunderscore init},  \textit{\textbf{XML node, required parameter}}. In this xml-node,the following xml sub-nodes need to be specified:
  \begin{itemize}
    \item \xmlNode{limit}, \textit{\textbf{integer,required field}}, number of MonteCarlo samples needs to be generated;
    \item \xmlNode{initial\textunderscore seed}, \textit{\textbf{integer, optional field}}, initial seeding of random number generator
    \item \xmlNode{reseed\_at\_each\_iteration},  \textit{\textbf{boolean/string(case insensitive), optional field}}, perform a re-seeding for each sample generated (True values = True, yes, y, t). \default{False};
    \item \xmlNode{dist\textunderscore init},  \textit{\textbf{integer, optional field}}, in this node the user specifies the initialization of the random number generator function for each N-Dimensional Probability Distributions (see Section~\ref{subsec:NdDist}).
  \end{itemize}
\end{itemize}
\begin{itemize}
...
\nb{SparseGridCollocation requires uncorrelated input dimensions; thus, for the present, it is not compatible with NDDistribution objects.}
\begin{itemize}
\itemsep0em
\item \nameDescription
...
<Samplers>
  ...
  <SparseGridCollocation name="mySG" parallel="0">
    ...
    <Restart class = 'Datas' type = 'PointSet' >solns</Restart>
...
\nb{Sobol sampler relies on SparseGridCollocation, which requires uncorrelated input dimensions; thus, for the present,
it is not compatible with NDDistribution objects.}
\begin{itemize}
\itemsep0em
\item \nameDescription
...
This is possible by defining a \xmlNode{global\_grid} node and associate such global\_grid to each variable belonging to the ND distribution as follows.
...
\item \variableDescription
 ...
 \begin{itemize}
    \item \distributionDescription
     ...
     It needs to be noticed that only the construction ``custom'' is available. In the \xmlNode{grid} body only the lower and upper bounds can be inputted (2 numbers only).
 \end{itemize}
\end{itemize}
...
\specBlock{an}{Limit Surface Search}
%
\attrIntro
...
\variableIntro{Limit Surface Search}
...
\begin{itemize}
  ...
  \item \assemblerDescription{Limit Surface Search}
    \begin{itemize}
...
In addition to the \xmlNode{variable} nodes, the main XML node 
\xmlNode{Adaptive Sparse Grid} needs to contain two supplementary sub-nodes:
  ...
  \item \xmlNode{Convergence}, \xmlDesc{float, required field}, Convergence 
    ...
    \begin{itemize}
      \item \xmlAttr{target}, \xmlDesc{required string attribute}, the metric for convergence.
        Currently there are three metrics.  In order of strictness: \xmlString{mean}, which converges the sparse 
        quadrature integration of the first moment of the model; \xmlString{variance}, which
