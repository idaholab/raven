...
\newcommand{\variablesTransformationDescription}[1]
{
    \begin{itemize}
      ...
      \begin{itemize}
        \item \xmlAttr{model}, \xmlDesc{required string attribute}, the name for the model defined in the XML node \xmlNode {Models}. 
        This attribute indicates the \xmlNode{manifestVariables} defined in the subnode will be used by the \xmlAttr{model}.
      \end{itemize}
      ...
      \begin{itemize}
        ...
        that are used by the \xmlAttr{model}.
        \item \xmlNode{method}, \xmlDesc{string, required field}, the method that is used for the variables transformation.
      \end{itemize}
    \end{itemize}
}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
This is possible by defining a \xmlNode{globalGrid} node and associate such \xmlNode{globalGrid} to each variable belonging to the ND distribution as follows.
...
\subsubsection{Limit Surface Batch Search}
\label{subsubsubsec:LimitSurfaceBatchSearch}
The \textbf{Limit Surface Batch Search} approach is very similar to the
\textbf{Limit Surface Search} with the added benefit of being able to specify
advanced criteria such as the querying points in a batch fashion and also being
able to switch between different scoring functions. The addition of the ``batch''
mode makes use of the topology of the scoring function's landscape restricted to
the limit surface in order to select several points at a time before retraining
the surrogate model.
%

\specBlock{a}{LimitSurfaceBatchSearch}
%
\attrIntro
...
\variableIntro{LimitSurfaceBatchSearch}
...
In addition to the \xmlNode{variable} nodes, the main XML node
\xmlNode{LimitSurfaceBatchSearch} may specify the following supplementary
sub-nodes:
...
\begin{itemize}
  ...
  \default{0}.
  % Limit Surface Batch Search Objects
  \item \assemblerDescription{LimitSurfaceBatchSearch}
    \begin{itemize}
      \item \xmlNode{Function}, \xmlDesc{string, required field},  the
        body of this XML block needs to contain the name of an external
        function object defined within the \xmlNode{Functions} main block (see
...
<Samplers>
  ...
  <LimitSurfaceBatchSearch name='LSBSName'>
    <ROM class='Models' type='ROM'>ROMname</ROM>
    <Function class='Functions' type='External' >FunctionName</Function>
    <TargetEvaluation class='DataObjects' type='PointSet'>DataName</TargetEvaluation>
    <Convergence limit='3000'  forceIteration='False' weight='CDF'  subGridTol='1e-4' persistence='5'>
