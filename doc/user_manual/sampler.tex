...
\newcommand{\restartDescription}[1]
{
    \begin{itemize}
      \item \xmlNode{Restart}, \xmlDesc{string, optional field}, the
        body of this XML node must contain the name of an appropriate \textbf{TimePointSet} defined in the
  ...
  \item \assemblerDescription{Limit Surface Search}
    \begin{itemize}
      ...
      \item \xmlNode{TargetEvaluation}, \xmlDesc{string, required field},
        ...
        %
        The Limit Surface Search sampling accepts ``DataObjects'' of type ``TimePoint'' and
        ``TimePointSet'' only.
    \end{itemize}
\end{itemize}
...
<Samplers>
  ...
  <Limit Surface Search name='LSSName'>
    <ROM class='Models' type='ROM'>ROMname</ROM>
    <Function class='Functions' type='External' >FunctionName</Function>
    <TargetEvaluation class='DataObjects' type='TimePointSet'>DataName</TargetEvaluation>
  ...
  \item \assemblerDescription{AdaptiveDynamicEventTree}
    \begin{itemize}
      ...
      \item \xmlNode{TargetEvaluation}, \xmlDesc{string, required field},
        ...
        Section~\ref{sec:DataObjects}).
        %
        The adaptive sampling accepts ``DataObjects'' of type ``TimePoint'' and
        ``TimePointSet'' only.
    \end{itemize}
\end{itemize}
...
in a similar manner to Sparse Grid Collaction samplers.  In each iterative step, the adaptive
...
\end{itemize}
  ...
  \assemblerDescription{Adaptive Sparse Grid}
      ...
      \item \xmlNode{TargetEvaluation}, \xmlDesc{string, required field}, 
        ...
        Section~\ref{sec:DataObjects}).
        %
        The Adaptive Sparse Grid sampling accepts ``DataObjects'' of type ``TimePoint'' and
        ``TimePointSet'' only.
   % \end{itemize}
\end{itemize}
