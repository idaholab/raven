...
\item \xmlNode{samplerInit},  \textit{\textbf{XML node, required parameter}}. In this xml-node,the following xml sub-nodes need to be specified:
  \begin{itemize}
    ...
    \item \xmlNode{reseedAtEachIteration},  \textit{\textbf{boolean/string(case insensitive), optional field}}, perform a re-seeding for each sample generated (True values = True, yes, y, t). \default{False};
...
In addition to the \xmlNode{variable} nodes, the main XML node
\xmlNode{Adaptive Sobol} needs to contain two supplementary sub-nodes:

\begin{itemize}
  \item \xmlNode{Convergence}, \xmlDesc{float, required field}, Convergence
    tolerance.  This is the relative tolerance for converging the variance of the response ROM.
    %
    \begin{itemize}
      \item \xmlAttr{maxRuns}, \xmlDesc{optional string attribute},
    ...
    In summary, this XML node contains the information that is needed in order
    to control this sampler's convergence criterion.
  \item \xmlNode{maxSobolOrder}, \xmlDesc{optional field}, the maximum cardinality of subsets to consider when
    performing the adaptive decomposition.  For example, for a model $f(a,b,c)$, a max Sobol order of 1
    results in subsets $(a),(b),(c)$, while a max Sobol order of 2 additionally considers $(a,b), (a,c),
    (b,c)$.  Experience has shown that many problems converge well with a max Sobol order of 2, which is the
    default.

\end{itemize}
  % Adaptive Sobol
  \assemblerDescription{Adaptive Sobol}
       \ROMDescription{Adaptive Sobol}
        %
        \begin{itemize}
      \item \xmlNode{TargetEvaluation}, \xmlDesc{string, required field},
        represents the container where the system evaluations are stored.
...
<Samplers>
  ...
  <AdaptiveSobol name="AS" verbosity='debug'>
    <Convergence maxRuns='100'>1e-7</Convergence>
