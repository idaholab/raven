...
In addition to the \xmlNode{variable} nodes, the main XML node
\xmlNode{Adaptive Sparse Grid} needs to contain the following supplementary sub-nodes:
...
\begin{itemize}
  \item \xmlNode{Convergence}, \xmlDesc{float, required field}, Convergence
    ...
    In summary, this XML node contains the information that is needed in order
    to control this sampler's convergence criterion.
  \item \xmlNode{convergenceStudy}, \xmlDesc{optional node},
    ...
    OutStreamManager for ROMs at the state points requested, using \xmlString{all} as the requested
    \xmlNode{what} values.
...
In addition to the \xmlNode{variable} nodes, the main XML node
\xmlNode{Adaptive Sobol} needs to contain the following supplementary sub-nodes:
