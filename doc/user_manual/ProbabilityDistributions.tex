...
\begin{itemize}
  ...
  \begin{itemize}
    \item \xmlAttr{type}, \xmlDesc{string, optional field}, specifies the type of covariance. Possible values for \xmlAttr{type} are \xmlString{abs} and \xmlString{rel}. \nb currently, method \xmlString{pca} can be combined with both types, and method \xmlString{spline} only accept the type \xmlString{abs}
  \end{itemize}
  \item \xmlNode{reduction}, \xmlDesc{XML node, optional field}, option to enable input parameter reduction using PCA approach. The content will specify two attributes:
  \begin{itemize}
    \item \xmlAttr{rank}, \xmlDesc{positive integer, required field}, user-defined dimensionality reduction.
    \item \xmlAttr{variables}, \xmlDesc{comma separated strings, required field},list of the names of input variables, the content of this XML node depends on the definition of the associated attributes:
    \begin{itemize}
      \item \xmlAttr{type}, \xmlDesc{required string attribute}, user-defined type for the list. \nb only \xmlString{model} is accepted now.
    \end{itemize}
  \end{itemize}
\end{itemize}
...
<Distributions>
  ...
    <MultivariateNormal name='MultivariateNormal_test' method='pca'>
        ...
        <reduction>
          <rank>2</rank>
          <variables type="model">x1,y1</variables>
        </reduction>
    </MultivariateNormal>
  ...
</Distributions>
