...
\begin{itemize}
\itemsep0em
\item \textbf{name}, \textit{required string attribute}, user-defined name of the distribution
\end{itemize}
\vspace{-5mm}
...
\begin{itemize}
\item \xmlNode{p}, float, required parameter, power parameter. Greater values of p assign greater influence to values closest to the interpolated point.
\item \xmlNode{data\textunderscore filename}, string,  name of the data file containing scattered values (file type .txt). The type attribute indicate if the data in indicated file is PDF or CDF. 
\item \xmlNode{working\textunderscore dir}, string, folder location of the data file
\end{itemize}
