...
\end{lstlisting}
Each distribution has a pre-defined support (domain) based on its definition.
For example, the Normal distribution domain is $[-\infty,+\infty]$.
%
...
The specifications of this distribution must be defined within the xml block \xmlNode{MultivariateNormal}.  In RAVEN the MultivariateNormal distribution can be initialized through the following keywords:
\begin{itemize}
\item \xmlNode{mu}, list of mean values of each dimension
\item \xmlNode{data\textunderscore filename}, \xmlDesc{string, required parameter}, name of the data file containing the covariance matrix (file type .txt).
\item \xmlNode{working\textunderscore dir}, \xmlDesc{string}, folder location of the data\textunderscore filename
\end{itemize}
...
Example covariance matrix data file:
\begin{lstlisting}
1.0 0.7
0.7 1.0
\end{lstlisting}
