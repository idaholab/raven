...
%\begin{algorithmic}[1]
%\STATE $S \leftarrow 0$
%\STATE $\Sigma$ $\leftarrow 0$ Create $N \times d$ data matrix, with one row vector $x_{n}$ per data point
%\STATE X  subtract mean x from each row vector $x_{n}$ in X
%\STATE S $S \leftarrow 0$ covariance matrix of X
%\STATE Find eigenvectors and eigenvalues of $\Sigma$
%\STATE PCs $S \leftarrow 0$ the $M$ eigenvectors with largest eigenvalues
%\end{algorithmic}
