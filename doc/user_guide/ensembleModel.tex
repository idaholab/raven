...
ROM?  To maintain consistency, we leave the \xmlNode{Input} node in place.  Instead of specifying a file, a
placeholder data object can be used instead.  A placeholder data object has variables listed in its input, but
for output either the \xmlNode{Output} is omitted, or the special keyword \texttt{OutputPlaceHolder} is used
instead.  For example, see the excerpt from a regression test:
\xmlExample{framework/ensembleModelTests/index_input_output.xml}{DataObjects.first_in}
The inputs
