...
type of step is defined within a \xmlNode{PostProcess} XML block. As for the oher objects, the attribute 
...
\begin{itemize}
  \item \xmlNode{Input}: accepts \textbf{Files}, \textbf{DataObjects} and \textbf{Databases}.
  ...
  \item \xmlNode{Output}: accepts \textbf{Files}, \textbf{DataObjects}, \textbf{Databases} and \textbf{OutStreams}.
\end{itemize}
...
One can use the node attribute \xmlAttr{subType} to select which of the PostProcessors to be used. As with other
...
The \xmlNode{Files} will be used to include all input and output files. In this example, the single input file
...
\begin{itemize}
  \item \xmlNode{MultiRun} named ``sampleMC'', used to run the multiple instances of the driven code and
     ...
     strategy.
  \item \xmlNode{PostProcess} named ``statAnalysis\_1'', used compute all the statistical moments and FOMs
  ...
  \item \xmlNode{PostProcess} named ``statAnalysis\_2'', used compute all the statistical moments and FOMs
...
%\xmlExample{framework/user_guide/ravenTutorial/gold/StatisticalAnaly/output_stat_analysis_static.xml}
