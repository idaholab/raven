...
\begin{equation}
\boldsymbol{\Gamma}(\boldsymbol{X},\boldsymbol{Y}) = \frac{\boldsymbol{\Sigma}(\boldsymbol{X},\boldsymbol{Y})}{\sqrt{diag(\boldsymbol{\Sigma}(\boldsymbol{X},\boldsymbol{Y}))diag(\boldsymbol{\Sigma}(\boldsymbol{X},\boldsymbol{Y}))}}
\end{equation}
...
\end{lstlisting}
   ...
   needs to be performed. In this specific case, two steps  (\xmlNode{Sequence}) are going to be sequentially run 
   using 40 processors (\xmlNode{batchSize}). 
   \\In the first step, the original physical model is going to be sampled. The obtained results are going to be analyzed with the Statistical Post-Processor. 
...
\end{lstlisting}
 As mentioned above, the goal of this example is to show how the 
 principal statistical Figure of Merits can be computed through RAVEN.
 \\Indeed, in addition to the previously explained Code 
 model, a Post-Processor model (BasicStatistics) is here specified. 
As it can be noticed, the post-processing process is going to be
