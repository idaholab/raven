...
tests in a PBS environment to test the parallel code.
...
The dynamic tests have a variety of objectives.  First of all since
they are run before any merge requests are approved, they catch many
changes in the code that cause unexpected failures.  Second of all,
the dynamic tests detect many changes in the underlying libraries or
environments.  Both of these help find potential issues close to when
they first appear, instead of later when they are harder to find the
cause of, or worse, by users.
...
requirements test must be reviewed by the lead developer or the
project manager in addition to a second developer.
...
testing scripts) will be stored in CIVIT on moosebuild.inl.gov.
