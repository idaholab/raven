...
class hdf5Database(object):
    ...
    def __init__(self,name,type):
      ...
      self.h5_file_w    = h5.File(self.onDiskFile,'r+')
    ...
    def addGroup(self,gname,attributes,source):
      if source['type'] == 'csv':
        ...
        sgrp.attrs["n_params"]    = len(headers)        
        sgrp.attrs["parent"]     = parent_group_name
        sgrp.attrs["start_time"] = data[0,0]
        sgrp.attrs["end_time"]   = data[0,data[0,:].size-1]
    ...
    def retrieveHistory(self,name,filter=None):
      ...
      for i in xrange(len(self.allGroupPaths)):
        ...
        if list_str_w[len(list_str)-1] == name:
          found = True
      ...
      if found:
        ...
        if not filter or filter == 0:
          ...
          for i in xrange(len(where_list)):
            ...
            dataset = grp.require_dataset(name=name_list[i] +'_data', (grp.attrs('n_params'),grp.attrs('n_ts')), dtype='float', exact=True)
            if i == 0:
          ...
          attrs["end_time"]        = result[0,result[0,:].size-1]
