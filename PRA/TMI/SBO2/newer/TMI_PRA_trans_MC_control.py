...
def control_function(monitored, controlled, auxiliary):
    ...
    #auxiliary.DeltaTimeScramToAux = distcont.randGen('auxBackUpTimeDist',random_n_1)   
        auxiliary.DeltaTimeScramToAux = 4000
        #random_n_8 = distcont.random()
        #auxiliary.scram_start_time = auxiliary.scram_start_time + distcont.randGen('noise',random_n_8)     
    ...
    if auxiliary.ScramStatus: #we are in scram     
        ...
        if controlled.Head_PumpB>1.e-4*8.9:
            ...
            noise_b = 0 #distcont.randGen('noise',random_n_3)
            ...
            if monitored.time<(auxiliary.scram_start_time+auxiliary.DeltaTimeScramToAux): # not yet auxiliary system up
                controlled.Head_PumpB = toolcont.compute('PumpCoastDown',monitored.time-auxiliary.scram_start_time) + noise_b 
        ...
        if controlled.Head_PumpA>1.e-4*8.9:
            ...
            noise_a = 0# distcont.randGen('noise',random_n_4)
            if monitored.time<(auxiliary.scram_start_time+auxiliary.DeltaTimeScramToAux): # not yet auxiliary system up
                controlled.Head_PumpA = toolcont.compute('PumpCoastDown',monitored.time-auxiliary.scram_start_time) + noise_a  
    ...
    if monitored.time<(auxiliary.scram_start_time+auxiliary.DeltaTimeScramToAux) and auxiliary.ScramStatus: # not yet auxiliary system up
        ...
        controlled.MassFlowRateIn_SC_B = 2.432*toolcont.compute('PumpCoastDownSec',monitored.time-auxiliary.scram_start_time) 
        controlled.MassFlowRateIn_SC_A = 2.432*toolcont.compute('PumpCoastDownSec',monitored.time-auxiliary.scram_start_time)
