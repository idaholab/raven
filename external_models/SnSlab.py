...
sigma_t = 1*np.ones(cells).transpose();             # Total cross section
c = 0.2*np.ones(cells).transpose();                 # Scattering ratio
...
src = 2*np.ones(cells).transpose();                  # Internal source
tol = 1e-6;
...
flux = np.zeros(cells).transpose();	                 # Scalar flux
flux_old = np.zeros(cells).transpose(); 	         # Old flux
flux_oo = np.zeros(cells).transpose();
...
delta_x = (length/cells)*np.ones(cells).transpose(); # Delta_X vector can be varies but here we choose same dx
