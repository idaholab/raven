...
class Tester:
  ...
  def get_valid_params():
    ...
    params.add_param('min_python_version', 'none', 'The Minimum python version required for this test. Example 3.8 (note, format is major.minor)')
    return params
  ...
  def _run_backend(self, _):
    ...
    if self.specs['min_python_version'].strip().lower() != 'none':
      ...
      if int(major) > sys.version_info.major or (int(major) == sys.version_info.major and int(minor) > sys.version_info.minor):
        self.results.group = self.group_skip
        self.results.message = "skipped because need python version "+self.specs['min_python_version'].strip()+" but have "+sys.version
        return self.results
