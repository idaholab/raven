  ...
  def modifySimulation(self):
    ...
    try:
      if multiprocessing.cpu_count() < self.__simulation.runInfoDict['batchSize']:
        self.raiseAWarning(self,"cpu_count "+str(multiprocessing.cpu_count())+" < batchSize "+str(self.__simulation.runInfoDict['batchSize']))
...
def createAndRunQSUB(simulation):
  ...
  if any(char not in validChars for char in jobName):
    self.raiseAnError(IOError,'SIMULATION->QSUB:','JobName can only contain alphanumeric and "_", "-" characters! Received'+jobName)
  #check jobName for length
  if len(jobName) > 15:
    jobName = jobName[:10]+'-'+jobName[-4:]
    self.raiseAMessage('SIMULATION->QSUB','JobName is limited to 15 characters; truncating to '+jobName)
  ...
  self.raiseAMessage('SIMULATION->QSUB',os.getcwd()+' '+command)
  ...
  def modifySimulation(self):
    if self.__nodefile or self.__in_pbs:
      ...
      if newBatchsize != oldBatchsize:
        ...
        self.raiseAWarning(self,"changing batchsize from",oldBatchsize,"to",newBatchsize)
      if newBatchsize > 1:
    ...
    self.raiseAMessage('SIMULATION',"precommand: "+self.__simulation.runInfoDict['precommand']+", postcommand: "+self.__simulation.runInfoDict['postcommand'])
  ...
  def XMLread(self, xmlNode):
    for child in xmlNode:
      if child.tag == "nodefileenv":
        ...
        self.raiseAMessage(self,"We should do something with child "+str(child))
    return
...
class Simulation(MessageHandler.MessageUser):
  ...
  def __checkExistPath(self,filein):
    ...
    if not os.path.exists(self.filesDict[filein]): self.raiseAnError(IOError,self,'The file '+ filein +' has not been found')
  ...
  def XMLread(self,xmlNode,runInfoSkip = set(),xmlFilename=None):
    ...
    except: self.raiseAnError(IOError,self,'The run info node is mandatory')
    self.__readRunInfo(runInfoNode,runInfoSkip,xmlFilename)
    for child in xmlNode:
      if child.tag in list(self.whichDict.keys()):
        self.raiseADebug(self,'-'*2+' Reading the block: {0:15}'.format(str(child.tag))+2*'-')
        Class = child.tag
        ...
        if Class != 'RunInfo':
          for childChild in child:
            ...
            if 'name' in childChild.attrib.keys():
              ...
              self.raiseADebug(self,'Reading type '+str(childChild.tag)+' with name '+name)
...
#              if name not in self.whichDict[Class].keys():  self.whichDict[Class][name] = self.addWhatDict[Class].returnInstance(childChild.tag,self)
              if Class != 'OutStreamManager':
                  if name not in self.whichDict[Class].keys():  self.whichDict[Class][name] = self.addWhatDict[Class].returnInstance(childChild.tag,self)
                  else: self.raiseAnError(IOError,self,'Redundant naming in the input for class '+Class+' and name '+name)
              else:
    ...
    if not set(self.stepSequenceList).issubset(set(self.stepsDict.keys())):
      self.raiseAnError(IOError,self,'The step list: '+str(self.stepSequenceList)+' contains steps that have no bee declared: '+str(list(self.stepsDict.keys())))
  ...
  def checkStep(self,stepInstance,stepName):
    ...
    for [role,myClass,objectType,name] in stepInstance.parList:
      if myClass!= 'Step' and myClass not in list(self.whichDict.keys()):
        self.raiseAnError(IOError,self,'For step named '+stepName+' the role '+role+' has been assigned to an unknown class type '+myClass)
      if myClass != 'OutStreamManager':
          if name not in list(self.whichDict[myClass].keys()):
            self.raiseAMessage(self,'name: '+name,'DEBUG')
            self.raiseAMessage(self,'list: '+str(list(self.whichDict[myClass].keys())),'DEBUG')
            self.raiseAMessage(self,str(self.whichDict[myClass]),'DEBUG')
            self.raiseAnError(IOError,self,'In step '+stepName+' the class '+myClass+' named '+name+' supposed to be used for the role '+role+' has not been found')
      else:
          if name not in list(self.whichDict[myClass][objectType].keys()):
            self.raiseAMessage(self,'name: '+name,'DEBUG')
            self.raiseAMessage(self,'list: '+str(list(self.whichDict[myClass][objectType].keys())),'DEBUG')
            self.raiseAMessage(self,str(self.whichDict[myClass][objectType]),'DEBUG')
            self.raiseAnError(IOError,self,'In step '+stepName+' the class '+myClass+' named '+name+' supposed to be used for the role '+role+' has not been found')
  ...
  def __readRunInfo(self,xmlNode,runInfoSkip,xmlFilename):
    ...
    for element in xmlNode:
      if element.tag in runInfoSkip:
        ...
        if xmlFilename == None:
          self.raiseAnError(IOError,self,'RelativeWorkingDir requested but xmlFilename is None.')
        ...
        if self.runInfoDict['mode'] in self.__modeHandlerDict:
          ...
          self.raiseAnError(IOError,self,"Unknown mode "+self.runInfoDict['mode'])
  ...
  def run(self):
    ...
    #
    self.raiseADebug(self,'entering the run')
    ...
    for stepName in self.stepSequenceList:
      ...
      self.raiseAMessage(self,'-'*2+' Beginning step {0:50}'.format(stepName+' of type: '+stepInstance.type)+2*'-')
        ...
        for stp in stepindict:
          if "whatDoINeed" in dir(stp):
            ...
            for mainClassStr in neededObjects.keys():
              if mainClassStr not in self.whichDict.keys() and mainClassStr != 'internal': self.raiseAnError(IOError,self,'Main Class '+mainClassStr+' needed by '+stp.name + ' unknown!')
              ...
              for obj in neededObjects[mainClassStr]:
                if obj[1] in vars(self):
                  ...
                  if obj[0]:
                    if obj[0] not in self.whichDict[mainClassStr][obj[1]].type: self.raiseAnError(IOError,self,'Type of requested object '+obj[1]+' does not match the actual type!'+ obj[0] + ' != ' + self.whichDict[mainClassStr][obj[1]].type)
                  neededobjs[mainClassStr][obj[1]] = self.whichDict[mainClassStr][obj[1]]
                else: self.raiseAnError(IOError,self,'Requested object '+obj[1]+' is not part of the Main Class '+mainClassStr + '!')
      ...
      for output in stepInputDict['Output']:
        if self.FIXME: self.raiseAMessage(self,'This is for the filter, it needs to go when the filtering strategy is done','FIXME')
        if "finalize" in dir(output):
          output.finalize()
      self.raiseAMessage(self,'-'*2+' End step {0:50} '.format(stepName+' of type: '+stepInstance.type)+2*'-')
