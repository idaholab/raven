...
class IOStep(Step):
  ...
  def _localInitializeStep(self,inDictionary):
    ...
    if self.fromDirectory and len(self.actionType) == 0: self.raiseAnError(IOError,'In Step named ' + self.name + '. "fromDirctory attribute provided but not conversion action is found (remove this atttribute for OutStream actions only"')
