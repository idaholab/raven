...
class Step(utils.metaclass_insert(abc.ABCMeta, BaseType)):
  """
    ...
    self._localInitializeStep(inDictionary) : called after this call the step should be able the accept the call
    self.takeAstep(inDictionary):
  ...
  def __init__(self):
    ...
    # Waiting time before checking if a run is finished
  ...
  def _handleInput(self, paramInput):
    ...
    if 'repeatFailureRuns' in paramInput.parameterValues:
      ...
      # self.raiseAnError(IOError,'In Step named '+self.name+' it was not possible to cast "perturbationFactor"
      # attribute into a float!')
  ...
  def getInitParams(self):
    """
      Exports a dictionary with the information that will stay constant during the existence of the instance of this
      class. Overloaded from BaseType
  ...
  def _localInitializeStep(self, inDictionary):
    """
      ...
      The inDictionary contains the instances for each possible role supported in the step (dictionary keywords) the
      instances of the objects in list if more than one is allowed
      ...
      self.raiseADebug('for the role "+key+" the item of class '+inDictionary['key'].type+' and name
      '+inDictionary['key'].name+' has been initialized')
      @ In, inDictionary, dict, the initialization dictionary
      @ Out, None
    """
    pass
...
class SingleRun(Step):
  ...
  def _localInputAndCheckParam(self, paramInput):
    ...
    """
    self.raiseADebug(
        'the mapping used in the model for checking the compatibility of usage should be more similar to self.parList to avoid the double mapping below',
        'FIXME')
    found = 0
    rolesItem = []
    ...
    if self.type == 'SingleRun':
      if self.parList[modelIndex][2] != 'Code':
        self.raiseAnError(
            IOError,
            '<SingleRun> steps only support running "Code" model types!  Consider using a <MultiRun> step using a "Custom" sampler for other models.'
        )
      if 'Optimizer' in roles or 'Sampler' in roles:
        self.raiseAnError(
            IOError,
            '<SingleRun> steps does not allow the usage of <Sampler> or <Optimizer>!  Consider using a <MultiRun> step.'
        )
      if 'SolutionExport' in roles:
        self.raiseAnError(
            IOError,
            '<SingleRun> steps does not allow the usage of <SolutionExport>!  Consider using a <MultiRun> step with a <Sampler>/<Optimizer> that allows its usage.'
        )
  ...
  def _localTakeAstepRun(self, inDictionary):
    ...
    model.submit(inputs, None, jobHandler, **{
        'SampledVars': {},
        'additionalEdits': {}
    })
    while True:
      ...
      for finishedJob in finishedJobs:
        if finishedJob.getReturnCode() == 0:
          # if the return code is > 0 => means the system code crashed... we do not want to make the statistics poor
          # => we discard this run
...
class MultiRun(SingleRun):
  ...
  def __init__(self):
    ...
    # this is a dictionary that gets sent as key-worded list to the initialization of the sampler
    self._samplerInitDict = {}
    # just an handy counter of the runs already performed
    self.counter = 0
  ...
  def _localInitializeStep(self, inDictionary):
    ...
    for outIndex, output in enumerate(inDictionary['Output']):
      if output.type not in ['OutStreamPlot', 'OutStreamPrint']:
        if 'SolutionExport' in inDictionary.keys(
        ) and output.name == inDictionary['SolutionExport'].name:
  ...
  def _localTakeAstepRun(self, inDictionary):
    ...
    while True:
      ...
      if jobHandler.isFinished() and not sampler.amIreadyToProvideAnInput():
        self.raiseADebug(
            'Finished with %d runs submitted, %d jobs running, and %d completed jobs waiting to be processed.'
            % (jobHandler.numSubmitted(), jobHandler.numRunning(),
...
class RomTrainer(Step):
  ...
  def _localTakeAstepRun(self, inDictionary):
    """
      ...
      @ Out, None
    """
    # Train the ROM... It is not needed to add the trainingSet since it's already been added in the initialization
    # method
...
class IOStep(Step):
  ...
  def _localInitializeStep(self, inDictionary):
    ...
    if self.fromDirectory and len(self.actionType) == 0:
      self.raiseAnError(
          IOError, 'In Step named ' + self.name +
          '. "fromDirectory" attribute provided but not conversion action is found (remove this atttribute for OutStream actions only"'
      )
    ...
    for i in range(len(outputs)):
      ...
      if 'HDF5' in inDictionary['Output'][i].type:
        if outputs[i].name not in databases:
          ...
          self.raiseADebug(
              'for the role Output the item of class {0:15} and name {1:15} has been initialized'.
              format(outputs[i].type, outputs[i].name))
