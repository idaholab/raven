  ...
  def _localInputAndChecks(self,xmlNode):
    ...
    for  myInput in self.parList:
      if myInput[0] in rolesItem:
        toBeTested[ myInput[0]].append({'class'
...
class RomTrainer(Step):
  ...
  def _localInputAndChecks(self,xmlNode):
    ...
    if [item[0] for item in self.parList].count('Input')!=1:
      self.raiseAnError(IOError,'Only one Input and only one is allowed for a training step. Step name
    if [item[0] for item in self.parList].count('Output')<1:
      self.raiseAnError(IOError,'At least one Output is need in a training step. Step name
