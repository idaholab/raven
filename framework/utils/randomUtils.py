...
if stochasticEnv == 'numpy':
  ...
  distStochEnv = findCrowModule(
      'distribution1D').DistributionContainer.instance()
...
def randomSeed(value):
  ...
  if stochasticEnv == 'numpy':
    ...
    # we must do the following now since there is no separation between the distribution (stoch eviroment) and the
    # one here
...
def random(dim=1, samples=1, keepMatrix=False):
  """
    ...
    @ Out, vals, float, random normal number (or np.array with size [n] if n>1, or np.array with size [n,samples] if
           sampels>1)
  """
  dim = int(dim)
  samples = int(samples)
...
def randomNormal(dim=1, samples=1, keepMatrix=False):
  """
    Function to get a single random value, an array of random values, or a matrix of random values, normally
    distributed
    @ In, dim, int, optional, dimensionality of samples
    @ In, samples, int, optional, number of arrays to deliver
    @ In, keepMatrix, bool, optional, if True then will always return np.array(np.array(float))
...
def randPointsOnHypersphere(dim, samples=1, r=1, keepMatrix=False):
  """
    ...
    "On decompositional algorithms for uniform sampling from n-spheres and n-balls", Harman and Lacko, 2010, J.
...
def _reduceRedundantListing(data, dim, samples):
  ...
  """
  # user expects single float
  if dim == 1 and samples == 1:
    return data[0][0]
  # user expects array of floats
  elif samples == 1:
    return data[0]
