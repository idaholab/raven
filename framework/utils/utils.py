...
def identifyIfExternalModelExists(caller, moduleIn, workingDir):
  """
    ...
    @ Out, (moduleToLoad, fileName), tuple, a tuple containing the module to load (that should be used in method
           importFromPath) and the filename (no path)
  """
...
def stringsThatMeanSilent():
  """
    Return list of strings that indicate a verbosity of the lowest level (just errors). You linguists add what you
    wish
    @ In, None
...
def stringsThatMeanPartiallyVerbose():
  """
    Return list of strings that indicate a verbosity of the medium level (errors and warnings). You linguists add
    what you wish.
    @ In, None
...
def getRelativeSortedListEntry(sortedList, value, tol=1e-15):
  ...
  """
  # necessary to prevent errors at module load
