  ...
  if os.path.exists(workingDirModule+".py"):
    ...
    if (path != ''):
      ...
      if os.path.exists(abspath):
        ...
        caller.raiseAnError(IOError,'The path provided for the' + caller.type + ' named '+ caller.name +' does not exist!!! Got
  ...
  for key, value in dict(inspect.getmembers(obj)).items():
    ...
    if key != value.__name__:
      if value.__name__.split(".")[-1] != key:
        ...
        mods.append(str('from ' + '.'.join(value.__name__.split(".")[
    else:
      mods.append(str(key))
  return mods
...
def convertMultipleToBytes(sizeString):
  ...
  if   'mb' in sizeString:
    ...
    try:
      ...
      raise IOError(UreturnPrintTag('UTILITIES')+'
