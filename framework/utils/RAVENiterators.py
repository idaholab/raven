...
class ravenArrayIterator(object):
  ...
  def __init__(self, shape=None, arrayIn=None):
    """
     Init method.
      @ In, shape, tuple or list, (required if 'arrayIn' is not provided), the shape of the N-D array for which an
             iterator needs to be created
      @ In, arrayIn, ndarray or cachedarray, (required if 'shape' is not provided) the array for which the iterator
             needs to be created
     @ Out, None
    """
    ...
    # counter used for the iterations
    self.cnt = 0
    # has the iterator hitted the end?
    self.finished = False
    # this is the internal iterator object.
    self.iterator = None
    # if the variable 'shape' is passed in, the
    # iterator is going to be created internally (no
    # cartesian product needed)
    # if the variable 'arrayIn' is passed in, the
    # iterator is going to be associated to the
    # numpy.nditer (it requires the
    # cartesian product, since the arrayIn needs to
    # represent a full grid)
