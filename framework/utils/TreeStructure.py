  ...
  if dType is None:
    ...
    if extension == 'xml':
      ...
      raise InputParsingError('Unrecognized file type for:', inFile,
                              ' | Expected .xml, .i, .in, or .inp')
  if dType.lower() == 'xml':
    #try:
    parser = ET.XMLParser(target=CommentedTreeBuilder())
    # parser is defined below, under XMLCommentParser
    xmltree = ET.parse(inFile, parser=parser)
...
def tostring(node):
  ...
  if isinstance(node, InputNode) or isinstance(node, InputTree):
    ...
    raise NotImplementedError('TreeStructure.tostring received "' + str(node) +
                              '" but was expecting InputNode or InputTree.')
...
def xmlToInputTree(xml):
  ...
  """
  # TODO what if they hand in element instead of tree?
  xmlRoot = xml.getroot()
  ...
  for line in getpot:
    ...
    if line.startswith('[./') and line.endswith(']'):
      ...
      if len(parentNodes) > 0:
        ...
        #this is the root
        root = currentNode
      # FIXME should belong to next child? Hard to say.
      addComment(currentNode)
    #empty line
    elif line == '':
...
class InputNode:
  ...
  def __init__(self, tag='', attrib=None, text='', comment=None):
    ...
    if attrib is None:
      ...
      # structure: {name: {value='',comment=''}}
      self.attrib = attrib

    # node name, in XML known as "tag"
    self.tag = tag
    # node text, same in XML
    self.text = text
    # branches off of this node
    self.children = []
...
class HierarchicalNode(MessageHandler.MessageUser):
  ...
  def __init__(self, messageHandler, name, valuesIn={}, text=''):
    ...
    if type(messageHandler) != MessageHandler.MessageHandler:
      raise (IOError, 'Tried to initialize ' + self.type +
             ' without a message handler!  Was given: ' + str(messageHandler))
    self.values = values
    self.text = text
    self._branches = []
  ...
  def writeNode(self, dumpFileObj):
    """
      This method is used to write the content of the node into a file (it recorsevely prints all the sub-nodes and
      sub-sub-nodes, etc)
    ...
    # it's unclear why this conditional was here. It results in the "branch" tag not closing itself. - talbpaul,
    # 2017-10
...
class DynamicMetadataTree(MetadataTree):
  ...
  def _findPivot(self, root, pivotVal, tol=1e-10):
    ...
    if not found:
      pivotNode = HierarchicalNode(
          self.messageHandler, self.pivotParam, valuesIn={'value': pivotVal})
