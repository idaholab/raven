...
def createInterp(x, y, lowFill, highFill, kind='linear'):
  """
    Creates an interpolation function that uses lowFill and highFill whenever a value is requested that lies outside
    of the range specified by x.
     @ In, x, list or np.array, x values
    ...
    @ Out, interp, function(float) returns float, an interpolation function that takes a single float value and
           return its interpolated value using lowFill or highFill when the input value is outside of the
           interpolation range.
  """
  sv = str(scipy.__version__).split('.')
  if int(sv[0]) > 0 or int(sv[1]) >= 17:
    interp = interpolate.interp1d(
        x, y, kind, bounds_error=False, fill_value=([lowFill], [highFill]))
    return interp
  ...
  def myInterp(value):
    ...
    """
    try:
      # why plus 0.0? Could this be done by casting as a float?
      return interp(value) + 0.0
...
def hyperdiagonal(lengths):
  """
    Obtains the length of a diagonal of a hyperrectangle given the lengths of the sides. Useful for high-dimensional
    distance scaling.
...
def historySnapShoots(valueDict, numberOfTimeStep):
  """
    ...
    @ In, valueDict, dict, the dictionary containing the data to be sliced
           {'varName':[history1Values,history2Values, etc.]}
...
def relativeDiff(f1, f2):
  ...
  if not isinstance(f1, float):
    try:
      ...
      raise RuntimeError(
          'Provided argument to compareFloats could not be cast as a float!  First argument is %s type %s'
          % (str(f1), type(f1)))
  if not isinstance(f2, float):
    try:
      ...
      raise RuntimeError(
          'Provided argument to compareFloats could not be cast as a float!  Second argument is %s type %s'
          % (str(f2), type(f2)))
  ...
  if abs(scale) == np.inf:
    ...
    if diff == np.inf:
      # assumption: inf/inf = 1
      return np.inf
    else:
      # assumption: x/inf = 0 for all finite x
      return 0.0
...
def numBinsDraconis(data):
  """
    Determine Bin size and number of bins determined by Freedman Diaconis rule
...
def diffWithInfinites(a, b):
  ...
  if abs(a) == np.inf or abs(b) == np.inf:
    if a == b:
      # not mathematically rigorous, but useful algorithmically
      res = 0
    elif a > b:
      res = np.inf
    # b > a
...
def computeTruncatedSingularValueDecomposition(X, truncationRank):
  """
    ...
    @ Out, (U, s, V), tuple of numpy.ndarray, (left-singular vectors matrix, singular values, right-singular vectors
           matrix)
  """
...
def computeEigenvaluesAndVectorsFromLowRankOperator(lowOperator,
  ...
  """
    ...
    @ In, exactModes, bool, optional, if True the exact modes get computed otherwise the projected ones are (Default
           = True)
...
def computeAmplitudeCoefficients(mods, Y, eigs, optmized):
  """
    ...
    @ In, optmized, bool, if True the amplitudes are computed minimizing the error between the mods and all entries
           (columns) in Y
    if False the amplitudes are computed minimizing the error between the mods and the 1st entry (columns) in Y
    (faster)
