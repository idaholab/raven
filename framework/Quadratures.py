...
import collections
# .operator.itemgetter as operator.itemgetter
import operator
import inspect
...
class SparseGrid(MessageHandler.MessageUser):
  ...
  def __init__(self):
    ...
    # array of coefficient terms for component tensor grid entries
    self.c = []
    # storage space for re-ordered versions of sparse grid
    self.oldsg = []
    # IndexSet object
    self.indexSet = None
    # dict{varName: Distribution object}
    self.distDict = None
    # dict{varName: Quadrature object}
    self.quadDict = None
    # dict{varName: OrthoPolynomial object}
    self.polyDict = None
    self.varNames = []  # array of names, in order of distDict.keys()
    # dimensionality of input space
    self.N = None
    # dict{ (point,point,point): weight}
    self.SG = None
    # message handler
  ...
  def initialize(self, varNames, indexSet, distDict, quadDict, handler,
    ...
    # keys on points, values on weights
    self.SG = collections.OrderedDict()
  ...
  def __setstate__(self, pdict):
    ...
    # it was missing. Andrea
    self.quadDict = pdict.pop('quadDict')
  ...
  def _remap(self, newNames):
    ...
    for combo in newptwt:
      # weight is last entry
      newSG[tuple(combo[:-1])] = combo[-1]
    self.SG = newSG
...
class TensorGrid(SparseGrid):
  ...
  def initialize(self, varNames, indexSet, distDict, quadDict, handler,
    ...
    # TODO give user access to this +1 rule
    quadSizes = self.quadRule(largest) + 1
    ...
    while True:
      ...
      if j < numRunsNeeded - 1:
        ...
        if handler.isFinished() and len(handler.getFinishedNoPop()) == 0:
          # FIXME this is significantly the second-most expensive line in this method
          break
      import time
      time.sleep(0.005)
    ...
    while True:
      ...
      if i < N - 1:
        ...
        for k in range(min(handler.availability(), N - 1 - i)):
          i += 1
          handler.addJob(
              (N, i, self.indexSet[i], self.indexSet[:]),
              makeSingleCoeff,
              prefix + str(i),
              modulesToImport=self.mods)
      else:
...
class QuadratureSet(MessageHandler.MessageUser):
  ...
  def __init__(self):
    ...
    # tool for generating points and weights for a given order
    self.rule = None
