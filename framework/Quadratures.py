    ...
    while True:
      ...
      if j<numRunsNeeded-1:
        for _ in range(min(numRunsNeeded-1-j,handler.availability())):
          ...
          idx = self.indexSet[j]
          m=self.quadRule(idx)+1
          handler.addJob(None, None, (m,),self.tensorGrid,prefix+str(cof),modulesToImport = self.mods)
      else:
    ...
    while True:
      ...
      if i<N-1:
        ...
        for k in range(min(handler.availability(),N-1-i)):
          i+=1
          handler.addJob(None, None, (N,i,self.indexSet[i],self.indexSet[:]),makeSingleCoeff,prefix+str(i),modulesToImport = self.mods)
      else:
        if handler.isFinished() and len(handler.getFinishedNoPop())==0:
          break
