  ...
  def readMoreXML(self, xmlNode):
    ...
    for child in xmlNode:
      if child.tag == 'pivotParameter':
        ...
        self.settings['operationValue'] = float(
            child.text) if child.tag != 'operator' else child.text
          ...
          if self.settings['operationType'] == 'row':
            ...
            if self.settings['pivotStrategy'] in ['nearest', 'floor', 'ceiling']:
              ...
              if self.settings['pivotStrategy'] == 'floor':
                if np.asarray(outputDic['data'][self.pivotParameter]
                              [hist])[idx] > self.settings['operationValue']:
                  idx -= 1
            ...
            if self.settings['operationValue'] == 'max':
              outputDic['data'][outputVar] = np.append(
                  outputDic['data'][outputVar],
                  copy.deepcopy(np.max(inputDict['data'][outputVar][hist])))
            elif self.settings['operationValue'] == 'min':
              outputDic['data'][outputVar] = np.append(
                  outputDic['data'][outputVar],
                  copy.deepcopy(np.min(inputDict['data'][outputVar][hist])))
            elif self.settings['operationValue'] == 'average':
              outputDic['data'][outputVar] = np.append(
                  outputDic['data'][outputVar],
                  copy.deepcopy(np.mean(inputDict['data'][outputVar][hist])))
      return outputDic
