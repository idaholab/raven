...
class HistorySetSync(PostProcessorInterfaceBase):
  ...
  def run(self,inputDic):
    ...
    if len(inputDic)>1:
      ...
      outputDic["data"] = copy.deepcopy(inputDic["data"])

      newTime = []
      if self.syncMethod == "grid":
        maxTime = inputDic["data"][self.pivotParameter].max
        minTime = inputDic["data"][self.pivotParameter].min
        newTime = np.linspace(minTime,maxTime,self.numberOfSamples)
      elif self.syncMethod == "all":
        times = set()
        for hist in inputDic["data"]["output"][self.pivotParameter]:
        ...
        for h,hist in enumerate(inputDic["data"]["output"].keys()):
          l = len(inputDic["data"]["output"][self.pivotParameter][hist])
          if (h==0) or (self.syncMethod == "max" and l > notableLength) or (self.syncMethod == "min" and l < notableLength):
            notableHist = inputDic["data"][self.pivotParameter][hist][:]
            notableLength = l
  ...
  def resampleHist(self, variables, newTime):
    newVars={}
