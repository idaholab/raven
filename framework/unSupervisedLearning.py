...
class unSupervisedLearning(
  ...
  """
  # this describe the type of information generated the
  returnType = ''
  ...
  def train(self, tdict, metric=None):
    ...
    if type(tdict) != dict:
      self.raiseAnError(
          IOError,
          ' method "train". The training set needs to be provided through a dictionary. Type of the in-object is '
          + str(type(tdict)))
    ...
    if len(unidentifiedFeatures) > 0:
      ...
      if len(unidentifiedFeatures) == 1:
        msg = 'The requested feature: %s does not exist in the training set.' % list(
            unidentifiedFeatures)[0]
      else:
        msg = 'The requested features: %s do not exist in the training set.' % str(
            list(unidentifiedFeatures))
    ...
    if not isinstance(tdict.values()[0], dict):
      for name, val in tdict.iteritems():
        if name in self.features and realizationCount != val.size:
          self.raiseAnError(
              IOError,
              ' In training set, the number of realizations are inconsistent among the requested features.'
          )
  ...
  def evaluate(self, edict):
    ...
    if len(unidentifiedFeatures) > 0:
      ## Me write English good!
      if len(unidentifiedFeatures) == 1:
        msg = 'The requested feature: %s does not exist in the evaluate set.' % list(
            unidentifiedFeatures)[0]
      else:
        msg = 'The requested features: %s do not exist in the evaluate set.' % str(
            list(unidentifiedFeatures))
      self.raiseAnError(IOError, msg)
...
class SciKitLearn(unSupervisedLearning):
  ...
  availImpl['cluster']['AffinityPropogation'] = (cluster.AffinityPropagation,
                                                 'float')
  # Perform DBSCAN clustering from vector array or distance matrix.
  availImpl['cluster']['DBSCAN'] = (cluster.DBSCAN, 'float')
  # K-Means Clustering
  availImpl['cluster']['KMeans'] = (cluster.KMeans, 'float')
  # Mini-Batch K-Means Clustering
  availImpl['cluster']['MiniBatchKMeans'] = (cluster.MiniBatchKMeans, 'float')
  # Mean Shift Clustering
  availImpl['cluster']['MeanShift'] = (cluster.MeanShift, 'float')
  # Apply clustering to a projection to the normalized laplacian.
  availImpl['cluster']['SpectralClustering'] = (cluster.SpectralClustering,
                                                'float')
  # Agglomerative Clustering - Feature of SciKit-Learn version 0.15
  availImpl['cluster']['Agglomerative'] = (cluster.AgglomerativeClustering,
                                           'float')
  # availImpl['cluster']['FeatureAgglomeration'   ] = (cluster.FeatureAgglomeration   , 'float')
  # - Feature of SciKit-Learn version 0.15
  # availImpl['cluster']['Ward'                   ] = (cluster.Ward                   , 'float')
  # Ward hierarchical clustering: constructs a tree and cuts it.

  #  availImpl['bicluster'] = {}
  # availImpl['bicluster']['SpectralBiclustering'] = (cluster.bicluster.SpectralBiclustering, 'float')
  # Spectral biclustering (Kluger, 2003).
  # availImpl['bicluster']['SpectralCoclustering'] = (cluster.bicluster.SpectralCoclustering, 'float')
  # Spectral Co-Clustering algorithm (Dhillon, 2001).

  availImpl['mixture'] = {
  }  # Generalized Gaussion Mixture Models (Classification)
  # Gaussian Mixture Model
  availImpl['mixture']['GMM'] = (mixture.GMM, 'float')
  ...
  ## their documentation.
  # availImpl['mixture']['DPGMM'] = (mixture.DPGMM, 'float')
  # Variational Inference for the Infinite Gaussian Mixture Model.
  # Variational Inference for the Gaussian Mixture Model
  availImpl['mixture']['VBGMM'] = (mixture.VBGMM, 'float')

  availImpl['manifold'] = {}  # Manifold Learning (Embedding techniques)
  # Locally Linear Embedding
  availImpl['manifold']['LocallyLinearEmbedding'] = (
      manifold.LocallyLinearEmbedding, 'float')
  # Isomap
  availImpl['manifold']['Isomap'] = (manifold.Isomap, 'float')
  # MultiDimensional Scaling
  availImpl['manifold']['MDS'] = (manifold.MDS, 'float')
  # Spectral Embedding for Non-linear Dimensionality Reduction
  availImpl['manifold']['SpectralEmbedding'] = (manifold.SpectralEmbedding,
                                                'float')
  # availImpl['manifold']['locally_linear_embedding'] = (manifold.locally_linear_embedding, 'float')
  # Perform a Locally Linear Embedding analysis on the data.
  # availImpl['manifold']['spectral_embedding'      ] = (manifold.spectral_embedding      , 'float')
  # Project the sample on the first eigen vectors of the graph Laplacian.

  # Matrix Decomposition
  availImpl['decomposition'] = {}
  availImpl['decomposition']['PCA'] = (
      decomposition.PCA, 'float')  # Principal component analysis (PCA)
  # availImpl['decomposition']['ProbabilisticPCA'    ] = (decomposition.ProbabilisticPCA    , 'float')
  # Additional layer on top of PCA that adds a probabilistic evaluationPrincipal component analysis (PCA)
  # Principal component analysis (PCA) using randomized SVD
  availImpl['decomposition']['RandomizedPCA'] = (decomposition.RandomizedPCA,
                                                 'float')
  availImpl['decomposition']['KernelPCA'] = (
      decomposition.KernelPCA,
      'float')  # Kernel Principal component analysis (KPCA)
  # FastICA: a fast algorithm for Independent Component Analysis.
  availImpl['decomposition']['FastICA'] = (decomposition.FastICA, 'float')
  # Dimensionality reduction using truncated SVD (aka LSA).
  availImpl['decomposition']['TruncatedSVD'] = (decomposition.TruncatedSVD,
                                                'float')
  ...
  availImpl['decomposition']['MiniBatchSparsePCA'] = (
      decomposition.MiniBatchSparsePCA, 'float')

  # availImpl['decomposition']['ProjectedGradientNMF'] = (decomposition.ProjectedGradientNMF, 'float')
  # Non-Negative matrix factorization by Projected Gradient (NMF)
  # availImpl['decomposition']['FactorAnalysis'      ] = (decomposition.FactorAnalysis      , 'float')
  # Factor Analysis (FA)
  # availImpl['decomposition']['NMF'                 ] = (decomposition.NMF                 , 'float')
  # Non-Negative matrix factorization by Projected Gradient (NMF)
  # availImpl['decomposition']['SparseCoder'         ] = (decomposition.SparseCoder         , 'float')
  # Sparse coding
  # availImpl['decomposition']['DictionaryLearning'  ] = (decomposition.DictionaryLearning  , 'float')
  # Dictionary Learning
  # availImpl['decomposition']['MiniBatchDictionaryLearning'] = (decomposition.MiniBatchDictionaryLearning, 'float')
  # Mini-batch dictionary learning
  # availImpl['decomposition']['fastica'                    ] = (decomposition.fastica                    , 'float')
  # Perform Fast Independent Component Analysis.
  # availImpl['decomposition']['dict_learning'              ] = (decomposition.dict_learning              , 'float')
  # Solves a dictionary learning matrix factorization problem.

  #  availImpl['covariance'] = {}  # Covariance Estimators
  # availImpl['covariance']['EmpiricalCovariance'] = (covariance.EmpiricalCovariance, 'float')
  # Maximum likelihood covariance estimator
  # availImpl['covariance']['EllipticEnvelope'   ] = (covariance.EllipticEnvelope   , 'float')
  # An object for detecting outliers in a Gaussian distributed dataset.
  # availImpl['covariance']['GraphLasso'         ] = (covariance.GraphLasso         , 'float')
  # Sparse inverse covariance estimation with an l1-penalized estimator.
  # availImpl['covariance']['GraphLassoCV'       ] = (covariance.GraphLassoCV       , 'float')
  # Sparse inverse covariance w/ cross-validated choice of the l1 penalty
  # availImpl['covariance']['LedoitWolf'         ] = (covariance.LedoitWolf         , 'float')
  # LedoitWolf Estimator
  # availImpl['covariance']['MinCovDet'          ] = (covariance.MinCovDet          , 'float')
  # Minimum Covariance Determinant (MCD): robust estimator of covariance
  # availImpl['covariance']['OAS'                ] = (covariance.OAS                , 'float')
  # Oracle Approximating Shrinkage Estimator
  # availImpl['covariance']['ShrunkCovariance'   ] = (covariance.ShrunkCovariance   , 'float')
  # Covariance estimator with shrinkage

  #  availImpl['neuralNetwork'] = {}  # Covariance Estimators
  # availImpl['neuralNetwork']['BernoulliRBM'] = (neural_network.BernoulliRBM, 'float')
  ...
  def __init__(self, messageHandler, **kwargs):
    ...
    if 'SKLtype' not in self.initOptionDict.keys():
      self.raiseAnError(
          IOError,
          ' to define a scikit learn unSupervisedLearning Method the SKLtype keyword is needed (from KDD '
          + self.name + ')')
  ...
  def __trainLocal__(self):
    ...
    if hasattr(self.Method, 'connectivity'):
      ...
      connectivity = kneighbors_graph(
          self.normValues, n_neighbors=min(10,
                                           len(self.normValues[:, 0]) - 1))
  ...
  def __confidenceLocal__(self):
    ...
    if 'cluster' == self.SKLtype and 'labels' in self.outputDict['outputs']:
      ...
      self.outputDict['confidence']['aic'] = self.Method.aic(self.normValues)
      # Bayesian Information Criterion
      self.outputDict['confidence']['bic'] = self.Method.bic(self.normValues)
      # log probabilities of each data point
      self.outputDict['confidence']['score'] = self.Method.score(
          self.normValues)
...
class temporalSciKitLearn(unSupervisedLearning):
  ...
  def checkArrayConsistency(arrayin, shape):
    """
      ...
      @ Out, tuple, tuple[0] is a bool (True -> everything is ok, False -> something wrong), tuple[1], string ,the
             error mesg
    """
    if type(arrayin) != np.ndarray:
      return (False, ' The object is not a numpy array')
  ...
  def __trainLocal__(self):
    ...
    for t in range(self.numberOfHistoryStep):
      ...
      if self.SKLtype in ['cluster']:
        ...
        # else:
        # self.metaDict['clusterCentersIndices'][t] = range(noClusters)
        # use list(set(self.SKLEngine.Method.labels_)) to collect outliers
        self.metaDict['clusterCentersIndices'][t] = range(noClusters)
        ...
        # re-order clusters
        if t > 0:
          remap = self.__reMapCluster__(t, self.metaDict['clusterCenters'],
                                        self.metaDict['clusterCentersIndices'])
          for n in range(len(self.metaDict['clusterCentersIndices'][t])):
            self.metaDict['clusterCentersIndices'][t][n] = remap[self.metaDict[
        ...
        # re-order components
        if t > 0:
          remap = self.__reMapCluster__(t, self.metaDict['means'],
                                        self.metaDict['componentMeanIndices'])
          for n in range(len(self.metaDict['componentMeanIndices'][t])):
            self.metaDict['componentMeanIndices'][t][n] = remap[self.metaDict[
                'componentMeanIndices'][t][n]]
  ...
  def __localReMap__(self, dMatrix, loc):
    """
      Method to return the mapping based on distance stored in dMatrix, the returned mapping shall minimize the
      global sum of distance
      ...
      @ In, loc, tuple, the first element is the cluster indeces for previous time step and the second one is for
...
class Scipy(unSupervisedLearning):
  ...
  availImpl['cluster']['Hierarchical'] = (hier.hierarchy, 'float')
  ...
  def __init__(self, messageHandler, **kwargs):
    ...
    if 'SCIPYtype' not in self.initOptionDict.keys():
      self.raiseAnError(
          IOError,
          ' to define a Scipy unSupervisedLearning Method the SCIPYtype keyword is needed (from KDD '
          + self.name + ')')
...
def returnClass(modelClass, caller):
  ...
  try:
    ...
    caller.raiseanError(NameError, 'unSupervisedLearning',
                        'not known ' + __base + ' type ' + modelClass)
