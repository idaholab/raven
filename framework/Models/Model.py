      ...
      if child.tag =='alias':
        ...
        if 'variable' in child.attrib.keys():
          ...
          if varFramework in self.alias[aliasType].keys():
            self.raiseAnError(IOError, f'The alias for variable "{varFramework}" already exists in {self.type} model "{self.name}"!')
          if varModel in self.alias[aliasType].values():
            self.raiseAnError(IOError, f'The alias "{varModel}" has been already used for another variable in {self.type} model "{self.name}"!')
          if varFramework in self.alias[complementAliasType]:
            self.raiseAnError(IOError, f'The alias for variable "{varFramework}" has already been input as "{complementAliasType}" for {self.type} model "{self.name}"!')
          for key, val in self.alias[complementAliasType].items():
            if val == varModel:
              usedVar = key
              self.raiseAnError(IOError, f'The alias "{varModel}" has already been used as "{complementAliasType}" for variable "{usedVar}" for {self.type} model "{self.name}"!')
