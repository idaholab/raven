...
class Model(utils.metaclass_insert(abc.ABCMeta, BaseType), Assembler):
  ...
  def specializeValidateDict(cls):
    """
      This method should be overridden to describe the types of input accepted with a certain role by the model
  ...
  def localValidateMethod(cls, who, what):
    """
      ...
      @ In, who, string, a string identifying the what is the role of what we are going to test (i.e. input, output
             etc)
  ...
  def __init__(self, runInfoDict):
    ...
    # if alias are defined in the input it defines a mapping between the variable names in the framework and the one
    # for the generation of the input
    # self.alias[framework variable name] = [input code name]. For Example, for a MooseBasedApp, the alias would be
    # self.alias['internal_variable_name'] = 'Material|Fuel|thermal_conductivity'
  ...
  def _replaceVariablesNamesWithAliasSystem(self,
    ...
    """
      ...
      @ In, fromModelToFramework, bool, optional, When we define aliases for some input variables, we need to be
             sure to convert the variable names
  ...
  def addOutputFromExportDictionary(self, exportDict, output, options,
    ...
    """
      ...
      @ In, exportDict, dict, dictionary containing the output/input values: {'inputSpaceParams':dict(sampled
             variables),
                                                                              'outputSpaceParams':dict(output variables),
                                                                              'metadata':dict(metadata)}
      @ In, output, "DataObjects" object, output where the results of the calculation needs to be stored
      @ In, options, dict, dictionary of options that can be passed in when the collect of the output is performed
             by another model (e.g. EnsembleModel)
