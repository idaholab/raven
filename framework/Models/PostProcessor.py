...
class PostProcessor(Model):
  ...
  def provideExpectedMetaKeys(self):
    ...
    try:
      ...
      # either "interface" has no method for returning meta keys, or "interface" is not established yet.
      pass
    return keys
  ...
  def initialize(self, runInfo, inputs, initDict=None):
    ...
    self.workingDir = os.path.join(runInfo['WorkingDir'], runInfo['stepName'])
    self.interface.initialize(runInfo, inputs, initDict)
    self.mods = self.mods + list(
        set(
            utils.returnImportModuleString(
                inspect.getmodule(PostProcessors), True)) - set(self.mods))
