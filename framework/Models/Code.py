...
# note: "from CsvLoader import CsvLoader" currently breaks internalParallel with Files and genericCodeInterface -
# talbpaul 2017-08-24
import CsvLoader
import Files
...
class Code(Model):
  ...
  def getInputSpecification(cls):
    ...
    # Code interfaces can allow new elements.
    inputSpecification.setStrictMode(False)
  ...
  def specializeValidateDict(cls):
    """
      ...
      @ Out, None
    """
    # FIXME think about how to import the roles to allowed class for the codes. For the moment they are not
    # specialized by executable
  ...
  def __init__(self, runInfoDict):
    ...
    # name of the pre-executable, if any
    self.preExec = None
    self.oriInputFiles = []  #list of the original input files (abs path)
    # location where the code is currently running
    self.workingDir = ''
    # root to be used to generate the sequence of output files
    self.outFileRoot = ''
  ...
  def _readMoreXML(self, xmlNode):
    ...
    # output:''
    self.clargs = {'text': '', 'input': {'noarg': []}, 'pre': '', 'post': ''}
      ...
      if child.getName() == 'preexec':
        ...
        if argtype == None:
          ...
          if arg == None:
            self.raiseAnError(
                IOError,
                'filearg type "input" requires the template variable be specified in "arg" attribute!'
            )
          if ext == None:
            self.raiseAnError(
                IOError,
                'filearg type "input" requires the auxiliary file extension be specified in "ext" attribute!'
            )
          ...
          if self.fargs['output'] != '':
            self.raiseAnError(
                IOError,
                'output fileargs already specified!  You can only specify one output fileargs node.'
            )
          if arg == None:
            self.raiseAnError(
                IOError,
                'filearg type "output" requires the template variable be specified in "arg" attribute!'
            )
          ...
          if self.fargs['moosevpp'] != '':
            self.raiseAnError(
                IOError,
                'moosevpp fileargs already specified!  You can only specify one moosevpp fileargs node.'
            )
          if arg == None:
            self.raiseAnError(
                IOError,
                'filearg type "moosevpp" requires the template variable be specified in "arg" attribute!'
            )
    ...
    # TODO figure out how to handle this with InputData
  ...
  def initialize(self, runInfoDict, inputFiles, initDict=None):
    """
      this needs to be over written if a re initialization of the model is need it gets called at every beginning of
      a step
  ...
  def _expandForWindows(self, origCommand):
    """
      # ! to a windows runnable command
      Function to expand a command that has a
  ...
  def evaluateSample(self, myInput, samplerType, kwargs):
    ...
    # self.raiseAnError(IOError, "the first entry in tuple returned by generateCommand method needs to be a list of
    # tuples!")
    ...
    process = utils.pickleSafeSubprocessPopen(
        command,
        shell=True,
        stdout=outFileObject,
        stderr=outFileObject,
        cwd=localenv['PWD'],
        env=localenv)
    process.wait()
    ...
    if 'finalizeCodeOutput' in dir(self.code) and returnCode == 0:
      ...
      if ravenCase and self.code.__class__.__name__ != 'RAVEN':
        self.raiseAnError(
            RuntimeError,
            'The return argument from "finalizeCodeOutput" must be a str containing the new output file root!'
        )
    ...
    if returnCode == 0:
      ...
      if not ravenCase:
        ...
        # we have two tasks to do: collect the input/output/meta/indexes from the INNER raven run, and ALSO the
        # input from the OUTER raven run.
        ...
        for n in range(numRlz):
          ...
          if numRlz > 1:
            if '_' in kwargs['prefix']:
              self.raiseAnError(
                  RuntimeError,
                  'OUTER RAVEN is using an OPTIMIZER, but INNER RAVEN is returning multiple realizations!'
              )
            addon = '_{}'.format(n)
          else:
            addon = ''
  ...
  def createExportDictionary(self, evaluation):
    """
      Method that is aimed to create a dictionary with the sampled and output variables that can be collected by the
      different
      output objects.
    ...
    for key, value in sampledVars.items():
      ...
      if key in outputEval.keys():
        self.raiseAWarning(
            'The model ' + self.type +
            ' reported a different value (%f) for %s than raven\'s suggested sample (%f). Using the value reported by the raven (%f).'
            % (outputEval[key][0], key, value, value))
      outputEval[key] = np.atleast_1d(value)
  ...
  def collectOutput(self, finishedJob, output, options=None):
    """
      ...
      @ In, options, dict, optional, dictionary of options that can be passed in when the collect of the output is
             performed by another model (e.g. EnsembleModel)
      @ Out, None
    """
  ...
  def collectOutputFromDataObject(self, exportDict, output):
    """
      Method to collect the output from a DataObject (if it is not a dataObject, it just returns a list with one
      single exportDict)
