...
class ExternalModel(Dummy):
  ...
  def getInputSpecification(cls):
    ...
    # External models can allow new elements
  ...
  def specializeValidateDict(cls):
    ...
    # cls.raiseADebug('think about how to import the roles to allowed class for the external model. For the moment
    # we have just all')
    pass
  ...
  def _externalRun(self, Input, modelVariables):
    ...
    for key in self.modelVariableType.keys():
      ...
      # if variable name is too strange to be a member of "self", then skip it
        ...
        self.raiseAWarning(
            'Variable "{}" could not be added to "self" due to complex name.  Find it in "Inputs" dictionary instead.'.
            format(key))
    #else:
    #  InputDict = Input
    ...
    for key in self.modelVariableType.keys():
      try:
        ...
        self.raiseAWarning(
            'Variable "{}" cannot be read from "self" due to complex name.  Retaining original value.'.
            format(key))
    ...
    # check type consistency... This is needed in order to keep under control the external model... In order to
    # avoid problems in collecting the outputs in our internal structures
    for key in self.modelVariableType.keys():
  ...
  def evaluateSample(self, myInput, samplerType, kwargs):
    ...
    # entry [1] is the external model object; it doesn't appear to be needed
    result, instSelf = self._externalRun(
        inRun,
        Input[1],
    )
    # build realization
