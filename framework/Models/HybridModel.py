...
class HybridModel(Dummy):
  ...
  def __init__(self, runInfoDict):
    ...
    # Instance of given model
    self.modelInstance = None
    # Instance of provided cross validation
    self.cvInstance = None
    # Instance of data object used to store the inputs and outputs of HybridModel
    self.targetEvaluationInstance = None
    # Instance of data object that are used to store the training set
    self.tempTargetEvaluation = None
    # dictionary of models that is going to be employed, i.e. {'romName':Instance}
    self.romsDictionary = {}
    # the initial size of training set
    self.romTrainStartSize = 10
    # the maximum size of training set
    self.romTrainMaxSize = 1.0e6
    # the size of rom validation set
    self.romValidateSize = 10
    # True if all roms are trained
    self.romTrained = False
    # waiting time before checking if a run is finished.
    self.sleepTime = 0.005
    # True if all roms are converged
    self.romConverged = False
    # True if all roms are valid for given input data
    self.romValid = False
    # The criterion used to check ROM convergence
    self.romConvergence = 0.01
    # dict used to store the validation methods and their settings
    self.validationMethod = {}
    # The size of existing training set in the provided data object via 'TargetEvaluation'
    self.existTrainSize = 0
    # print tag
    self.printTag = 'HYBRIDMODEL MODEL'
    # If the type of model is 'Code', this will set to true
    self.createWorkingDir = False
    # Indicators used to collect model inputs/outputs for rom training
    self.tempOutputs = {}
    # The size of training set that is previous used to train the rom
    self.oldTrainingSize = 0
    # a dict i.e. {jobPrefix: 1 or 0} used to indicate the runs: model or rom. '1' indicates ROM run, and '0'
    # indicates Code run
    self.modelIndicator = {}
  ...
  def initialize(self, runInfo, inputs, initDict=None):
    ...
    if len(self.targetEvaluationInstance):
      self.raiseAWarning(
          "The provided TargetEvaluation data object is not empty, the existing data will also be used to train the ROMs!"
      )
    ...
    for romInfo in self.romsDictionary.values():
      ...
      if romIn.amITrained:
        self.raiseAWarning("The provided rom ", romIn.name,
                           " is already trained, we will reset it!")
        romIn.reset()
  ...
  def trainRom(self, samplerType, kwargs):
    ...
    for romInfo in self.romsDictionary.values():
      ...
      if useCV:
        # always train the rom even if the rom is converged, we assume the cross validation and rom train are
        # relative cheap
        ...
        self.raiseAMessage(
            "Minimum initial training size is met, but the training size is not enough to be used to perform cross validation"
        )
