...
class PhysicsGuidedCoverageMapping(ValidationBase):
  ...
  def _evaluate(self, datasets, **kwargs):
    ...
    for feat, targ in zip(self.features, self.targets):
      ...
      meanPred = 0.0
      for i in range(len(Y[0, :])):
        meanPred += Y[0, i]*yAppPredPdfNorm[i]*(Y[0, 1]-Y[0, 0])

      # Calculate Variance of predicted application
      predVar = 0.0
      for i in range(len(Y[0, :])):
        predVar += (Y[0, i]-meanPred)**2.0 * yAppPredPdfNorm[i]*(Y[0, 1]-Y[0, 0])

      # Standard Deviation
      predStd = np.sqrt(predVar)
      priStd = np.std(yAppStd)
      reductStd = 1.0-predStd/priStd
      ...
      yAppPred = predDist.rvs(size=len(yAppStd))
