...
class Dummy(Model):
  ...
  def __init__(self, runInfoDict):
    ...
    # the list of admitted data is saved also here for run time checks
    self.admittedData = self.__class__.validateDict['Input'][0]['type']
  ...
  def _manipulateInput(self, dataIn):
    ...
    if type(dataIn[0]) != tuple:
      # this might happen when a single run is used and the input it does not come from self.createNewInput
      inRun = self._inputToInternal(dataIn[0])
    else:
      inRun = dataIn[0][0]
    return inRun
  ...
  def _inputToInternal(self, dataIN):
    """
      Transform it in the internal format the provided input. dataIN could be either a dictionary (then nothing to
      do) or one of the admitted data
      @ In, dataIn, object, the object that needs to be manipulated
    ...
    if type(dataIN) != dict:
      ...
      # Now if an OutputPlaceHolder is used it is removed, this happens when the input data is not representing is
      ...
      # here we do not make a copy since we assume that the dictionary is for just for the model usage and any
      # changes are not impacting outside
      localInput = dataIN
    return localInput
  ...
  def createNewInput(self, myInput, samplerType, **kwargs):
    ...
    if len(missing) != 0:
      self.raiseAnError(
          IOError,
          'Input values for variables {} not found while preparing the input for model "{}"!'.
          format(missing, self.name))
    # the inputs/outputs should not be store locally since they might be used as a part of a list of input for the
    # parallel runs
    # same reason why it should not be used the value of the counter inside the class but the one returned from
    # outside as a part of the input
