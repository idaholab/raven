  ...
  def createNewInput(self,myInput,samplerType,**Kwargs):
    ...
    if 'createNewInput' in dir(self.sim):
      ...
      if extCreateNewInput== None:
        self.raiseAnError(AttributeError,'in external Model '+self.ModuleToLoad+' the method createNewInput must return something. Got
      if 'SampledVars' in Kwargs.keys() and len(self.alias['input'].keys()) != 0:
        Kwargs['SampledVars'] = sampledVars
    ...
    if None in self.modelVariableType.values():
      ...
      for key in self.modelVariableType.keys():
        ...
        if self.modelVariableType[key] not in self._availableVariableTypes:
          if not errorFound:
            self.raiseADebug('Unsupported type found. Available ones are
          errorFound = True
  ...
  def collectOutput(self,finishedJob,output):
    ...
    for modelIn in self.modelsDictionary.keys():
      ...
      for key in targetEvaluations[modelIn].getParaKeys('inputs'):
        if key not in inputsValues.keys():
          self.raiseAnError(Exception,"the variable "+key+" is not in the input space of the model! Vars are
      ...
      for key in targetEvaluations[modelIn].getParaKeys('outputs'):
        if key not in outputsValues.keys():
          self.raiseAnError(Exception,"the variable "+key+" is not in the output space of the model! Vars are
      ...
      if output.name == self.modelsDictionary[modelIn]['TargetEvaluation'].name:
        self.raiseAnError(RuntimeError, "The Step output can not be one of the target evaluation outputs!")
    if output.type == 'HDF5':
      output.addGroupDataObjects({'group'
    else:
      for key in exportDict['inputSpaceParams' ] :
        if key in output.getParaKeys('inputs'):
