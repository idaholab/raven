...
class LimitSurface(PostProcessor):
  ...
  def __init__(self, messageHandler):
    ...
    # The grid has been passed from outside (self._initFromDict)?
    self.gridFromOutside = False
  ...
  def _initializeLSpp(self, runInfo, inputs, initDict):
    ...
    # self.raiseAnError(IOError, 'LimitSurface PostProcessor: The param ' + param + ' not contained in Data ' +
    # self.inputs[self.indexes].name + ' !')
    #    if param in inpKeys:
    ...
    if self.bounds == None:
      ...
      for key in self.parameters['targets']:
        ...
        # self.bounds["lowerBounds"][key], self.bounds["upperBounds"][key] =
        # min(self.inputs[self.indexes].getParam(self.paramType[key],key,nodeId = 'RecontructEnding')),
        # max(self.inputs[self.indexes].getParam(self.paramType[key],key,nodeId = 'RecontructEnding'))
        if utils.compare(
    ...
    self.nVar = len(self.parameters['targets'])
    # this list is the implicit mapping of the name of the variable with the grid axis ordering self.axisName[i] =
    # name i-th coordinate
    self.axisName = self.gridEntity.returnParameter("dimensionNames",
                                                    self.name)
    # grid where the values of the goalfunction are stored
    self.testMatrix[self.name] = np.zeros(
        self.gridEntity.returnParameter("gridShape", self.name))
  ...
  def getTestMatrix(self, nodeName=None, exceptionGrid=None):
    """
      ...
      @ In, nodeName, string, optional, which grid node should be returned. If None, the self.name one, If "all",
             all of theme, else the nodeName
  ...
  def run(self,
    ...
    for nodeName in allGridNames:
      ...
      # get the prediction on the testing grid
      self.testMatrix[nodeName][:] = self.ROM.evaluate(tempDict)[
          self.externalFunction.name]
      # bring back the grid structure
      ...
      if self.lsSide in ["negative", "both"]:
        # it returns the list of points belonging to the limit state surface and resulting in a negative response by
        # the ROM
      ...
      if self.lsSide in ["positive", "both"]:
        # it returns the list of points belonging to the limit state surface and resulting in a positive response by
        # the ROM
