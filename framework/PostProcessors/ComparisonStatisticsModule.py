...
def _getPDFandCDFfromData(dataName, data, csv, methodInfo, interpolation,
  ...
  """
    ...
    @ Out, (dataStats, cdfFunc, pdfFunc), tuple, dataStats is dictionary with things like "mean" and "stdev",
           cdfFunction is a function that returns the CDF value and pdfFunc is a function that returns the PDF
           value.
  """
...
class ComparisonStatistics(PostProcessor):
  ...
  def getInputSpecification(cls):
    ...
    FZInput = InputData.parameterInputFactory(
        "fz", contentType=InputData.StringType)
  ...
  def __init__(self, messageHandler):
    ...
    # Dictionary of all the input data, keyed by the name
    self.dataDict = {}
    # List of each of the groups that will be compared
    self.compareGroups = []
    # self.dataPulls = [] #List of data references that will be used
    # self.referenceData = [] #List of reference (experimental) data
    # Information on what stuff to do.
    self.methodInfo = {}
    self.fZStats = False
