  ...
  def __runRemoteListeningSockets(self, newPort):
    ...
    if len(uniqueNodes) > 1:
      ...
      for nodeId in uniqueNodes:
        ...
        # subprocess.Popen(['ssh', nodeId, "python2.7",
        # ppserverScript,"-w",str(ntasks),"-i",remoteHostName,"-p",str(newPort),"-t","1000","-g",localenv["PYTHONPATH"],"-d"],shell=False,stdout=outFile,stderr=outFile,env=localenv)
