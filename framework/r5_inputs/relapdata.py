...
class relapdata:
   ...
   def gettime(self,outlines):
      '''    gettime function reads the end of simulation time '''
      for i in outlines:
         if re.match('^\s*Final time=',i):
            time=i.split()[2]
      return time
       ...
       if re.search('RAVEN',self.lines[i]):
         ...
         while flagg==0: 
           if re.search('RAVEN',self.lines[i]):
             ...
             self.ravendata[self.lines[i].split()[1].replace("*","")]=self.lines[i].split()[3]
           i=i+1
         return

   def write_csv(self,file):
      '''   writes the csv file from minor edit data '''
      IOcsvfile=open(file,'w')
      for i in range(len(self.minordata.keys())):
        IOcsvfile.write('%s,' %(self.minordata.keys()[i]))
