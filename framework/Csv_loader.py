...
#     return (NtimeSteps,maxNumOfParams,NSamples)
  ...
  def csvLoadData(self,filein,options):
    ...
    if   options['type'] == 'TimePoint':    return self.__csvLoaderForTimePoint(filein[0],options['time'],options['inParam'],options['outParam'],options['inputTs'])
      ...
      raiseAnError(IOError,self,'Type ' + options['type'] + 'unknown')
  ...
  def __csvLoaderForTimePoint(self,filein,time,inParam,outParam,inputTs):
    ...
    if ints > data[:,0].size -1  and ints != -1: raiseAnError(IOError,self,'inputTs is greater than number of actual ts in file '+ str(filein) + '!')
  ...
  def __csvLoaderForTimePointSet(self,filesin,time,inParam,outParam,inputTs):
    ...
    for i in range(len(filesin)):
      ...
      if ints > data[:,0].size -1  and ints != -1: raiseAnError(IOError,self,'inputTs is greater than number of actual ts in file '+ str(filesin[i]) + '!')
      if i == 0:
        if(self.all_out_param):
      ...
      for key in inParam:
        if key in self.all_field_names:
          ...
          raiseAnError(IOError,self,"the parameter " + str(key) + " has not been found")
      # time end case
      if time_end:
        last_row = data[:,0].size - 1
        if self.all_out_param:
            ...
            if self.all_out_param:
              ...
              for key in outParam:
                if key in self.all_field_names:
                  ...
                  raiseAnError(IOError,self,"the parameter " + key + " has not been found")
      del data
    return (inDict,outDict)
