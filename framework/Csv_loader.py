  ...
  def __csvLoaderForPoint(self,fileIn,options):
      ...
      if ix != None:
        if inputPivotVal != None:
          if float(inputPivotVal) > np.max(data[:
            ,pivotIndex]) or float(inputPivotVal) < np.min(data[
    ...
    if outputPivotValEnd:
      ...
      if self.allOutParam:
        for key in self.allFieldNames:
          if operator == 'max':
            outDict[key] = np.atleast_1d(np.array(np.max(data[
          if operator == 'min':
            outDict[key] = np.atleast_1d(np.array(np.min(data[
          if operator == 'average':
            outDict[key] = np.atleast_1d(np.array(np.average(data[
      else:
        for key in outParam:
          if key in self.allFieldNames:
            if operator == 'max':
      ...
      if self.allOutParam:
        ...
        for key in outParam:
          if key in self.allFieldNames:
            outDict[key] = np.atleast_1d(np.array(interp1d(data[
          else:
            self.raiseAnError(IOError,"the parameter " + key + " has not been found")
        ...
        if self.allOutParam:
          for key in self.allFieldNames:
            if i == 0:
              outDict[key] = np.zeros(len(filesIn))
            if operator == 'max':
              outDict[key][i] = np.max(data[
            if operator == 'min':
              outDict[key][i] = np.min(data[
            if operator == 'average':
              outDict[key][i] = np.average(data[
        else:
          for key in outParam:
            if key in self.allFieldNames:
              if i == 0:
