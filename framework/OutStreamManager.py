...
class OutStreamManager(BaseType):
  ...
  ********************************************************************
  ...
  def addOutput(self):
    ...
    @ Out, None
    '''
    self.raiseAnError(NotImplementedError,self,'method addOutput must be implemented by derived classes!!!!')
    ...
    for agrosindex in range(self.numberAggregatedOS):
      ...
      if not foundData and 'SolutionExport' in inDict.keys():
        if inDict['SolutionExport'].name.strip() == self.sourceName[agrosindex] and inDict['SolutionExport'].type in DataObjects.knownTypes():
          ...
          foundData = True
      if not foundData: self.raiseAnError(IOError,self,'the Data named ' + self.sourceName[agrosindex] + ' has not been found!!!!')
#
#
#
  ...
  def __splitVariableNames(self,what,where):
    ...
    if var:
      if '(' in var and ')' in var:
        if var.count('(') > 1: self.raiseAnError(IOError,self,'In Plot ' +self.name +'.Only a couple of () is allowed in variable names!!!!!!')
        result = var.split('|(')[0].split('|')
        result.append(var.split('(')[1].replace(")", ""))
      else:  result = var.split('|')
    else: result = None
    if len(result) != 3: self.raiseAnError(IOError,self,'In Plot ' +self.name +'.Only three level variables are accepted !!!!!')
    return result
      ...
      if len(node):
        for subnode in node:
          if subnode.tag != 'kwargs':
            ...
            if not subnode.text: self.raiseAnError(IOError,self,'In Plot ' +self.name +'. Problem in sub-tag ' + subnode.tag + ' in '+node.tag+' block. Please check!')
          else:
      ...
      if   key in ['how','plotSettings','figureProperties']: pass
        ...
        if self.dim == 2:
          ...
          if 'zmin' in self.options[key].keys():
            if 'zmax' not in self.options[key].keys(): self.raiseAWarning(self,'zmin inputted but not zmax. zmin ignored! ')
          ...
          if 'zmax' in self.options[key].keys():
            if 'zmin' not in self.options[key].keys(): self.raiseAWarning(self,'zmax inputted but not zmin. zmax ignored! ')
            else:self.plt3D.set_zlim(ast.literal_eval(self.options[key]['zmin']),ast.literal_eval(self.options[key]['zmax']))
        ...
        if self.dim == 2: self.raiseAWarning(self,'2D plots have not a camera attribute... They are 2D!!!!')
        elif self.dim == 3:
        ...
        if 'fontdict' not in self.options[key].keys(): self.options[key]['fontdict'] = 'None'
          ...
          except AttributeError: self.raiseAnError(TypeError,self,'In ' + key +' tag: can not convert the string "' + self.options[key]['fontdict'] + '" to a dictionary! Check syntax for python function ast.literal_eval')
        if self.dim == 2 :
        ...
        if self.dim == 3: self.raiseAWarning(self,'horizontal_line not available in 3-D plots!!')
        elif self.dim == 2:
        ...
        if self.dim == 3: self.raiseAWarning(self,'vertical_line not available in 3-D plots!!')
        elif self.dim == 2:
        ...
        if self.dim == 3: self.raiseAWarning(self,'horizontal_rectangle not available in 3-D plots!!')
        elif self.dim == 2:
          if 'ymin' not in self.options[key].keys(): self.raiseAnError(IOError,self,'ymin parameter is needed for function horizontal_rectangle!!')
          if 'ymax' not in self.options[key].keys(): self.raiseAnError(IOError,self,'ymax parameter is needed for function horizontal_rectangle!!')
          if 'xmin' not in self.options[key].keys()  : self.options[key]['xmin'] = '0'
          if 'xmax' not in self.options[key].keys() : self.options[key]['xmax'] = '1'
        ...
        if self.dim == 3: self.raiseAWarning(self,'vertical_rectangle not available in 3-D plots!!')
        elif self.dim == 2:
          if 'xmin' not in self.options[key].keys(): self.raiseAnError(IOError,self,'xmin parameter is needed for function vertical_rectangle!!')
          if 'xmax' not in self.options[key].keys(): self.raiseAnError(IOError,self,'xmax parameter is needed for function vertical_rectangle!!')
          if 'ymin' not in self.options[key].keys()  : self.options[key]['ymin'] = '0'
          if 'ymax' not in self.options[key].keys() : self.options[key]['ymax'] = '1'
        ...
        if   self.dim == 3: self.raiseAWarning(self,'axes_box not available in 3-D plots!!')
        ...
        self.raiseAWarning(self,'Try to perform not-predifined action ' + key +'. If it does not work check manual and/or relavite matplotlib method specification.')
        command_args = ' '
        ...
        try:
          ...
          self.raiseAnError(RuntimeError,self,'<'+str(ae)+'> -> in execution custom action "' + key + '" in Plot ' + self.name + '.\n ' + self.printTag+' command has been called in the following way: ' + 'self.plt.' + key + '(' + command_args + ')')
  ...
  def initialize(self,inDict):
    ...
    for pltindex in range(len(self.options['plotSettings']['plot'])):
      ...
      if 'y' in self.options['plotSettings']['plot'][pltindex].keys():
        ...
        if self.yCoordinates [pltindex][0].split('|')[0] != self.sourceName[pltindex]: self.raiseAnError(IOError,self,'Every plot can be linked to one Data only. x_cord source is ' + self.sourceName[pltindex] + '. Got y_cord source is' + self.yCoordinates [pltindex][0].split('|')[0])
      if 'z' in self.options['plotSettings']['plot'][pltindex].keys():
        ...
        if self.zCoordinates [pltindex][0].split('|')[0] != self.sourceName[pltindex]: self.raiseAnError(IOError,self,'Every plot can be linked to one Data only. x_cord source is ' + self.sourceName[pltindex] + '. Got z_cord source is' + self.zCoordinates [pltindex][0].split('|')[0])
      if 'colorMap' in self.options['plotSettings']['plot'][pltindex].keys():
        ...
        if self.colorMapCoordinates[pltindex][0].split('|')[0] != self.sourceName[pltindex]: self.raiseAnError(IOError,self,'Every plot can be linked to one Data only. x_cord source is ' + self.sourceName[pltindex] + '. Got colorMap_coordinates source is' + self.colorMapCoordinates[pltindex][0].split('|')[0])
      for pltindex in range(len(self.options['plotSettings']['plot'])):
        ...
        elif self.options['plotSettings']['plot'][pltindex]['interpolationType'] not in self.interpAvail: self.raiseAnError(IOError,self,'surface interpolation unknown. Available are :' + str(self.interpAvail))
...
#            else:             self.options['plotSettings']['plot'][pltindex]['cmap'] = 'jet'
        ...
        elif self.options['plotSettings']['plot'][pltindex]['interpolationTypeBackUp'] not in self.interpAvail: self.raiseAnError(IOError,self,'surface interpolation (BackUp) unknown. Available are :' + str(self.interpAvail))
  ...
  def localReadXML(self,xmlNode):
    ...
    if self.dim not in [2,3]: self.raiseAnError(IOError,self,'Wrong dimension... 2D or 3D only!!! Got '+ str(self.dim)+'D')
    foundPlot = False
    for subnode in xmlNode:
      ...
      if subnode.tag == 'figure_properties':
        ...
        for subsub in subnode: self.options[subnode.tag][subsub.tag] = subsub.text.strip()
    self.type = 'OutStreamPlot'
    if not 'plotSettings' in self.options.keys(): self.raiseAnError(IOError,self,'For plot named ' + self.name + ' the plotSettings block is required.')
    if not foundPlot: self.raiseAnError(IOError,self,'For plot named'+ self.name + ', No plot section has been found in the plotSettings block!')
    self.outStreamTypes = []
    for pltindex in range(len(self.options['plotSettings']['plot'])):
      if not 'type' in self.options['plotSettings']['plot'][pltindex].keys(): self.raiseAnError(IOError,self,'For plot named'+ self.name + ', No plot type keyword has been found in the plotSettings/plot block!')
      else:
        if self.availableOutStreamTypes[self.dim].count(self.options['plotSettings']['plot'][pltindex]['type']) == 0: self.raiseAMessage(self,'For plot named'+ self.name + ', type '+self.options['plotSettings']['plot'][pltindex]['type']+' is not among pre-defined plots! \n The OutstreamSystem will try to construct a call on the fly!','ExceptedError')
    ...
    self.raiseADebug(self,'matplotlib version is ' + str(self.mpl.__version__))
    if self.dim not in [2,3]: self.raiseAnError(TypeError,self,'This Plot interface is able to handle 2D-3D plot only')
  ...
  def addOutput(self):
    ...
    if not self.__fillCoordinatesFromSource():
      self.raiseAWarning(self,'Nothing to Plot Yet... Returning!!!!')
      return
    self.counter += 1
    if self.counter > 1:
    ...
    for pltindex in range(len(self.outStreamTypes)):
      ...
      if 'zlabel' in self.options['plotSettings'].keys():
        if self.dim == 2  : self.raiseAWarning(self,'zlabel keyword does not make sense in 2-D Plots!')
      ...
      #  SCATTER PLOT #
      #################
      self.raiseADebug(self,'creating plot'+ self.name)
      ...
      ##################
        ...
        if self.dim == 2:
          ...
          self.raiseAWarning(self,'step Plot not available in 3D')
          return
      ########################
      #    PSEUDOCOLOR PLOT  #
      ########################
        ...
        if self.dim == 2:
            ...
            for x_index in range(len(self.xValues[pltindex][key])):
              for y_index in range(len(self.yValues[pltindex][key])):
                if not self.colorMapCoordinates:
                  self.raiseAMessage(self,'STREAM MANAGER: pseudocolor Plot needs coordinates for color map... Returning without plotting')
                  return
          ...
          self.raiseAWarning(self,'pseudocolor Plot is considered a 2D plot, not a 3D!')
          return
      ########################
      #     SURFACE PLOT     #
      ########################
        ...
        if self.dim == 2:
          self.raiseAWarning(self,'surface Plot is NOT available for 2D plots, IT IS A 3D!')
          return
        elif self.dim == 3:
      ...
      ########################
        ...
        if self.dim == 2:
          self.raiseAWarning(self,'TRI-surface Plot is NOT available for 2D plots, it is 3D!')
          return
        elif self.dim == 3:
          if 'color' not in self.options['plotSettings']['plot'][pltindex].keys(): self.options['plotSettings']['plot'][pltindex]['color'] = 'b'
      ...
      ########################
        ...
        if self.dim == 2:
          self.raiseAWarning(self,'wireframe Plot is NOT available for 2D plots, IT IS A 3D!')
          return
        elif self.dim == 3:
          if 'rstride' not in self.options['plotSettings']['plot'][pltindex].keys(): self.options['plotSettings']['plot'][pltindex]['rstride'] = '1'
          if 'cstride' not in self.options['plotSettings']['plot'][pltindex].keys(): self.options['plotSettings']['plot'][pltindex]['cstride'] = '1'
            ...
            for x_index in range(len(self.xValues[pltindex][key])):
              for y_index in range(len(self.yValues[pltindex][key])):
                for z_index in range(len(self.zValues[pltindex][key])):
                  ...
                  xig, yig, zi = utils.interpolateFunction(self.xValues[pltindex][key][x_index],self.yValues[pltindex][key][y_index],self.options['plotSettings']['plot'][pltindex],z = self.zValues[pltindex][key][z_index],returnCoordinate=True)
                  if self.colorMapCoordinates[pltindex] != None:
                    self.raiseAWarning(self,'Currently, ax.plot_wireframe() in MatPlotLib version: '+self.mpl.__version__+' does not support a colormap! Wireframe plotted on a surface plot...')
                    if self.actcm: first = False
                    else         : first = True
                    if self.options['plotSettings']['plot'][pltindex]['cmap'] == 'None': self.options['plotSettings']['plot'][pltindex]['cmap'] = 'jet'
      ...
      ########################
        ...
        if self.dim == 2:
          ...
          for key in self.xValues[pltindex].keys():
            if not self.colorMapCoordinates:
              self.raiseAWarning(self,self.outStreamTypes[pltindex]+' Plot needs coordinates for color map... Returning without plotting')
              return
            for x_index in range(len(self.xValues[pltindex][key])):
              for y_index in range(len(self.yValues[pltindex][key])):
                for z_index in range(len(self.colorMapValues[pltindex][key])):
          ...
          self.raiseAWarning(self,'contour/filledContour is a 2-D plot, where x,y are the surface coordinates and colorMap vector is the array to visualize!\n               contour3D/filledContour3D are 3-D! ')
          return
      elif self.outStreamTypes[pltindex] == 'contour3D' or self.outStreamTypes[pltindex] == 'filledContour3D':
        if self.dim == 2:
          self.raiseAWarning(self,'contour3D/filledContour3D Plot is NOT available for 2D plots, IT IS A 2D! Check "contour/filledContour"!')
          return
        elif self.dim == 3:
          if 'numberBins' in self.options['plotSettings']['plot'][pltindex].keys(): nbins = int(self.options['plotSettings']['plot'][pltindex]['numberBins'])
          else: nbins = 5
        ...
        self.raiseAWarning(self,'Try to create a not-predifined plot of type ' + self.outStreamTypes[pltindex] +'. If it does not work check manual and/or relavite matplotlib method specification.')
        command_args = ' '
        import CustomCommandExecuter as execcommand
    ...
    if 'screen' in self.options['how']['how'].split(',') and disAvail:
      def handle_close(event):
        ...
        self.raiseAMessage(self,'Closed Figure')
...
def returnInstance(Type,caller):
  ...
  except KeyError: caller.raiseAnError(NameError,'OUTSTREAMMANAGER','not known '+__base+' type '+Type)
