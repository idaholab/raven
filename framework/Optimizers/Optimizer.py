...
class Optimizer(Sampler):
  ...
  def getInputSpecification(cls):
    ...
    shape = InputData.parameterInputFactory(
        'shape', contentType=InputData.StringListType)
  ...
  def __init__(self):
    ...
    }  # as {traj: [pt0, pt1]} where each pt is {'inputs':{var:val}, 'output':val}, the two most recently-accepted points by value
    ...
    self.limit[
        'mdlEval'] = 2000  # Maximum number of the loss function evaluation
    self.limit[
        'varsUpdate'] = 650  # Maximum number of the optimization iteration.
    #variable lists
    ...
    self.mlHoldBatches = {
    }  # dict of {batchName:[list,of,vars]} that defines the optional output subspaces that need to be kept constant till convergence of this space
    self.mlSequence = [
    ]  # list of batch names that determines the order of convergence.  Last entry is converged most often and fastest (innermost loop).
    self.mlDepth = {
    }  # {traj: #} index of current recursion depth within self.mlSequence, must be initialized to None
    self.mlStaticValues = {
    ...
    for child in paramInput.subparts:
      ...
      if child.getName() == "variable":
        ...
        try:
          ...
          self.raiseAnError(
              IOError, '"{}" node does not have the "name" attribute'.format(
                  child.getName()))
        ...
        for childChild in child.subparts:
          if childChild.getName() == "upperBound":
            ...
            try:
              ...
              self.raiseAnError(
                  IOError,
                  'Failed to interpret "shape" for variable "{}"! Should be comma-separated list of integers.'.
                  format(varName))
        ...
        try:
          ...
          self.raiseAnError(
              KeyError,
              child.getName() +
              '"constant" nodes must have the attribute "name" (missing in "{}").'.
              format(self.name))
  ...
  def updateVariableHistory(self, data, traj):
    ...
    allData = {}
    allData.update(
        self.normalizeData(data))  # data point not normalized a priori
