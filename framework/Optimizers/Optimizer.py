...
class Optimizer(Sampler):
  """
    ...
    values to optimize a loss function.
  ...
  def getInputSpecification(cls):
    ...
    # so far, there's no distinction between optimizers, so all the input from SPSA is here. Fix this when we sort
    # out optimizers.

    inputSpecification = super(Optimizer, cls).getInputSpecification()
    # assembled objects
    targEval = InputData.parameterInputFactory(
        'TargetEvaluation', contentType=InputData.StringType, strictMode=True)
    ...
    precond = InputData.parameterInputFactory(
        'Preconditioner', contentType=InputData.StringType, strictMode=True)
    ...
    whenWriteEnum = InputData.makeEnumType('whenWriteEnum', 'whenWriteType',
                                           ['final', 'every'])
    ...
    write = InputData.parameterInputFactory(
        'writeSteps', contentType=whenWriteEnum)
    init.addSub(limit)
    init.addSub(seed)
    init.addSub(minmax)
    init.addSub(thresh)
    ...
    rel = InputData.parameterInputFactory(
        'relativeThreshold', contentType=InputData.FloatType)
    abst = InputData.parameterInputFactory(
        'absoluteThreshold', contentType=InputData.FloatType)
    grad = InputData.parameterInputFactory(
        'gradientThreshold', contentType=InputData.FloatType)
    minstep = InputData.parameterInputFactory(
        'minStepSize', contentType=InputData.FloatType)
    grow = InputData.parameterInputFactory(
        'gainGrowthFactor', contentType=InputData.FloatType)
    ...
    stoch = InputData.parameterInputFactory(
        'stochasticDistribution', contentType=stochEnum)
    ...
    sequence = InputData.parameterInputFactory(
        'sequence', contentType=InputData.StringType)
    multilevel.addSub(sequence)
    subspace = InputData.parameterInputFactory(
        'subspace', contentType=InputData.StringListType, strictMode=True)
  ...
  def __init__(self):
    ...
    self.counter = {}
    # Counter of the model evaluation performed (better the input generated!!!). It is reset by calling the function
    # self.initialize
    self.counter['mdlEval'] = 0
    # Counter of the optimization iteration.
    self.counter['varsUpdate'] = 0
    # as {traj: [pt0, pt1]} where each pt is {'inputs':{var:val}, 'output':val}, the two most recently-accepted
    # points by value
    self.counter['recentOptHist'] = {}
    # as {traj: [prefix1, prefix2]} where each prefix is the job identifier for each trajectory
    self.counter['prefixHistory'] = {}
    # as {traj: n} where n is the number of consecutive converges
    self.counter['persistence'] = {}
    #limits
    ## while "limit" is scalar in Sampler, it's more complicated in Optimizer
    # Dict containing limits for each counter
    self.limit = {}
    # Maximum number of the loss function evaluation
    self.limit['mdlEval'] = 2000
    # Maximum number of the optimization iteration.
    self.limit['varsUpdate'] = 650
    #variable lists
    # Objective variable to be optimized
    self.objVar = None
    # By trajectory, current decision variables for optimization
    self.optVars = {}
    # Decision variables for optimization, full space
    self.fullOptVars = None
    # Identifiers of parallel optimization trajectories
    self.optTraj = None
    ...
    # Dict of the ranges (min and max) of each variable's domain
    ...
    self.paramDict = {}
    # Sampler that can be used to initialize the optimizer trajectories
    self.initializationSampler = None
    # Dict {var1:<initial> present?,var2:<initial> present?}
    self.optVarsInitialized = {}
    #convergence tools
    # History of normalized decision variables for each iteration
    self.optVarsHist = {}
    # Threshold used to determine the convergence of parallel optimization trajectories
    self.thresholdTrajRemoval = None
    ...
    # number of retries to attempt before accepting convergence
    self.convergencePersistence = 1
    ...
    # by traj, a place (deque) to store points that should be submitted some time after they are discovered
    self.submissionQueue = {}
    #functions and dataojbects
    # External constraint function, could be not present
    self.constraintFunction = None
    # by name, Models that might be used as preconditioners
    self.preconditioners = {}
    # This is the data used to export the solution
    self.solutionExport = None
    # Containing information of all model evaluation
    self.mdlEvalHist = None
    # ROM used internally for fast loss function evaluation
    self.objSearchingROM = None
    #multilevel
    # indicates if operating in multilevel mode
    self.multilevel = False
    # dict of {batchName:[list,of,vars]} that defines input subspaces
    self.mlBatches = {}
    # dict of {batchName:[list,of,vars]} that defines the optional output subspaces that need to be kept constant
    # till convergence of this space
    self.mlHoldBatches = {}
    # list of batch names that determines the order of convergence. Last entry is converged most often and fastest
    # (innermost loop).
    self.mlSequence = []
    # } index of current recursion depth within self.mlSequence, must be initialized to None
    self.mlDepth = {}
    # by traj, dictionary of static values for variables in fullOptVars but not in optVars due to multilevel
    self.mlStaticValues = {}
    # by traj, dictionary of list of output that must be kept constant due to multilevel
    self.mlOutputStaticVariables = {}
    # by traj, integer to track iterations performed in optimizing the current, active subspace
    self.mlActiveSpaceSteps = {}
    # by batch, by traj, info includes 'lastStepSize','gradientHistory','recommendToGain'
    self.mlBatchInfo = {}
    # by batch, the preconditioner models to use when transitioning subspaces
    self.mlPreconditioners = {}
    #stateful tracking
    # by traj, the next recommended point (as a dict) in the input space to move to
    self.recommendedOptPoint = {}
    # tool for localStillReady to inform localGenerateInput on the next action needed
    self.nextActionNeeded = (None, None)
    self.status = {
    }  # by trajectory, ("string-based status", arbitrary, other, entries)
    ...
    # "collecting grad eval points" - all the required gradient evaluation points are submitted, so we're just
    # waiting to collect them
    # "submitting new opt points" - a new optimal point has been postulated, and is being submitted for evaluationa
    # (not actually used)
    # "collecting new opt points" - the new hypothetical optimal point has been submitted, and we're waiting on it
    # to finish
    # "evaluate gradient" - localStillReady notes we have all the new grad eval points, and has flagged for gradient
    # to be evaluated in localGenerateInput
    ...
    # "received recommended point" - something other than the normal algorithm (such as a preconditioner) suggested
    # a point
    # example usage:
    # self.status[traj]['process'] == 'submitting grad eval points' and self.status[traj]['reason'] == 'rejecting
    # bad opt point'
    #
    ### END explanation
    ...
    self.addAssemblerObject(
        'Sampler', '-1'
    )  #This Sampler can be used to initialize the optimization initial points (e.g. partially replace the <initial> blocks for some variables)
  ...
  def _localGenerateAssembler(self, initDict):
    ...
    for mainClass in ['Functions', 'Distributions', 'DataObjects']:
      for funct in initDict[mainClass]:
        self.assemblerDict[mainClass].append([
            mainClass, initDict[mainClass][funct].type, funct,
  ...
  def _localWhatDoINeed(self):
    ...
    """
    needDict = {}
    # We get ALL Distributions in case a Sampler is used for the initialization of the initial points
    needDict['Distributions'] = [(None, 'all')]
    # We get ALL Functions in case a Sampler is used for the initialization of the initial points
    needDict['Functions'] = [(None, 'all')]
    # We get ALL DataObjects in case a CustomSampler is used for the initialization of the initial points
  ...
  def _readMoreXML(self, xmlNode):
    """
      ...
      @ Out, None
    """
    # TODO can be combined with Sampler's _readMoreXML, but needs to implement paramInput passing to
    # localInputAndChecks (new input checker)
  ...
  def _readMoreXMLbase(self, xmlNode):
        ...
        for childChild in child.subparts:
          if childChild.getName() == "upperBound":
            ...
            if self.optTraj == None:
              self.optTraj = range(
                  len(self.optVarsInit['initial'][varName].keys()))
        ...
        for subnode in child.subparts:
          if subnode.getName() == 'subspace':
            ...
            try:
              ...
              self.raiseAnError(
                  IOError,
                  'A multilevel subspace is missing the "name" attribute!')
            if name in self.mlBatches.keys():
              self.raiseAnError(
                  IOError,
                  'Multilevel subspace "{}" has a duplicate name!'.format(
                      name))
            if "holdOutputSpace" in subnode.parameterValues:
              self.mlHoldBatches[name] = [
                  var.strip() for var in subnode.parameterValues[
                      'holdOutputSpace'].split(",")
              ]
              self.raiseAMessage(
                  'For subspace "' + name +
                  '" the following output space is asked to be kept on hold: '
                  + ','.join(self.mlHoldBatches[name]))
            #subspace text
            subspaceVars = subnode.value
            if len(subspaceVars) < 1:
              self.raiseAnError(
                  IOError,
                  'Multilevel subspace "{}" has no variables specified!'.
                  format(name))
    ...
    if self.objVar is None:
      self.raiseAnError(IOError,
                        'Object variable is not specified for optimizer!')
    if self.fullOptVars is None:
      self.raiseAnError(IOError,
                        'Decision variable(s) not specified for optimizer!')
    ...
    for varName in self.fullOptVars:
      if varName not in self.optVarsInit['upperBound'].keys():
        self.raiseAnError(IOError,
                          'Upper bound for ' + varName + ' is not provided')
      if varName not in self.optVarsInit['lowerBound'].keys():
        self.raiseAnError(IOError,
                          'Lower bound for ' + varName + ' is not provided')
      #store ranges of variables
      self.optVarsInit['ranges'][
          varName] = self.optVarsInit['upperBound'][varName] - self.optVarsInit['lowerBound'][varName]
    ...
    if self.multilevel:
      if len(self.mlSequence) < 1:
        self.raiseAnError(
            IOError,
            'No "sequence" was specified for multilevel optimization!')
      if set(self.mlSequence) != set(self.mlBatches.keys()):
        self.raiseAWarning(
            'There is a mismatch between the multilevel batches defined and batches used in the sequence!  Some variables may not be optimized correctly ...'
        )
  ...
  def _numberOfSamples(self, traj=None):
    ...
    """
    return sum(
        np.prod(self.variableShapes[var]) for var in self.getOptVars(traj))
  ...
  def initialize(self, externalSeeding=None, solutionExport=None):
    ...
    for entry in self.assemblerDict.get('Sampler', []):
      ...
      for key in self.initializationSampler.getInitParams().keys():
        if key.startswith("sampled variable:"):
          ...
          if var not in self.getOptVars():
            self.raiseAnError(IOError, 'The variable "' + var +
      ...
      while self.initializationSampler.amIreadyToProvideAnInput():
        ...
        for varName, value in sampledVars.items():
          self.optVarsInit['initial'][varName][
              self.initializationSampler.counter] = np.atleast_1d(value)
        self.initializationSampler.counter += 1

    # NOTE: counter['varsUpdate'] needs to be set AFTER self.optTraj length is set by the sampler (if used
    # exclusively)
    self.counter['mdlEval'] = 0
    ...
    for varName in self.fullOptVars:
      if varName not in ins:
        self.raiseAnError(RuntimeError,
                          "the optimization variable " + varName +
    ...
    if type(solutionExport).__name__ != "PointSet":
      self.raiseAnError(IOError,
                        'solutionExport type must be a PointSet. Got ' +
    ...
    for batch, precondName in self.mlPreconditioners.items():
      try:
        ...
        self.raiseAnError(
            IOError,
            'Could not find preconditioner "{}" in <Preconditioner> nodes!'.
            format(precondName))

    # apply multilevel preconditioners, in order
    for traj in self.optTraj:
      # initial point(s) are in self.optVarsInit['initial']
      initPoint = dict((var, self.optVarsInit['initial'][var][traj])
                       for var in self.optVarsInit['initial'].keys())
      # run all preconditioners on that point
      for depth in range(len(self.mlSequence)):
        batch = self.mlSequence[depth]
        initPoint = self.applyPreconditioner(
            batch, initPoint, denormalize=False)
      #check initial point consistency
      okay, missing = self.checkInputs(initPoint)
      if not okay:
        self.raiseAnError(
            IOError,
            'While initializing model inputs, some were not set! Set them through preconditioners or using the <initial> block or a linked Sampler.\n  Missing:',
            ', '.join(missing))
    ...
    # the hypothetical correct length
    rightLen = len(self.optTraj)
    for var in self.getOptVars(full=True):
    ...
    for trajInd in self.optTraj:
      ...
      if not satisfied:
        ...
        self.raiseAWarning(
            "the initial values specified for trajectory " + str(trajInd) +
        ...
        while not satisfied and randomGuessesCnt < self.constraintHandlingPara['innerLoopLimit']:
          for varName in self.getOptVars():
            varK[
                varName] = self.optVarsInit['lowerBound'][varName] + randomUtils.random(
  ...
  def applyPreconditioner(self, batch, originalPoint, denormalize=True):
    """
      ...
      @ In, denormalize, bool, optional, if True then the originalPoint will be denormalized before running in the
             preconditioner
    ...
    if precond is not None:
      ...
      # TODO someday this might need to be extended when other models or more complex external models are used for
      # precond
      ...
      preResults = precond.evaluateSample([infoDict['SampledVars']],
                                          'Optimizer', infoDict)
  ...
  def amIreadyToProvideAnInput(self):
    """
      ...
      Reason for not being ready could be for example: exceeding number of model evaluation, convergence criteria
      met, etc.
      @ In, None
    ...
    # -> however, if we're waiting on point collection, don't do multilevel check; only when we want to submit a new
    # point.
    ...
    # 4.) We're in a non-innermost subspace, and have perturbed but not converged, so we need to move back to
    # innermost again
    # 5.) We're in an intermediate subspace, and have perturbed and converged, so we need to move to one subspace
    # higher
    # will be True if we changed the state of the optimizer
    mlIntervene = False
    ...
    for traj in checkMLTrajs:
      if self.multilevel and self.status[traj]['reason'] in [
          'found new opt point', 'converged'
      ]:
        # do we have any opt points yet?
        if len(self.counter['recentOptHist'][traj][0]) > 0:
          # get the latset optimization point (normalized)
          latestPoint = dict((var, self.counter['recentOptHist'][traj][0][var])
                             for var in self.getOptVars())
          #some flags for clarity of checking
          justStarted = self.mlDepth[traj] is None
          inInnermost = self.mlDepth[traj] is not None and self.mlDepth[traj] == len(
              self.mlSequence) - 1
          ...
          if justStarted:
            self.raiseADebug(
                'Multilevel: initializing for trajectory "{}"'.format(traj))
            self.updateMultilevelDepth(
                traj, len(self.mlSequence) - 1, latestPoint, setAll=True)
            mlIntervene = True
          ...
          # otherwise, if we're in the innermost subspace AND we're converged, then move to a higher subspace
            ...
            if inOutermost:
              self.raiseADebug(
                  'Multilevel: outermost subspace converged for trajectory "{}"!'.
                  format(traj))
            else:
              self.raiseADebug(
                  'Multilevel: moving from converged subspace to higher subspace for trajectory "{}"'.
                  format(traj))
              self.updateMultilevelDepth(traj, self.mlDepth[traj] - 1,
                                         latestPoint)
              mlIntervene = True
          ...
          # aka not converged
            ...
            if not inInnermost and self.mlActiveSpaceSteps[traj] >= 1:
              self.raiseADebug(
                  'Multilevel: moving from perturbed higher subspace back to innermost subspace for trajectory "{}"'.
                  format(traj))
              self.updateMultilevelDepth(
                  traj, len(self.mlSequence) - 1, latestPoint, setAll=True)
              mlIntervene = True
    ...
    if mlIntervene:
      self.raiseADebug(
          'Because multilevel intervened, rechecking readiness of optimizer for trajectory "{}"'.
          format(traj))
  ...
  def updateMultilevelDepth(self, traj, depth, optPoint, setAll=False):
    """
      ...
      @ In, depth, int, recursion depth in subspace loops, which ranges between 0 and the last index of
             self.multilevelSequence
    ...
    self.raiseADebug(
        'Transitioning multilevel subspace from "{}" to "{}" for trajectory "{}"...'.
        format(oldBatch, newBatch, traj))
    ...
    if depth > oldDepth:
      self.raiseADebug('Preconditioning subsets below', oldDepth,
                       range(oldDepth + 1, depth + 1))
  ...
  def checkConstraint(self, optVars):
    """
      ...
      @ In, optVars, dict, dictionary containing the value of decision variables to be checked, in form of {varName:
             varValue}
      @ Out, satisfaction, tuple, (bool,list) => (variable indicating the satisfaction of constraints at the point
             optVars, masks for the under/over violations)
    """
    ...
    for var in optVars:
      ...
      if np.sum(overMask) + np.sum(underMask) > 0:
        self.raiseAWarning(
            'A variable violated boundary constraints! "{}"={}'.format(
                var, optVars[var]))
        satisfied = False
  ...
  def localCheckConstraint(self, optVars, satisfaction=True):
    """
      ...
      @ In, optVars, dict, dictionary containing the value of decision variables to be checked, in form of {varName:
             varValue}
      @ In, satisfaction, bool, optional, variable indicating how the caller determines the constraint satisfaction
             at the point optVars
  ...
  def normalizeData(self, optVars):
    """
      ...
      @ In, optVars, dict, dictionary containing the value of decision variables to be normalized, in form of
             {varName: varValue}
      @ Out, optVarsNorm, dict, dictionary containing the value of normalized decision variables, in form of
             {varName: varValue}
    """
    optVarsNorm = {}
    for var in optVars.keys():
      try:
        optVarsNorm[var] = (optVars[var] - self.optVarsInit['lowerBound'][var]
  ...
  def denormalizeData(self, optVars):
    """
      Method to normalize the data
      @ In, optVars, dict, dictionary containing the value of decision variables to be deormalized, in form of
             {varName: varValue}
      @ Out, optVarsDenorm, dict, dictionary containing the value of denormalized decision variables, in form of
             {varName: varValue}
    """
    optVarsDenorm = {}
    for var in optVars.keys():
      try:
        optVarsDenorm[
            var] = optVars[var] * (self.optVarsInit['upperBound'][var] -
                                   self.optVarsInit['lowerBound'][var]
                                   ) + self.optVarsInit['lowerBound'][var]
      except KeyError:
  ...
  def updateVariableHistory(self, data, traj):
    """
      ...
      # }
      @ In, data, dict, UNNORMALIZED new input space entries as {var:
      @ In, traj, int, integer label of the current trajectory of interest
      @ Out, None
    """
    # collect static vars, values
    allData = {}
    # data point not normalized a priori
    allData.update(self.normalizeData(data))
    # these are normalized
    allData.update(self.mlStaticValues[traj])
    self.optVarsHist[traj][self.counter['varsUpdate'][traj]] = copy.deepcopy(
        allData)
