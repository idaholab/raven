...
class SPSA(GradientBasedOptimizer):
  ...
  def localInputAndChecks(self, xmlNode):
    ...
    if self.paramDict['a'] is None:
      self.paramDict['a'] = mathUtils.hyperdiagonal(
          np.ones(numValues))  # the features are always normalized
  ...
  def localStillReady(self, ready, convergence=False):
    ...
    """
    self.nextActionNeeded = (None,
                             None)  #prevents carrying over from previous run
