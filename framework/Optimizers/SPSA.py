...
class SPSA(GradientBasedOptimizer):
  ...
  def getInputSpecification(cls):
    ...
    param.addSub(
        InputData.parameterInputFactory(
            'gamma', contentType=InputData.FloatType, strictMode=True))
    param.addSub(
        InputData.parameterInputFactory(
            'c', contentType=InputData.FloatType, strictMode=True))
    param.addSub(
        InputData.parameterInputFactory(
            'a', contentType=InputData.FloatType, strictMode=True))
    param.addSub(
        InputData.parameterInputFactory(
            'alpha', contentType=InputData.FloatType, strictMode=True))
    param.addSub(
        InputData.parameterInputFactory(
            'A', contentType=InputData.FloatType, strictMode=True))
  ...
  def __init__(self):
    ...
    # Distribution used to generate perturbations
    self.stochasticDistribution = None
    # Random number generator used to generate perturbations
  ...
  def localInputAndChecks(self, xmlNode):
    ...
    # FIXME normalizing doesn't seem to have the desired effect, currently; it makes the step size very small (for
    # large scales)
    ...
    if self.paramDict['a'] is None:
      # the features are always normalized
      self.paramDict['a'] = mathUtils.hyperdiagonal(np.ones(numValues))
    ...
    if stochDist == 'Bernoulli':
      ...
      # Initialize bernoulli distribution for random perturbation. Add artificial noise to avoid that specular loss
      # functions get false positive convergence
      ...
      self.raiseAnError(IOError, self.paramDict['stochasticEngine'] +
  ...
  def localLocalInitialize(self, solutionExport):
    """
      ...
      @ Out, None
    """
    self._endJobRunnable = (self._endJobRunnable *
                            self.gradDict['pertNeeded']) + len(self.optTraj)
  ...
  def _newOptPointAdd(self, gradient, traj):
    ...
    """
    # Compute the new ak
    ak = self._computeGainSequenceAk(self.paramDict,
                                     self.counter['varsUpdate'][traj], traj)
    self.optVarsHist[traj][self.counter['varsUpdate'][traj]] = {}
    varK = dict((var, self.counter['recentOptHist'][traj][0][var])
                for var in self.getOptVars(traj))
    varKPlus, modded = self._generateVarsUpdateConstrained(
        traj, ak, gradient, varK)
    ...
    if modded:
      ...
      self.raiseADebug('Resetting step size for trajectory', traj,
  ...
  def localStillReady(self, ready, convergence=False):
    ...
    """
    # prevents carrying over from previous run
    self.nextActionNeeded = (None, None)
    #get readiness from parent
    ready = ready and GradientBasedOptimizer.localStillReady(
        self, ready, convergence)
    ...
    for traj in self.optTraj:
      self.raiseADebug(
          '   Traj: "{:^n}": Process: "{:^30.30}", Reason: "{:^30.30}"'.format(
              traj,
              self.status[traj].get('process', 'None'),
              self.status[traj].get('reason', 'None'),
              n=len(str(max(self.optTraj)))))
    for _ in range(len(self.optTrajLive)):
      ...
      # still in the process of submitting new opt point evaluations (for stochastic denoising)
        ...
        if reason == 'just started':
          ...
          self.raiseAnError(RuntimeError,
                            'unexpected reason for submitting new opt points:',
                            reason)
      ...
      # all gradient evaluation points submitted, but waiting for them to be collected
        ...
        for pertID in range(self.gradDict['pertNeeded']):
          if not self._checkModelFinish(traj, self.counter['varsUpdate'][traj],
                                        pertID)[0]:
            evalsFinished = False
            break
        ...
        if evalsFinished:
          ...
          for i in self.perturbationIndices:
            evalIndex = self._checkModelFinish(
                traj, self.counter['varsUpdate'][traj], i)[1]
            outval = float(
                self.mdlEvalHist.realization(index=evalIndex)[self.objVar])
          ...
          if len(self.submissionQueue[traj]) == 0:
            ...
            if self.status[traj]['reason'] == 'removed as redundant':
              # loops back to the next opt traj
              continue
          self.nextActionNeeded = ('add more opt point evaluations', traj)
        ...
        # otherwise, we're not ready to sample yet
          ...
          self.raiseADebug(
              '    Traj "{}": Waiting on collection of gradient evaluation points.'.
              format(traj))
          continue
      ...
      # all opt point evaluations have been submitted, but waiting for them to be collected
        ...
        self.raiseADebug(
            '    Traj "{}": Waiting on collection of new optimization point.'.
            format(traj))
        continue
      ...
      # trajectory has already converged, so there's no work to do for it.
        ...
        self.raiseADebug(
            '    Traj "{}": Trajectory is marked as converged.'.format(traj))
        continue

      # trajectory didn't converge, but was killed because of redundancy
      elif reason == 'removed as redundant':
        self.raiseADebug(
            '    Traj "{}": Trajectory is removed for redundancy.'.format(
                traj))
        continue
      # unknown status
      else:
        self.raiseAnError(RuntimeError, 'Unrecognized status:'.format(traj),
                          self.status[traj])
    ...
    if self.nextActionNeeded[0] is None:
      ...
      self.raiseADebug('Next action needed: "%s" on trajectory "%i"' %
  ...
  def localEvaluateGradient(self, optVarsValues, traj, gradient=None):
    ...
    for i in range(numGrads):
      # the latest opt point
      opt = optVarsValues[i]
      # the perturbed point
      pert = optVarsValues[i + numGrads]
      ...
      for var in self.getOptVars(traj=traj):
        ...
        # a sample so close cannot be taken without violating minimum step, so this check should not be necessary
        # (left for reference)
        #if abs(dh) < 1e-15:
  ...
  def localGenerateInput(self, model, oldInput):
    ...
    if action == 'start new trajectory':
      if len(self.submissionQueue[traj]) == 0:
        ...
        for var in self.getOptVars(traj=traj):
          ...
          values[var] = self._checkBoundariesAndModify(
      ...
      # note that currently we use the opt point as half of the perturbation point, so each gradient eval will be
      # between
      ...
      if self.counter['perturbation'][traj] == 1:
        ...
        ck = self._computeGainSequenceCk(self.paramDict,
                                         self.counter['varsUpdate'][traj] + 1)
        varK = dict((var, self.counter['recentOptHist'][traj][0][var])
                    for var in self.getOptVars(traj))
        # check the submission queue is empty; otherwise something went wrong
        # TODO this is a sanity check, might be removed for efficiency
          ...
          for var in self.getOptVars(traj=traj):
            ...
            if size > 1:
              ...
              for v, origVal in enumerate(varK[var]):
                ...
                new[v] = self._checkBoundariesAndModify(
                    1.0, 0.0, 1.0, new[v], 0.9999, 0.0001)
                index += 1
              point[var] = new
            else:
              val = varK[var] + ck * direction[index]
              index += 1
              val = self._checkBoundariesAndModify(1.0, 0.0, 1.0, val, 0.9999,
                                                   0.0001)
              point[var] = val
          #create identifier
          prefix = self._createEvaluationIdentifier(
              traj, self.counter['varsUpdate'][traj], i)
          #queue it up
          self.submissionQueue[traj].append({
              'inputs': point,
              'prefix': prefix
          })
      ...
      prefix = entry['prefix']
      point = entry['inputs']
      # self.normalizeData(point)}
      self.gradDict['pertPoints'][traj][int(prefix.split('_')[-1])] = {
          'inputs': point
      }
    ...
    #unrecognized action
      ...
      self.raiseAnError(RuntimeError,
                        'Unrecognized "action" in localGenerateInput:', action)
    self.raiseADebug('Queuing run "{}"'.format(self.inputInfo['prefix']))

  def _checkBoundariesAndModify(self, upperBound, lowerBound, varRange,
                                currentValue, pertUp, pertLow):
    """
    ...
    if isinstance(currentValue, np.ndarray):
      convertedValue = np.asarray([
          self._checkBoundariesAndModify(upperBound, lowerBound, varRange,
                                         value, pertUp, pertLow)
  ...
  def clearCurrentOptimizationEffort(self, traj):
    ...
    self.status[traj] = {
        'process': 'submitting grad eval points',
        'reason': 'found new opt point'
    }
    try:
      del self.counter['lastStepSize'][traj]
    except KeyError:
  ...
  def _generateVarsUpdateConstrained(self, traj, ak, gradient, varK):
    ...
    """
    varKPlus = {}
    # technically too many entries, but unneeded ones will be *0 anyway just below here
    gain = [ak] * self._numberOfSamples()
    ...
    for var in self.getOptVars(traj=traj):
      ...
      if numSamples == 1:
        ...
        for i in range(numSamples):
          ...
          index += 1
      varKPlus[var] = new
    satisfied, activeViolations = self.checkConstraint(
    ...
    if len(activeViolations['internal']) > 0:
      ...
      for var, under, over in activeViolations['internal']:
        if np.prod(self.variableShapes[var]) == 1:
          ...
          projectedOnBoundary[var] = self.denormalizeData({
              var: varKPlus[var]
          })[var]
      ...
      newNormWithoutComponents = self.calculateMultivectorMagnitude(
          gradient.values())
    ...
    # Try to find varKPlus by rotate the gradient towards its orthogonal, since we consider the gradient as
    # perpendicular
    ...
    if innerLoopLimit < 0:
      self.raiseAnError(
          IOError,
          'Limit for internal loop for constraint handling should be nonnegative'
      )
    loopCounter = 0
    ...
    while not foundPendVector and loopCounter < innerLoopLimit:
      ...
      depVarPos = randomUtils.randomIntegers(
          0, len(self.getOptVars(traj=traj)) - 1, self)
      # if that variable is multidimensional, pick a dimension -> this is not precisely equal probability of
      # picking, but that should be okay.
      ...
      for varID, var in enumerate(self.getOptVars(traj=traj)):
        ...
        if varSize == 1:
          ...
          for i in range(varSize):
            pendVector[var][
                i] = self.stochasticEngineForConstraintHandling.rvs() if (
                    varID != depVarPos and depVarIdx != i) else 0.0
      ...
      r = self.calculateMultivectorMagnitude(
          [gradient[var] for var in self.getOptVars(traj=traj)])
      r /= self.calculateMultivectorMagnitude(
          [pendVector[var] for var in self.getOptVars(traj=traj)])
      for var in self.getOptVars(traj=traj):
      ...
      for var in self.getOptVars(traj=traj):
        ...
        for i in range(varSize):
          if varSize == 1:
            ...
            new[i] = copy.copy(
                varK[var][i] - gain[index] * pendVector[var][i] * 1.0)
          index += 1
        varKPlus[var] = new
        ...
        for var in self.getOptVars(traj=traj):
          ...
          for i in range(varSize):
            if varSize == 1:
              ...
              new[i] = copy.copy(
                  varK[var][i] - gain[index] * sumVector[var][i] * 1.0)
            index += 1
  ...
  def _bisectionForConstrainedInput(self, traj, varK, ak, vector):
    """
      ...
      @ In, ak, float or array, it is step size (gain) for variable update (if array, different gain for each
             variable)
      @ In, vector, dictionary, contains the gradient information as a unit vector
      @ Out, _bisectionForConstrainedInput, tuple(bool,dict), (indicating whether a fractional step is found,
             contains the fraction of step that satisfies constraint)
    """
    try:
      gain = ak[:]
    ...
    innerBisectionThreshold = self.constraintHandlingPara[
        'innerBisectionThreshold']
    # FIXME REWORK this is an input check, not a runtime check
    if innerBisectionThreshold <= 0 or innerBisectionThreshold >= 1:
      self.raiseAnError(
          ValueError,
          'The innerBisectionThreshold should be greater than 0 and less than 1'
      )
    bounds = [0, 1.0]
    tempVarNew = {}
    frac = 0.5
      ...
      for var in self.getOptVars(traj=traj):
        ...
        for i in range(numSamples):
          if numSamples > 1:
            ...
            new[i] = copy.copy(
                varK[var][i] - gain[index] * vector[var][i] * 1.0 * frac)
          else:
            # FIXME is this copy needed?
            new = copy.copy(varK[var] - gain[index] * vector[var] * 1.0 * frac)
          index += 1
        tempVarNew[var] = new
  ...
  def _computeGainSequenceAk(self, paramDict, iterNum, traj):
    ...
    ak *= frac
    self.raiseADebug('step gain size for traj "{}" iternum "{}": {}'.format(
        traj, iterNum, ak))
  ...
  def _getAlgorithmState(self, traj):
    ...
    """
    state = {}
    state['lastStepSize'] = copy.deepcopy(self.counter['lastStepSize'].get(
        traj, None))
    state['gradientHistory'] = copy.deepcopy(
        self.counter['gradientHistory'].get(traj, None))
    state['recommendToGain'] = copy.deepcopy(
        self.recommendToGain.get(traj, None))
    return state
  ...
  def _getPerturbationDirection(self, perturbationIndex, traj):
    ...
    if perturbationIndex == self.perturbationIndices[0]:
      ...
      # in order to perform the de-noising we keep the same perturbation direction and we repeat the evaluation
      # multiple times
