...
class SPSA(GradientBasedOptimizer):
  ...
  def getInputSpecification(cls):
    ...
    param.addSub(
        InputData.parameterInputFactory('gamma', contentType=InputData.FloatType, strictMode=True))
    param.addSub(
        InputData.parameterInputFactory('c', contentType=InputData.FloatType, strictMode=True))
    param.addSub(
        InputData.parameterInputFactory('a', contentType=InputData.FloatType, strictMode=True))
    param.addSub(
        InputData.parameterInputFactory('alpha', contentType=InputData.FloatType, strictMode=True))
    param.addSub(
        InputData.parameterInputFactory('A', contentType=InputData.FloatType, strictMode=True))
  ...
  def localInputAndChecks(self, xmlNode):
    ...
    if stochDist == 'Bernoulli':
      ...
      self.raiseAnError(IOError,
                        self.paramDict['stochasticEngine'] + 'is currently not supported for SPSA')
  ...
  def localStillReady(self, ready, convergence=False):
    ...
    for traj in self.optTraj:
      self.raiseADebug('   Traj: "{:^n}": Process: "{:^30.30}", Reason: "{:^30.30}"'.format(
          traj,
    ...
    for _ in range(len(self.optTrajLive)):
      ...
      # still in the process of submitting new opt point evaluations (for stochastic denoising)
        ...
        if reason == 'just started':
          ...
          self.raiseAnError(RuntimeError, 'unexpected reason for submitting new opt points:',
                            reason)
      ...
      # all gradient evaluation points submitted, but waiting for them to be collected
        ...
        # otherwise, we're not ready to sample yet
          ...
          self.raiseADebug(
              '    Traj "{}": Waiting on collection of gradient evaluation points.'.format(traj))
          continue

      # all opt point evaluations have been submitted, but waiting for them to be collected
      elif process == 'collecting new opt points':
        self.raiseADebug(
            '    Traj "{}": Waiting on collection of new optimization point.'.format(traj))
        continue
  ...
  def _checkBoundariesAndModify(self, upperBound, lowerBound, varRange, currentValue, pertUp,
                                pertLow):
    """
    ...
    if isinstance(currentValue, np.ndarray):
      convertedValue = np.asarray([
          self._checkBoundariesAndModify(upperBound, lowerBound, varRange, value, pertUp, pertLow)
    ...
    if len(activeViolations['internal']) > 0:
      ...
      for var in gradient.keys():
        gradient[var] = gradient[
            var] / newNormWithoutComponents if newNormWithoutComponents != 0.0 else gradient[var]
      ...
      for varID, var in enumerate(self.getOptVars(traj=traj)):
        ...
        if varSize == 1:
          ...
          for i in range(varSize):
            pendVector[var][i] = self.stochasticEngineForConstraintHandling.rvs() if (
      ...
      for var in self.getOptVars(traj=traj):
        ...
        for i in range(numSamples):
          if numSamples > 1:
            new[i] = copy.copy(
                varK[var][i] -
                gain[index] * vector[var][i] * 1.0 * frac)  # FIXME is this copy needed?
          else:
            new = copy.copy(
                varK[var] - gain[index] * vector[var] * 1.0 * frac)  # FIXME is this copy needed?
          index += 1
        tempVarNew[var] = new
