...
class GridBase(metaclass_insert(abc.ABCMeta, BaseType)):
    ...
    """
      ...
      @ In, initDictionary, dict, optional, dictionary of input arguments needed to create a Grid:
        ...
        {volumetriRatio:float or stepLength:dict}, required, p.u. volumetric ratio
                                        of the grid or dictionary of stepLengths ({'varName:list,etc'}
  ...
  def returnParameter(self, parameterName):
    ...
    if parameterName not in self.gridContainer.keys():
      self.raiseAnError(Exception,
                        'parameter ' + parameterName + 'unknown among ones in GridEntity class.')
  ...
  def addCustomParameter(self, parameterName, value):
    ...
    if parameterName in self.gridContainer.keys():
      self.raiseAnError(Exception, 'parameter ' + parameterName + 'already present in GridEntity!')
...
class GridEntity(GridBase):
  ...
  def __init__(self, messageHandler):
    ...
    self.gridContainer[
        'gridCoorShape'] = None  # shape of the matrix containing all coordinate of all points in the grid
    self.gridContainer[
        'gridCoord'] = None  # the matrix containing all coordinate of all points in the grid
    self.gridContainer['nVar'] = 0  # this is the number of grid dimensions
    self.gridContainer[
        'transformationMethods'] = None  # Dictionary of methods to transform the coordinate from 0-1 values to something else. These methods are pointed and passed into the initialize method. {varName:method}
    ...
    self.gridContainer[
        'initDictionary'] = None  # dictionary of initialization parameters passed in the initialize method
    ...
    self.gridInitDict = {
    }  # dictionary with initialization grid info from _readMoreXML. If None, the "initialize" method will look for all the information in the in Dictionary
  ...
  def _fillGrid(self, child):
    ...
    if constrType == 'custom':
      ...
      self.raiseAnError(IOError, 'construction type unknown! Got: ' + str(constrType))
    return gridStruct
  ...
  def initialize(self, initDictionary=None):
    ...
    self.constructTensor = bool(
        initDict['constructTensor']) if 'constructTensor' in initDict.keys() else False
    ...
    self.nVar = len(self.gridInitDict[
        "dimensionNames"]) if "dimensionNames" in self.gridInitDict.keys() else len(
            initDict["dimensionNames"])
    self.gridContainer['dimensionNames'] = self.gridInitDict[
        "dimensionNames"] if "dimensionNames" in self.gridInitDict.keys() else initDict[
            "dimensionNames"]
    upperkeys = list(self.gridInitDict["upperBounds"].keys() if "upperBounds" in
                     self.gridInitDict.keys() else initDict["upperBounds"].keys())
    lowerkeys = list(self.gridInitDict["lowerBounds"].keys() if "lowerBounds" in
                     self.gridInitDict.keys() else initDict["lowerBounds"].keys())
    ...
    self.gridContainer['bounds']["upperBounds"] = self.gridInitDict[
        "upperBounds"] if "upperBounds" in self.gridInitDict.keys() else initDict["upperBounds"]
    self.gridContainer['bounds']["lowerBounds"] = self.gridInitDict[
        "lowerBounds"] if "lowerBounds" in self.gridInitDict.keys() else initDict["lowerBounds"]
    ...
    if "volumetricRatio" in initDict.keys() and "stepLength" in list(initDict.keys()) + readKeys:
      self.raiseAWarning(
    ...
    if "volumetricRatio" in initDict.keys():
      ...
      for varId in range(len(self.gridContainer['dimensionNames'])):
        stepLength.append([
            ratioRelative *
            (self.gridContainer['bounds']["upperBounds"][self.gridContainer['dimensionNames']
                                                         [varId]] - self.gridContainer['bounds']
             ["lowerBounds"][self.gridContainer['dimensionNames'][varId]])
        ])
    else:
      ...
      for dimName in self.gridContainer['dimensionNames']:
        stepLength.append(initDict["stepLength"][dimName] if "stepLength" not in readKeys else
    ...
    for varId, varName in enumerate(self.gridContainer['dimensionNames']):
      ...
      if len(stepLength[varId]) == 1:
        ...
        if self.volumetricRatio != None:
          ...
          self.gridContainer['gridVectors'][varName] = np.arange(lowerBound, upperBound,
                                                                 stepLength[varId][-1])
        else:
          ...
          self.gridContainer['gridVectors'][varName] = np.concatenate((np.arange(
              lowerBound, upperBound - myEps, stepSize), np.atleast_1d(upperBound)))
      else:
        # custom grid
      ...
      if checkBounds and compare(self.gridContainer['bounds']["lowerBounds"][varName],
                                 self.gridContainer['bounds']["upperBounds"][varName]):
        self.raiseAnError(
            IOError, "the lowerBound and upperBound for dimension named " + varName +
            " are the same!. lowerBound = " +
            str(self.gridContainer['bounds']["lowerBounds"][varName]) + " and upperBound = " + str(
                self.gridContainer['bounds']["upperBounds"][varName]))
      lowerBound = self.gridContainer['bounds']["lowerBounds"][
          varName] if not excludeBounds['lowerBounds'] else self.gridContainer['bounds']["lowerBounds"][varName] + stepLength[varId][-1]
      upperBound = self.gridContainer['bounds']["upperBounds"][
          varName] if not excludeBounds['upperBounds'] else self.gridContainer['bounds']["upperBounds"][varName] - stepLength[varId][-1]
      if not compare(max(self.gridContainer['gridVectors'][varName]),
    ...
    self.gridContainer['gridLength'] = np.int(np.prod(np.asarray(
        pointByVar, dtype=np.float64)))  # total number of point on the grid
    self.gridContainer['gridCoorShape'] = tuple(
        pointByVar +
        [self.nVar])  # shape of the matrix containing all coordinate of all points in the grid
    if self.constructTensor:
      self.gridContainer['gridCoord'] = np.zeros(self.gridContainer[
          'gridCoorShape'])  # the matrix containing all coordinate of all points in the grid
    ...
    self.gridIterator = ravenArrayIterator(
        arrayIn=self.gridContainer['gridCoord']) if self.constructTensor else ravenArrayIterator(
    ...
    if self.constructTensor or computeCells:
      while not self.gridIterator.finished:
        if self.constructTensor:
          ...
          self.gridContainer['gridCoord'][self.gridIterator.multiIndex] = self.gridContainer[
              'gridVectors'][dimName][valuePosition]
        if computeCells:
          if all(np.greater(pp, list(self.gridIterator.multiIndex[:-1]))) and list(
  ...
  def returnGridAsArrayOfCoordinates(self):
    ...
    """
    returnCoordinates = self.__returnCoordinatesReshaped((self.gridContainer['gridLength'],
                                                          self.nVar))
  ...
  def returnShiftedCoordinate(self, coordinates, shiftingSteps):
    ...
    for varName in self.gridContainer['dimensionNames']:
      ...
      elif varName in coordinates.keys() and not varName in shiftingSteps.keys():
  ...
  def returnPointAndAdvanceIterator(self, returnDict=False, recastMethods={}):
    ...
    if not self.gridIterator.finished:
      coordinates = self.returnCoordinateFromIndex(self.gridIterator.multiIndex, returnDict,
                                                   recastMethods)
      ...
      if multiDimIndex[cnt] < 0:
        ...
        if key in recastMethods.keys():
          ...
          coordinates[vvkey] = recastMethods[key][0](
              self.gridContainer['gridVectors'][key][multiDimIndex[cnt]], *args)
        else:
...
class MultiGridEntity(GridBase):
  ...
  def _readMoreXml(self, xmlNode, dimensionTags=None, messageHandler=None, dimTagsPrefix=None):
    """
      ...
      @ Out, None
    """
    self.grid.getrootnode().get("grid")._readMoreXml(xmlNode, dimensionTags, messageHandler,
                                                     dimTagsPrefix)
  ...
  def updateSubGrid(self, parentNode, refineDict):
    ...
    if parentNode == -1:
      self.raiseAnError(Exception, "parent Node named " + parentNode + " has not been found!")
  ...
  def refineGrid(self, refineDict):
    ...
    if "refiningNumSteps" not in refineDict.keys() and "volumetricRatio" not in refineDict.keys():
      self.raiseAnError(
          IOError, "the refining Number of steps or the volumetricRatio has not been provided!!!")
    ...
    for node in self.grid.iter():
      ...
      if len(foundCells) > 0:
        ...
        for idcnt, fcellId in enumerate(foundCells):
          ...
          lowerBounds, upperBounds = dict.fromkeys(
              parentGrid.returnParameter('dimensionNames'), sys.float_info.max), dict.fromkeys(
                  parentGrid.returnParameter('dimensionNames'), -sys.float_info.max)
          for vertex in verteces:
            ...
            for key in lowerBounds.keys():
              lowerBounds[key], upperBounds[key] = min(lowerBounds[key], coordinates[key]), max(
          ...
          if "volumetricRatio" in refineDict.keys():
            ...
            for key in lowerBounds.keys():
              initDict["stepLength"][key] = [
                  (upperBounds[key] - lowerBounds[key]) / float(refineDict["refiningNumSteps"])
              ]
          initDict["startingCellId"] = maxCellId + 1
          newGrid.initialize(initDict)
          maxCellId = max(newGrid.returnParameter('cellIDs').keys())
          refinedNode = self.__createNewNode(node.name + "_cell:" + str(fcellId), {
              "grid": newGrid,
      ...
      if foundAll:
        break
    if not foundAll:
      self.raiseAnError(Exception, "the following cell IDs have not been found: " + ' '.join(
          [cellId for cellId, value in didWeFoundCells.items() if value == True]))
      ...
      if nodeName == None:
        for node in self.grid.iter():
          fullReshapedCoordinates = np.concatenate(
              (fullReshapedCoordinates, node.get('grid').returnGridAsArrayOfCoordinates()))
      else:
  ...
  def returnPointAndAdvanceIterator(self, returnDict=False, recastMethods={}):
    ...
    for node in startingNode.iter():
      ...
      if not subGrid.gridIterator.finished:
        ...
        break
      self.multiGridIterator[0], self.multiGridIterator[1] = node.get("level"), node.get(
  ...
  def returnParameter(self, parameterName, nodeName=None):
    ...
    if nodeName != None and nodeName != "*":
      ...
      if node == None:
        self.raiseAnError(
            Exception,
            'node  ' + nodeName + ' has not been found in the MultiGrid hierarchal tree!')
      return node.get("grid").returnParameter(parameterName)
    elif nodeName == "*":
      if parameterName not in self.gridContainer.keys():
        self.raiseAnError(
            Exception,
            'parameter ' + parameterName + 'unknown among ones in MultiGridEntity class.')
  ...
  def addCustomParameter(self, parameterName, value, nodeName=None):
    ...
    if nodeName != None:
      ...
      if parameterName in self.gridContainer.keys():
        self.raiseAnError(Exception,
                          'parameter ' + parameterName + 'already present in MultiGridEntity!')
    else:
      for node in self.grid.iter():
