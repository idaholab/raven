...
class AdaptiveDET(DynamicEventTree, LimitSurfaceSearch):
  ...
  def _constructEndInfoFromBranch(self,model, myInput, info, cdfValues):
    ...
    subGroup = ETS.HierarchalNode(self.messageHandler,rname)
  ...
  def localGenerateInput(self,model,myInput):
    ...
    if self.startAdaptive == True and self.adaptiveReady == True:
      ...
      if closestBranch:
        ...
        elm = ETS.HierarchalNode(self.messageHandler,self.name + '_' + str(len(self.TreeInfo.keys())+1))
        ...
        self.TreeInfo[self.name + '_' + str(len(self.TreeInfo.keys())+1)] = ETS.HierarchalTree(self.messageHandler,elm)
