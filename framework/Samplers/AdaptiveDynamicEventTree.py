  ...
  def _checkClosestBranch(self):
    ...
    for key,value in self.values.items():
      ...
      if key not in self.epistemicVariables.keys():
        ...
        try:
          index = utils.first(np.asarray(self.branchProbabilities[key]) <= cdfValues[key]).nonzero()[-1]
  ...
  def localGenerateInput(self,model,myInput):
    ...
    if self.startAdaptive == True and self.adaptiveReady == True:
      ...
      for key,value in cdfValues.items():
        try:
          ind = utils.first(np.asarray(self.branchProbabilities[key]) <= value).nonzero()[-1]
      ...
      if closestBranch:
        ...
        branchedLevel = {}
        for key,value in cdfValues.items():
          branchedLevel[key] = utils.first(np.asarray(self.branchProbabilities[key]) == value).nonzero()[-1]
