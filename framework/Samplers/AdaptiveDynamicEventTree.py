...
class AdaptiveDynamicEventTree(DynamicEventTree, LimitSurfaceSearch):
  ...
  def getInputSpecification(cls):
    ...
    """
    inputSpecification = super(AdaptiveDynamicEventTree,
  ...
  def __init__(self):
    ...
    # init Adaptive
    LimitSurfaceSearch.__init__(self)
    ...
    # Flag to trigger the begin of the adaptive limit surface search
    self.startAdaptive = False
    # Flag to store the response of the LimitSurfaceSearch.localStillReady method
    self.adaptiveReady = False
    # List containing the points that have been already investigated
    self.investigatedPoints = []
    # Counter of the completed histories
    self.completedHistCnt = 1
    self.hybridDETstrategy = None  # Integer flag to turn the hybrid strategy on:
    # None -> No hybrid approach,
    # 1 -> the epistemic variables are going to be part of the limit surface
    # search
    # 2 -> the epistemic variables are going to be treated by a normal hybrid
    # DET approach and the LimitSurface search
    ...
    # name of the root tree used in self.hybridDETstrategy=2 to check which Tree needs to be used for the current LS
    # search
    self.actualHybridTree = ''
    # sorted list of histories
    self.sortedListOfHists = []
  ...
  def _localWhatDoINeed(self):
    ...
    needDict = dict(
        LimitSurfaceSearch._localWhatDoINeed(self).items() +
        DynamicEventTree._localWhatDoINeed(self).items())
    return needDict
  ...
  def _checkClosestBranch(self):
    ...
    for key, value in self.values.items():
      ...
      if key not in self.epistemicVariables.keys():
        ...
        lowerCdfValues[key] = utils.find_le(self.branchProbabilities[key],
                                            cdfValues[key])[0]
    ...
    if self.hybridDETstrategy is not None:
      ...
      for tree in self.TreeInfo.values():
        epistemicVars = tree.getrootnode().get("hybridsamplerCoordinate")[0][
            'SampledVars']
    ...
    if not self.insertAdaptBPb:
      ...
      if len(candidatesBranch) > 0:
        ...
        for invPoint in candidatesBranch:
          pbth = [invPoint[self.toBeSampled[key]] for key in cdfValues.keys()]
          if all(
              i >= pbth[cnt] for cnt, i in enumerate(lowerCdfValues.values())):
    ...
    for ending in treer.iterProvidedFunction(self._checkEnded):
      # already ended branches, create training set for nearest algorithm (take coordinates <= of cdfValues) ->
      # TODO: improve efficiency
      pbth = [
          ending.get('SampledVarsPb')[key] for key in lowerCdfValues.keys()
      ]
      if all(pbth[cnt] <= i for cnt, i in enumerate(lowerCdfValues.values())):
        if nntrain is None:
          ...
          nntrain = np.concatenate(
              (nntrain, np.atleast_2d(np.array(copy.copy(pbth)))), axis=0)
    ...
    if nntrain is not None:
      ...
      valBranch = self._checkValidityOfBranch(
          neigh.kneighbors([lowerCdfValues.values()]), mapping)
      if self.hybridDETstrategy is not None:
  ...
  def _checkValidityOfBranch(self, branchSet, mapping):
    ...
    for closestBranch in idOfBranches:
      if not mapping[closestBranch + 1].get(
          'completedHistory') and not mapping[closestBranch
                                              + 1].get('happenedEvent'):
  ...
  def _constructEndInfoFromBranch(self, model, myInput, info, cdfValues):
    """
      ...
      @ In, info, dict, dictionary of information at the end of a branch (information collected by the method
             _retrieveBranchInfo)
    ...
    # Get Parent node name => the branch name is creating appending to this name a comma and self.branchCountOnLevel
    # counter
    ...
    self.raiseADebug(
        'cond pb = ' + str(info['parentNode'].get('conditionalPbr')))
    ...
    # The probability Thresholds are stored here in the cdfValues dictionary... We are sure that they are whitin the
    # ones defined in the grid
    # check is not needed
    ...
    for varname in self.standardDETvariables:
      self.inputInfo['SampledVars'][varname] = self.distDict[varname].ppf(
          cdfValues[varname])
  ...
  def localStillReady(self, ready):
    ...
    if len(self.RunQueue['queue']) != 0:
      ...
      detReady = False
    # since the RunQueue is empty, let's check if there are still branches running => if not => start the adaptive
    # search
    ...
    if self.startAdaptive:
      ...
      if numCompletedHistories > self.completedHistCnt:
        lastOutDict = {
            key: endingData[key].values
            for key in endingData.keys()
        }
      if numCompletedHistories > self.completedHistCnt:
        ...
        self.raiseAMessage(
            "Completed full histories are " + str(self.completedHistCnt))
      else:
        ready = False
  ...
  def localGenerateInput(self, model, myInput):
    ...
    if self.startAdaptive == True and self.adaptiveReady == True:
      ...
      if closestBranch:
        ...
        branchedLevel = {}
        for key, value in cdfValues.items():
          branchedLevel[key] = utils.index(self.branchProbabilities[key],
                                           value)
        ...
        if self.hybridDETstrategy is not None and not self.foundEpistemicTree:
          ...
          hybridSampled = copy.deepcopy(self.TreeInfo.values()[
              0].getrootnode().get('hybridsamplerCoordinate'))
          for hybridStrategy in hybridSampled:
            for key in self.epistemicVariables.keys():
              if key in hybridStrategy['SampledVars'].keys():
                self.raiseADebug("epistemic var " + str(key) + " value = " +
                                 str(self.values[key]))
                hybridStrategy['SampledVars'][key] = copy.copy(
                    self.values[key])
                hybridStrategy['SampledVarsPb'][key] = self.distDict[key].pdf(
                    self.values[key])
        ...
        # Here it is stored all the info regarding the DET => we create the info for all the branchings and we store
        # them
        self.TreeInfo[
            self.name + '_' +
            str(len(self.TreeInfo.keys()) + 1)] = ETS.HierarchicalTree(
                self.messageHandler, elm)
        self._createRunningQueueBeginOne(
            self.TreeInfo[self.name + '_' + str(len(self.TreeInfo.keys()))],
  ...
  def localInputAndChecks(self, xmlNode, paramInput):
    ...
    if len(hybridNodes) != 0:
      ...
      for elm in hybridNodes:
        ...
        if samplType == 'LimitSurface':
          if len(hybridNodes) != 1:
            self.raiseAnError(
                IOError,
                'if one of the HybridSampler is of type "LimitSurface", it can not be combined with other strategies. Only one HybridSampler node can be inputted!'
            )
      ...
      if self.hybridDETstrategy == 2:
        self.raiseAnError(
            IOError,
            'The sheaf of LSs for the Adaptive Hybrid DET is not yet available. Use type "LimitSurface"!'
        )
    ...
    if 'mode' in xmlNode.attrib.keys():
      if xmlNode.attrib['mode'].lower() == 'online':
        ...
        self.raiseAnError(IOError, 'unknown mode ' + xmlNode.attrib['mode'] +
                          '. Available are "online" and "post"!')
    if 'noTransitionStrategy' in xmlNode.attrib.keys():
      if xmlNode.attrib['noTransitionStrategy'].lower() == 'mc':
        ...
        self.raiseAnError(IOError, 'unknown noTransitionStrategy ' +
                          xmlNode.attrib['noTransitionStrategy'] +
      ...
      if self.hybridDETstrategy == 1:
        ...
        for varName, dist in self.distDict.items():
          if varName.replace('<distribution>',
            ...
            # found an epistemic
            varNode = ET.Element(
                'Distribution' if varName.startswith('<distribution>') else
                'variable', {'name': varName.replace('<distribution>', '')})
            varNode.append(
                ET.fromstring(
                    "<distribution>" + dist.name.strip() + "</distribution>"))
            distDict[dist.name.strip()] = self.distDict[varName]
            varNode.append(
                ET.fromstring(
                    '<grid construction="custom" type="value">' + ' '.join([
                        str(elm)
                        for elm in gridVector.values()[0][varName.replace(
                            '<distribution>', '')]
                    ]) + '</grid>'))
  ...
  def generateInput(self, model, oldInput):
    """
      ...
      @ Out, generateInput, tuple(0,list), list containing the new inputs -in reality it is the model that returns
  ...
  def localFinalizeActualSampling(self, jobObject, model, myInput):
    """
      ...
      @ Out, None
    """
    returncode = DynamicEventTree.localFinalizeActualSampling(
        self, jobObject, model, myInput, genRunQueue=False)
