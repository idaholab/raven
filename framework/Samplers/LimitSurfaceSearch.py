...
class LimitSurfaceSearch(AdaptiveSampler):
  ...
  def __init__(self):
    ...
    )  #this is a matrix that for each point of the testing grid tracks the inverse of the persistence of the limit surface position
  ...
  def localInitialize(self, solutionExport=None):
    ...
    self.persistenceMatrix[self.name + "LSpp"] = np.zeros(
        matrixShape
    )  #matrix that for each point of the testing grid tracks the persistence of the limit surface position
    self.oldTestMatrix[self.name + "LSpp"] = np.zeros(
        matrixShape)  #swap matrix fro convergence test
  ...
  def localStillReady(self, ready):  #,lastOutput=None
    ...
    if len(testMatrixDict) > 0:
      testError += np.sum(
          np.abs(np.subtract(testMatrixDict,
                             oldTestMatrixDict)))  # compute the error
