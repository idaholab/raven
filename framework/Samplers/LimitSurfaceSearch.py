  ...
  def localInputAndChecks(self,xmlNode):
    ...
    if 'forceIteration' in convergenceNode.attrib.keys():
      ...
      if   convergenceNode.attrib['forceIteration']=='True':
        ...
        self.raiseAnError(RuntimeError,'Reading the convergence setting for the adaptive sampler '+self.name+' the forceIteration keyword had an unknown value
    #assembler node: Hidden from User
    # set subgrid
    if self.subGridTol == None:
      self.subGridTol = self.tolerance
    if self.subGridTol > self.tolerance:
      self.raiseAnError(IOError,'The sub grid tolerance '+str(self.subGridTol)+' must be smaller than the tolerance
    if len(attribList)>0:
      self.raiseAnError(IOError,'There are unknown keywords in the convergence specifications
