...
class AdaptiveSparseGrid(SparseGridCollocation, AdaptiveSampler):
  ...
  def __init__(self):
    ...
    #assembler objects
    # TimePointSet of solutions -> assembled
    self.solns = None
    # eventual final ROM object
    self.ROM = None
    #input parameters
    # max size of polynomials to allow
    self.maxPolyOrder = 0
    # number of forced iterations, default 2
    self.persistence = 0
    # convergence criterion to use
    self.convType = None
    # file to print log to, optional
    self.logFile = None
    #convergence/training tools
    self.expImpact = {
    }  #dict of potential included polynomials and their estimated impacts, [target][index]
    # dict of included polynomials and their current impact, [target][index] = impact
    self.actImpact = {}
    # current sparse grid
    self.sparseGrid = None
    # previously-accepted sparse grid
    self.oldSG = None
    # estimate of percent of moment calculated so far
    self.error = 0
    # when printing the log, tracks the number of prints
    self.logCounter = 0
    #convergence study
    ...
    self.studyPoints = []
    # if true, dumps ROM to pickle at each step
    self.studyPickle = False
    #solution storage
    # queue of points to submit
    self.neededPoints = []
    ...
    # number of runs still running when convergence complete
    self.unfinished = 0
    # flag for whether jobHandler has complete batch or not
    self.batchDone = True
    # flipped when converged
    self.done = False
    # used to track and debug intended size of solutions
  ...
  def localInitialize(self):
    ...
    # save it to re-set it after calling generateQuadsAndPolys
    mpo = self.maxPolyOrder
    # lives in GaussPolynomialRom object
    ...
    for pt in self.indexSet.active:
      ...
      for t in self.targets:
        # dummy, just to help algorithm be consistent
        self.expImpact[t][pt] = 1.0
  ...
  def _addNewPoints(self, SG=None):
    ...
    if SG is None:
      SG = self.sparseGrid
    for pt in SG.points()[:]:
      # sets won't store redundancies
      self.pointsNeededToMakeROM.add(pt)
      #if pt isn't already in needed, and it hasn't already been solved, add it to the queue
      if pt not in self.neededPoints and self.solns.realization(
          matchDict=self._tupleToDict(pt))[1] is None:
  ...
  def _estimateImpact(self, idx):
    ...
    for t in self.targets:
      ...
      self.expImpact[t][idx] = np.prod(
          np.power(np.array(predecessors[t]), 1.0 / len(predecessors[t])))
  ...
  def _integrateFunction(self, sg, r, i):
    ...
    for n in range(len(sg)):
      ...
      if inExisting is None:
        self.raiseAnError(RuntimeError, 'Trying to integrate with point', pt,
                          'but it is not in the solutions!')
  ...
  def _makeARom(self, grid, inset):
    ...
    # preserves interpolation requests via deepcopy
  ...
  def _updateQoI(self):
    """
      Updates Reduced Order Models (ROMs) for Quantities of Interest (QoIs), as well as impact parameters and
      estimated error.
      @ In, None
      @ Out, None
    """
