...
class Sampler(utils.metaclass_insert(abc.ABCMeta, BaseType), Assembler):
  ...
  def getInputSpecification(cls):
    ...
    variablesTransformationInput = InputData.parameterInputFactory(
        "variablesTransformation")
    variablesTransformationInput.addParam('distribution', InputData.StringType)

    variablesTransformationInput.addSub(
        InputData.parameterInputFactory(
            "latentVariables", contentType=InputData.StringListType))
    variablesTransformationInput.addSub(
        InputData.parameterInputFactory(
            "manifestVariables", contentType=InputData.StringListType))
    variablesTransformationInput.addSub(
        InputData.parameterInputFactory(
            "manifestVariablesIndex", contentType=InputData.StringListType))
    variablesTransformationInput.addSub(
        InputData.parameterInputFactory(
            "method", contentType=InputData.StringType))

    inputSpecification.addSub(variablesTransformationInput)

    constantInput = InputData.parameterInputFactory(
        "constant", contentType=InputData.InterpretedListType)
    ...
    restartToleranceInput = InputData.parameterInputFactory(
        "restartTolerance", contentType=InputData.FloatType)
    inputSpecification.addSub(restartToleranceInput)

    restartInput = InputData.parameterInputFactory(
        "Restart", contentType=InputData.StringType)
  ...
  def __init__(self):
    ...
    # is this sampler able to handle failed runs?
    self.ableToHandelFailedRuns = False
    # Counter of the samples performed (better the input generated!!!). It is reset by calling the function
    # self.initialize
    self.counter = 0
    ...
    # Contains the instance of the distribution to be used, it is created every time the sampler is initialized.
    # keys are the variable names
    self.distDict = {}
    # Contains the instance of the function to be used, it is created every time the sampler is initialized. keys
    # are the variable names
    self.funcDict = {}
    # for each variable the current value {'var name':value}
    self.values = {}
    self.variableShapes = {
    }  # stores the dimensionality of each variable by name, as tuple e.g. (2,3) for [[#,#,#],[#,#,#]]
    # depending on the sampler several different type of keywarded information could be present only one is
    # mandatory, see below
    self.inputInfo = {}
    # if not provided the seed is randomly generated at the istanciation of the sampler, the step can override the
    # seed by sending in another seed
    self.initSeed = None
    # this is the location where to get the values of the sampled variables
    ...
    # self.inputInfo['PointProbability'] = None
    # this is the location where the point wise probability is stored (probability associated to a sampled point)
    ...
    # In this dictionary
    self.constants = {}
    # Logical flag. True if every newer evaluation is performed after a new reseeding
    self.reseedAtEachIteration = False
    # FIXME flag
    self.FIXME = False
    self.printTag = self.type  # prefix for all prints (sampler type)
    # presampled points to restart from
    self.restartData = None
    # strictness with which to find matches in the restart data
    self.restartTolerance = 1e-15
    ...
    ######
    # for each variable 'varName' , the following informations are included: 'varName': {'dim': 1, 'reducedDim':
    # 1,'totDim': 2, 'name': 'distName'} ; dim = dimension of the variable; reducedDim = dimension of the variable
    # in the transformed space; totDim = total dimensionality of its associated distribution
    self.variables2distributionsMapping = {}
    # for each variable 'distName' , the following informations are included: 'distName': [{'var1': 1}, {'var2':
    # 2}]} where for each var it is indicated the var dimension
    ...
    # for each variable 'modelName', the following informations are included: {'modelName':
    # {latentVariables:[latentVar1, latentVar2, ...], manifestVariables:[manifestVar1,manifestVar2,...]}}
  ...
  def _readMoreXMLbase(self, xmlNode):
    ...
    for child in paramInput.subparts:
      ...
      if child.getName() == 'Distribution':
        ...
        for childChild in child.subparts:
          if childChild.getName() == 'distribution':
            ...
            if not foundDistOrFunc:
              ...
              self.raiseAnError(
                  IOError,
                  'A sampled variable cannot have both a distribution and a function, or more than one of either!'
              )
        ...
        if not foundDistOrFunc:
          self.raiseAnError(
              IOError, 'Sampled variable', varName,
    ...
    # Creation of the self.distributions2variablesMapping dictionary: {'distName': [{'variable_name1': dim1},
    # {'variable_name2': dim2}]}
    ...
    for distName in self.distributions2variablesMapping.keys():
      ...
      if sum(set(positionList)) > 1 and len(positionList) != len(
          set(positionList)):
        dups = set(
            str(var) for var in positionList if positionList.count(var) > 1)
        self.raiseAnError(
            IOError,
            'Each of the following dimensions are assigned to multiple variables in Samplers: "{}"'.
            format(', '.join(dups)), ' associated to ND distribution ',
            distName, '. This is currently not allowed!')
    ...
    for key in self.variables2distributionsMapping.keys():
      ...
      reducedDim = self.distributions2variablesIndexList[distName].index(
          dim) + 1
      # the dimension of variable in the transformed space
      self.variables2distributionsMapping[key]['reducedDim'] = reducedDim
    ...
    if self.variablesTransformationDict:
      for dist, varsDict in self.variablesTransformationDict.items():
        ...
        if len(set(listLatentElement)) != len(listLatentElement):
          dups = set(
              var for var in listLatentElement
        ...
        if len(set(varsDict['manifestVariablesIndex'])) != len(
          ...
          dups = set(var + 1 for var in varsDict['manifestVariablesIndex']
        ...
        for var in listElement:
          # reset the totDim to reflect the totDim of original input space
          self.variables2distributionsMapping[utils.first(
              var.keys())]['totDim'] = maxDim
        tempListElement = {
            k.strip(): v
        ...
        for var in listLatentElement:
          if var not in set(tempListElement.keys()):
            self.raiseAnError(
                IOError, 'The variable listed in latentVariables ' + var +
                ' is not listed in the given distribution: ' + dist)
  ...
  def getInitParams(self):
    ...
    for variable in self.toBeSampled.items():
      paramDict[
          "sampled variable: "
          + variable[0]] = 'is sampled using the distribution ' + variable[1]
  ...
  def initialize(self, externalSeeding=None, solutionExport=None):
    """
      ...
      @ In, solutionExport, DataObject, optional, in goal oriented sampling (a.k.a. adaptive sampling this is where
             the space/point satisfying the constrains)
      @ Out, None
    """
    if self.initSeed == None:
      self.initSeed = randomUtils.randomIntegers(0, 2**31, self)
    ...
    if not externalSeeding:
      # use the sampler initialization seed
      randomUtils.randomSeed(self.initSeed)
      self.auxcnt = self.initSeed
    elif externalSeeding == 'continue':
      # in this case the random sequence needs to be preserved
      pass
    else:
      # the external seeding is used
      randomUtils.randomSeed(externalSeeding)
    ...
    if 'Restart' in self.assemblerDict.keys():
      ...
      if not need.issubset(set(self.restartData.getVars())):
        ...
        self.raiseAnError(
            KeyError,
            'Restart data object "{}" is missing the following variables: "{}". No restart can be performed.'.
            format(self.restartData.name, ', '.join(missing)))
      else:
  ...
  def localInputAndChecks(self, xmlNode, paramInput):
    """
      Local method. Place here the additional reading, remember to add initial parameters in the method
      localGetInitParams
      @ In, xmlNode, xml.etree.ElementTree.Element, Xml element node
  ...
  def readSamplerInit(self, xmlNode):
    """
      ...
      This method has been moved from the base sampler class since the samplerInit block is needed only for the MC
      and stratified (LHS) samplers
      @ In, xmlNode, xml.etree.ElementTree.Element, Xml element node
      @ Out, None
    """
            ...
            for childChildChild in childChild.subparts:
              ...
              for childChildChildChild in childChildChild.subparts:
                if childChildChildChild.getName() == 'initialGridDisc':
                  NDdistData[childChildChildChild.getName()] = int(
                      childChildChildChild.value)
                elif childChildChildChild.getName() == 'tolerance':
                  NDdistData[childChildChildChild.getName()] = float(
                      childChildChildChild.value)
                else:
                  self.raiseAnError(
                      IOError,
                      'Unknown tag ' + childChildChildChild.getName() +
                      ' .Available are: initialGridDisc and tolerance!')
              self.NDSamplingParams[childChildChild.parameterValues[
                  'name']] = NDdistData
  ...
  def amIreadyToProvideAnInput(self):  #inLastOutput=None):
    """
      This is a method that should be call from any user of the sampler before requiring the generation of a new
      sample.
      This method act as a "traffic light" for generating a new input.
      Reason for not being ready could be for example: exceeding number of samples, waiting for other simulation for
      providing more information etc. etc.
      @ In, None
  ...
  def _checkRestartForEvaluation(self):
    ...
    if self.restartData is not None:
      index, inExisting = self.restartData.realization(
          matchDict=self.values, tol=self.restartTolerance, unpackXArray=True)
    else:
      index = None
      inExisting = None
  ...
  def _constantVariables(self):
    ...
    if len(self.constants) > 0:
      ...
      self.inputInfo.update(
          dict.fromkeys(
              ['ProbabilityWeight-' + key for key in self.constants.keys()],
              1.0))
  ...
  def _incrementCounter(self):
    ...
    # FIXME, the following condition check is make sure that the require info is only printed once when dump
    # metadata to xml, this should be removed in the future when we have a better way to dump the metadata
    if self.counter > 1:
  ...
  def _reassignSampledVarsPbToFullyCorrVars(self):
    ...
    """
    fullyCorrVars = {
        s: self.inputInfo['SampledVarsPb'].pop(s)
        for s in self.inputInfo['SampledVarsPb'].keys() if "," in s
    }
  ...
  def _reassignPbWeightToCorrelatedVars(self):
    ...
    for varName, varInfo in self.variables2distributionsMapping.items():
      ...
      if varInfo['totDim'] > 1:
        ...
        self.inputInfo['ProbabilityWeight-' + varName] = self.inputInfo[
            'ProbabilityWeight-' + distName]
      if "," in varName:
        for subVarName in varName.split(","):
          self.inputInfo['ProbabilityWeight-'
                         + subVarName.strip()] = self.inputInfo[
                             'ProbabilityWeight-' + varName]
  ...
  def generateInput(self, model, oldInput):
    """
      ...
      @ Out, generateInput, tuple(0,list), list contains the new inputs -in reality it is the model that returns
             this; the Sampler generates the value to be placed in the input of the model.
    ...
    # DO NOT format here, let that happen when a realization is made in collectOutput for each Model. Sampler
    ...
    #otherwise, return the restart point
      ...
      rlz['inputs'] = dict((var, np.atleast_1d(inExisting[var]))
                           for var in self.restartData.getVars('input'))
      rlz['outputs'] = dict((var, np.atleast_1d(inExisting[var]))
                            for var in self.restartData.getVars('output') +
                            self.restartData.getVars('indexes'))
      # TODO need deepcopy only because inputInfo is on self
      rlz['metadata'] = copy.deepcopy(self.inputInfo)
      return 1, rlz
  ...
  def generateInputBatch(self, myInput, model, batchSize, projector=None):
    """
      ...
      @ In, projector, object, optional, used for adaptive sampling to provide the projection of the solution on the
             success metric
  ...
  def pcaTransform(self, varsDict, dist):
    """
      ...
      both the latent variables and the model input variables will be stored in the dict:
      self.inputInfo['SampledVars']
      @ In, varsDict, dict, dictionary contains latent and manifest variables
             {'latentVariables':[latentVar1,latentVar2,...], 'manifestVariables':[var1,var2,...]}
