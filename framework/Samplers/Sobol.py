...
class Sobol(SparseGridCollocation):
  ...
  def __init__(self):
    ...
    # S, the order of the HDMR expansion (1,2,3), queried from the sobol ROM
    self.sobolOrder = None
    # the type of index set to use, queried from the sobol ROM
    ...
    self.importanceDict = {}
    # reference (mean) values for distributions, by var
    self.references = {}
    # pointer to output dataObjects object
    self.solns = None
    # pointer to sobol ROM
    self.ROM = None
  ...
  def _localGenerateAssembler(self, initDict):
    ...
    for dist in self.dists.values():
      if isinstance(dist, Distributions.NDimensionalDistributions):
        self.raiseAnError(
            IOError,
            'ND Distributions containing the variables in the original input space are  not supported for this sampler!'
        )
  ...
  def localInitialize(self):
    ...
    for combo in needCombos:
      ...
      initDict = {
          ...
          # polys, quads per input
          'Interpolation': SVL.itpDict,
          # input variables
          'Features': ','.join(combo),
          # set below, per-case basis
          'Target': ','.join(SVL.target)
      }
    ...
    for combo, rom in self.ROMs.items():
      # just for each combo
      # they all should have the same sparseGrid
      SG = rom.sparseGrid
      SG._remap(combo)
  ...
  def localGenerateInput(self, model, myInput):
    ...
    # weight has no meaning for sobol
    self.inputInfo['ProbabilityWeight'] = np.atleast_1d(1.0)
