...
class DynamicEventTree(Grid):
  ...
  def __readBranchInfo(self,outBase=None,currentWorkingDir=None):
    ...
    for node in root:
      ...
      break
    # remove the file
    if self.removeXmlBranchInfo: os.remove(filename)
    branchPresent = True
    return branchPresent
  ...
  def _createRunningQueueBranch(self,model,myInput,forceEvent=False):
    ...
    for _ in range(nBranches):
      ...
      subGroup = ETS.HierarchalNode(self.messageHandler,rname.encode())
...
#             errorFound = True
  ...
  def localInitialize(self):
    ...
    for precSample in range(hybridNumber):
      elm = ETS.HierarchalNode(self.messageHandler,self.name + '_' + str(precSample+1))
      ...
      self.TreeInfo[self.name + '_' + str(precSample+1)] = ETS.HierarchalTree(self.messageHandler,elm)
