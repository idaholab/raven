...
class DynamicEventTree(Grid):
  ...
  def __init__(self):
    ...
    self.branchValues = {}
    # List of Dictionaries of the last probability bin level (position in the array) reached for each distribution
    # ('distName':IntegerValue)
    # This container is a working dictionary. The branchedLevels are stored in the xml tree "self.TreeInfo" since
    # they must track
    ...
    # The end time step is used to construct the filename of the restart files needed for restart the new branch
    # calculations)
    self.actualEndTs = 0
    # Xml tree object. It stored all the info regarding the DET. It is in continue evolution during a DET calculation
    self.TreeInfo = None
    # List of Dictionaries. It is a working variable used to store the information needed to create branches from a
    # Parent Branch
    self.endInfo = []
    ...
    # Dictionary of variables that represent the epistemic space (hybrid det). Format =>
    # {'epistemicVarName':{'HybridTree name':value}}
  ...
  def localFinalizeActualSampling(self,
    ...
    jobWorkingDir = os.path.join(stepWorkingDir, kwargs['prefix']
    ...
    # else: endInfo['parentNode'] =
    # list(self.TreeInfo[self.rootToJob[jobObject.identifier]].getrootnode().iter(jobObject.identifier))[0]
    ...
    for key in endInfo['branchChangedParams']:
      ...
      if (len(endInfo['branchChangedParams'][key]['actualValue']) > 1):
        ...
        unchangedPb = 0.0
        try:
          # changed_pb = probability (not conditional probability yet) that the event A occurs and the final state
          # is 'alpha' """
          for pb in xrange(
  ...
  def __readBranchInfo(self, outBase=None, currentWorkingDir=None):
    """
      ...
      @ In, outBase, string, optional, it is the output root that, if present, is used to construct the file name
             the function is going to try reading.
      @ In, currentWorkingDir, string, optional, it is the current working directory. If not present, the branch
             info are going to be looked in the self.workingDir
      @ Out, branchPresent, bool, true if the info are present (a set of new branches need to be run), false if the
             actual parent calculation reached an end point
    """
    ...
    if not os.path.exists(filename):
      self.raiseADebug('branch info file ' + os.path.basename(filename) +
                       ' has not been found. => No Branching.')
      ...
      if node.tag == "Distribution_trigger":
        ...
        for child in node:
          self.actualBranchInfo[distName][child.text.strip()] = {
              'varType': child.attrib['type'].strip(),
              'actualValue': child.attrib['actual_value'].strip().split(),
  ...
  def _createRunningQueueBeginOne(self, rootTree, branchedLevel, model,
    ...
    """
      ...
      @ In, branchedLevel, dict, dictionary of the levels reached by the rootTree mapped in the internal grid
             dictionary (self.branchProbabilities)
    ...
    # for key in
    # self.branchProbabilities.keys():self.inputInfo['PbThreshold'].append(self.branchProbabilities[key][branchedLevel[key]])
    for key in self.branchProbabilities.keys():
      self.inputInfo['ValueThreshold'].append(
          self.branchValues[key][branchedLevel[key]])
    # for key in
    # self.branchProbabilities.keys():self.inputInfo['ValueThreshold'].append(self.branchValues[key][branchedLevel[key]])
    for varname in self.standardDETvariables:
      ...
      # self.inputInfo['SampledVars' ][varname] =
      # self.branchValues[self.toBeSampled[varname]][branchedLevel[self.toBeSampled[varname]]]
      ...
      # self.branchProbabilities[self.toBeSampled[varname]][branchedLevel[self.toBeSampled[varname]]]
    # constant variables
  ...
  def _createRunningQueueBranch(self, model, myInput, forceEvent=False):
    """
      ...
      @ In, model, Models object, the model that is used to explore the input space (e.g. a code, like RELAP-7)
      @ In, myInput, list, list of inputs for the Models object (passed through the Steps XML block)
      @ In, forceEvent, bool, if True the events are forced to happen (basically, the "unchanged event" is not
             created at all)
      @ Out, None
    """
    ...
    for _ in range(nBranches):
      ...
      # Get Parent node name => the branch name is creating appending to this name a comma and
      # self.branchCountOnLevel counter
      ...
      self.inputInfo = {
          'prefix': rname.encode(),
          'endTimeStep': endInfo['endTimeStep'],
  ...
  def _createRunningQueue(self, model, myInput, forceEvent=False):
    """
      Function to create and append new inputs to the queue. It uses all the containers have been updated by the
      previous functions
  ...
  def generateInput(self, model, oldInput):
    ...
    """
    # NB: if someday the DET handles restarts as other samplers do in generateInput, the return code 1 indicates the
    # result
  ...
  def _localInputAndChecksDET(self, xmlNode, paramInput):
    ...
    if 'printEndXmlSummary' in xmlNode.attrib.keys():
      self.printEndXmlSummary = xmlNode.attrib[
          'printEndXmlSummary'].lower() in utils.stringsThatMeanTrue()
    if 'removeXmlBranchInfo' in xmlNode.attrib.keys():
      self.removeXmlBranchInfo = xmlNode.attrib[
          'removeXmlBranchInfo'].lower() in utils.stringsThatMeanTrue()
    ...
    for keyk in self.axisName:
      ...
      if self.gridInfo[keyk] == 'CDF':
        ...
        if max(self.branchProbabilities[keyk]) > 1:
          errorMsgs += "One of the Thresholds for distribution " + str(
...
# if sorted(self.branchProbabilities[self.toBeSampled[keyk]],
# key=float).count(sorted(self.branchProbabilities[self.toBeSampled[keyk]], key=float)[index]) > 1:
# self.raiseAWarning("In distribution " + str(self.toBeSampled[keyk]) + " the Threshold " +
# str(sorted(self.branchProbabilities[self.toBeSampled[keyk]], key=float)[index])+" appears multiple times!!")
#               errorFound = True
      else:
...
# if sorted(self.branchValues[self.toBeSampled[keyk]],
# key=float).count(sorted(self.branchValues[self.toBeSampled[keyk]], key=float)[index]) > 1:
# self.raiseAWarning("In distribution " + str(self.toBeSampled[keyk]) + " the Threshold " +
# str(sorted(self.branchValues[self.toBeSampled[keyk]], key=float)[index])+" appears multiple times!!")
#             errorFound = True
...
# check if RELAP7 mode is activated, in case check that a <distribution> variable is unique in the input
  ...
  def localGetInitParams(self):
    ...
    paramDict = {}
    for key in self.branchProbabilities.keys():
      paramDict['Probability Thresholds for var ' + str(key) + ' are: '] = [
          str(x) for x in self.branchProbabilities[key]
      ]
    for key in self.branchValues.keys():
      paramDict['Values Thresholds for var ' + str(key) + ' are: '] = [
          str(x) for x in self.branchValues[key]
      ]
    return paramDict
  ...
  def localInitialize(self):
    ...
    for key in self.branchProbabilities.keys():
      ...
      # self.branchValues[key] =
      # [self.distDict[self.toBeSampled.keys()[self.toBeSampled.values().index(key)]].ppf(float(self.branchProbabilities[key][index]))
      # for index in range(len(self.branchProbabilities[key]))]
      self.branchValues[key] = [
          self.distDict[key].ppf(float(self.branchProbabilities[key][index]))
          for index in range(len(self.branchProbabilities[key]))
      ]
    for key in self.branchValues.keys():
      #kk = self.toBeSampled.values().index(key)
      # self.branchProbabilities[key] =
      # [self.distDict[self.toBeSampled.keys()[self.toBeSampled.values().index(key)]].cdf(float(self.branchValues[key][index]))
      # for index in range(len(self.branchValues[key]))]
      self.branchProbabilities[key] = [
          self.distDict[key].cdf(float(self.branchValues[key][index]))
          for index in range(len(self.branchValues[key]))
      ]
