...
class Stratified(Grid):
  ...
  def localInitialize(self):
    ...
    # For the multivariate normal distribtuion, if the user generates the grids on the transformed space, the user
    # needs to provide the grid for each variables, no globalGrid is needed
    if self.variablesTransformationDict:
      # for all variables
      tempFillingCheck = [[None] * (self.pointByVar - 1)] * len(
          self.gridEntity.returnParameter("dimensionNames"))
      self.sampledCoordinate = [[None] * len(self.axisName)] * (
          self.pointByVar - 1)
      for i in range(len(tempFillingCheck)):
        # pick a random interval sequence
        tempFillingCheck[i] = randomUtils.randomPermutation(
            list(range(self.pointByVar - 1)), self)
      mappingIdVarName = {}
      for cnt, varName in enumerate(self.axisName):
        mappingIdVarName[varName] = cnt
    # For the multivariate normal, if the user wants to generate the grids based on joint distribution, the user
    # needs to provide the globalGrid for all corresponding variables
    else:
      globGridsCount = {}
  ...
  def localGenerateInput(self, model, myInput):
    ...
    for varName in self.axisName:
        ...
        if self.variables2distributionsMapping[varName]['totDim'] > 1 and self.variables2distributionsMapping[varName]['reducedDim'] == 1:
          ...
          if self.variablesTransformationDict:
            ...
            for var in self.distributions2variablesMapping[distName]:
              # if the varName is a comma separated list of strings the user wants to sample the comma separated
              # variables with the same sampled value => link the value to all comma separated variables
      ...
      if ("<distribution>" in varName
        ...
        # 1D variable
        # if the varName is a comma separated list of strings the user wants to sample the comma separated variables
        # with the same sampled value => link the value to all comma separated variables
        upper = self.gridEntity.returnShiftedCoordinate(
            self.gridEntity.returnIteratorIndexes(),
