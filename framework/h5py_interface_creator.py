...
class hdf5Database(MessageHandler.MessageUser):
  ...
  def __init__(self,
    ...
    """
      ...
      @ In, variables, list, the user wants to store just some specific variables (default =None => all variables
             are stored)
      @ Out, None
    """
  ...
  def __checkTypeHDF5(self, value, neg):
    ...
    if neg:
      check = type(
          value
      ) == np.ndarray and value.dtype not in np.sctypes['float'] + np.sctypes['int'] and type(
  ...
  def __populateGroup(self, group, name, rlz):
    ...
    if len(varKeysIntfloat) > 0:
      ...
      group.create_dataset(
          name + "_dataIntFloat",
          dtype="float",
          data=(np.concatenate(dataIntFloat.values()).ravel()))
  ...
  def retrieveAllHistoryPaths(self, rootName=None):
    """
      ...
      @ In, rootName, string, optional, It's the root name, if present, only the groups that have this root are
  ...
  def retrieveAllHistoryNames(self, rootName=None):
    """
      ...
      @ In, rootName, string, optional, It's the root name, if present, only the history names that have this root
  ...
  def _getRealizationByName(self, name, options={}):
    """
      ...
      @ Out, (newData,attrs), tuple, tuple where position 0 = dict containing the realization, 1 = dictionary of
             some attributes
    """
