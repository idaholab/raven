...
class DataSet(DataObject):
  ...
  def __init__(
      self
  ):  #, in_vars, out_vars, meta_vars=None, dynamic=False, var_dims=None,cacheSize=100,prealloc=False):
    """
      Constructor.
      @ In, None
      @ Out, None
    ...
    self.name = 'DataSet'
    self.type = 'DataSet'
    # list of type objects, for each realization entry
    self.types = None
    self.printTag = self.name
    self.defaultDtype = object
    # mean, sigma for data for matching purposes
    self._scaleFactors = {}
    self._alignedIndexes = {
    }  # dict {index:values} of indexes with aligned coordinates (so they are not in the collector, but here instead)
    # metavariables required to reload this data object.
    self._neededForReload = [self.sampleTag]
  ...
  def addExpectedMeta(self, keys):
    ...
    keys = list(
        key for key in keys if key not in self.getVars() + self.indexes)
  ...
  def addMeta(self, tag, xmlDict):
    ...
    if tag not in self._meta.keys():
      ...
      new = StaticXMLOutput()
      # TODO replace name when writing later
      new.initialize(self.name, self.messageHandler)
      new.newTree(tag)
      self._meta[tag] = new
  ...
  def addRealization(self, rlz):
    ...
    try:
      ...
      self.raiseAnError(
          KeyError,
          'Provided realization does not have all requisite values for object "{}": "{}"'.
          format(self.name, e.args[0]))
    # check consistency, but make it an assertion so it can be passed over
    if not self._checkRealizationFormat(rlz):
      self.raiseAnError(
          SyntaxError,
          'Realization was not formatted correctly for "{}"! See warnings above.'.
          format(self.name))
    # format the data
    ...
    newData = np.array(
        list(rlz[var] for var in self._orderedVars) + [0.0], dtype=object)
    newData = newData[:-1]
  ...
  def addVariable(self, varName, values, classify='meta'):
    ...
    column = self._collapseNDtoDataArray(
        values, varName, labels=self._data[self.sampleTag])
    # add to the dataset
  ...
  def asDataset(self, outType='xrDataset'):
    ...
    if outType == 'xrDataset':
      ...
      self.raiseAnError(
          ValueError,
          'DataObject method "asDataset" has been called with wrong '
  ...
  def checkIndexAlignment(self, indexesToCheck=None):
    """
      ...
      @ In, indexesToCheck, list(str) or str or None, optional, indexes to check (or single index if string, or if
             None will check ALL indexes)
    ...
    if mathUtils.isAString(indexesToCheck):
      ...
      try:
        # TODO what if this errs?
        indexesToCheck = list(indexesToCheck)
      except TypeError:
        self.raiseAnError(
            'Unrecognized input to checkIndexAlignment!  Expected list, string, or None, but got "{}"'.
            format(type(indexesToCheck)))
    ...
    for index in indexesToCheck:
      ...
      if data.isnull().sum() > 0:
        self.raiseADebug(
            'Found misalignment index variable "{}".'.format(index))
        return False
  ...
  def getDimensions(self, var=None):
    ...
    if var is None:
      ...
      var = [var]
    dims = dict((v,
                 list(key for key in self._pivotParams.keys()
                      if v in self._pivotParams[key])) for v in var)
    return dims
  ...
  def getMeta(self, keys=None, pointwise=False, general=False):
    ...
    gKeys = set([]) if not general else set(self._meta.keys()).intersection(
        set(keys))
    pKeys = set([]) if not pointwise else set(self._metavars).intersection(
        set(keys))
    # get any left overs
    ...
    if len(missing) > 0:
      self.raiseAnError(
          KeyError,
          'Some requested keys could not be found in the requested metadata:',
          missing)
    meta = {}
    if pointwise:
      # TODO slow key crawl
  ...
  def getVars(self, subset=None):
    ...
    if subset == 'input':
      ...
      self.raiseAnError(KeyError,
                        'Unrecognized subset choice: "{}"'.format(subset))
  ...
  def load(self, fileName, style='netCDF', **kwargs):
    ...
    # TODO dask
      ...
      self.raiseAnError(NotImplementedError,
                        'Unrecognized read style: "{}"'.format(style))
  ...
  def realization(self,
    ...
    """
      ...
      @ In, unpackXArray, bool, optional, True if the coordinates of the xarray variables must be exposed in the
             dict (e.g. if P(t) => {P:ndarray, t:ndarray}) (valid only for dataset)
    ...
    if (index is None and matchDict is None) or (index is not None
                                                 and matchDict is not None):
      self.raiseAnError(
          TypeError,
          'Either "index" OR "matchDict" (not both) must be specified to use "realization!"'
      )
    numInData = len(
        self._data[self.sampleTag]) if self._data is not None else 0
      ...
      if index >= 0:
        ...
        if index > numInData - 1:
          ...
          if index > numInData + numInCollector - 1:
            self.raiseAnError(
                IndexError,
                'Requested index "{}" but only have {} entries (zero-indexed)!'.
                format(index, numInData + numInCollector))
      ...
      # handle "-" requests (counting from the end): first end of collector, or if not then end of data
        ...
        if abs(index) > numInCollector:
          ...
          if abs(index) > numInData + numInCollector:
            self.raiseAnError(
                IndexError,
                'Requested index "{}" but only have {} entries!'.format(
                    index, numInData + numInCollector))
    ...
    # matchDict must not be None
      ...
      if numInData == 0:
        ...
        # otherwise, get it from the collector
          ...
          index, rlz = self._getRealizationFromCollectorByValue(
              matchDict, tol=tol)
      # otherwise, first try to find it in the data
      else:
        index, rlz = self._getRealizationFromDataByValue(
            matchDict, tol=tol, unpackXArray=unpackXArray)
  ...
  def renameVariable(self, old, new):
    ...
    if isIndex:
      ...
      if old in self._alignedIndexes.keys():
        ...
        self._orderedVars = list(
            a if a != old else new for a in self._orderedVars)
  ...
  def sliceByIndex(self, index):
    """
      ...
      For example, if 'index' is 'time', then returns cross-sectional slices of the dataobject at each recorded
      'time' index value.
    ...
    if self._data is None or len(self._data) == 0:
      self.raiseAWarning(
          'Tried to return sliced data, but DataObject is empty!')
      return []
    # assert that index is indeed an index
    if index not in self.indexes + [self.sampleTag]:
      self.raiseAnError(
          IOError,
          'Requested slices along "{}" but that variable is not an index!  Options are: {}'.
          format(index, self.indexes))
    numIndexCoords = len(data[index])
    slices = list(data.isel(**{index: i}) for i in range(numIndexCoords))
    # NOTE: The slice may include NaN if a variable does not have a value along a different index for this snapshot
    # along "index"
    return slices
  ...
  def write(self, fileName, style='netCDF', **kwargs):
    """
      ...
      Includes: firstIndex, int, optional, if included then is the realization index that writing should start from
      (implies appending instead of rewriting)
      @ Out, index, int, index of latest rlz to be written, for tracking purposes
    """
    # just in case there is stuff left in the collector
    self.asDataset()
    if style.lower() == 'netcdf':
      self._toNetCDF(fileName, **kwargs)
    elif style.lower() == 'csv':
      # TODO what if it's just metadata?
      if len(self._data[self.sampleTag]) == 0:
    ...
    # TODO dask?
      ...
      self.raiseAnError(NotImplementedError,
                        'Unrecognized write style: "{}"'.format(style))
    if not self.hierarchical and 'RAVEN_isEnding' in self.getVars():
      return len(
          self._data.where(self._data['RAVEN_isEnding'] == True,
                           drop=True)['RAVEN_isEnding'])
    else:
      # so that other entities can track which realization we've written
      return len(self)
  ...
  def vars(self):
    """
      ...
      As opposed to "self._orderedVars", returns the variables clustered by subset (inp, out, meta) instead of order
      added
      @ In, None
  ...
  def _changeVariableValue(self, index, var, value):
    ...
    # if it's in the collector ...
      ...
      self.raiseAnError(
          IndexError,
          'Requested value change for realization "{}", which is past the end of the data object!'.
          format(index))
  ...
  def _checkAlignedIndexes(self, rlz, tol=1e-15):
    ...
    for index in self.indexes:
      ...
      if index in self._alignedIndexes.keys():
        ...
        # TODO there are concerns this check may slow down runs; it should be profiled along with other bottlenecks
        # to optimize our efforts.
        if len(rlz[index]) != len(self._alignedIndexes[index]):
          ...
          if mathUtils.isAFloatOrInt(rlz[index][0]):
            closeEnough = all(
                np.isclose(rlz[index], self._alignedIndexes[index], rtol=tol))
          else:
        ...
        if not closeEnough:
          ...
          self.raiseAWarning(
              'A mismatch in time scales has been found between consecutive realizations.  Consider synchronizing before doing any postprocessing!'
          )
      ...
      # if index is not among the aligned, check if it is already in the collector/data
        ...
        if self._collector is None or len(self._collector) == 0:
          try:
            ...
            # it's already gone; this can happen if this pivot parameter is only being used to collapse data (like
            # in PointSet case)
            pass
  ...
  def _checkRealizationFormat(self, rlz):
    ...
    for key, value in rlz.items():
      ...
      if not isinstance(value, np.ndarray):
        self.raiseAWarning(
            'Variable "{}" is not an acceptable type: "{}"'.format(
                key, type(value)))
        return False
      ...
      if self.indexes:
        if key in self._fromVarToIndex and rlz[self.
                                               _fromVarToIndex[key]].shape != rlz[key].shape:
          self.raiseAWarning((
              'Variable "{}" has not a consistent shape with respect its index "{}": '
              + 'shape({}) /= shape({})!').format(
                  key, self._fromVarToIndex[key], rlz[key].shape,
  ...
  def _clearAlignment(self):
    ...
    toRemove = list(
        self._orderedVars.index(var) for var in self.indexes
        if (var not in self._alignedIndexes.keys() and var in self._orderedVars
            ))
  ...
  def _collapseNDtoDataArray(self, data, var, labels=None, dtype=None):
    ...
    # second case: ND set (history set or higher dimension) --> CURRENTLY should be unused
      ...
      self.raiseAnError(TypeError,
                        'Unrecognized data type for var "{}": "{}"'.format(
                            var, type(data[0])))
    array.rename(var)
    return array
    ...
    if action == 'return':
      ...
      for var in self._inputs + self._outputs:
        ...
        try:
          ...
          # not there, so didn't need to remove
          pass
      ...
      self.raiseAnError(
          RuntimeError,
          'action "{}" was not an expected value for converting array list to dataset!'
          .format(action))
  ...
  def _convertFinalizedDataRealizationToDict(self, rlz, unpackXarray=False):
    """
      ...
      @ In, unpackXarray, bool, unpack XArray coordinates in numpy arrays (it assumes that the coordinates are
  ...
  def _convertToDict(self):
    ...
    """
    self.raiseAWarning(
        'DataObject._convertToDict can be a slow operation and should be avoided where possible!'
    )
    ...
    dataDict['data'] = dict((var, np.zeros(self.size, dtype=object))
  ...
  def _convertToXrDataset(self):
    ...
    if self._collector is not None and len(self._collector) > 0:
      ...
      firstSample = int(
          self._data[self.sampleTag][-1]) + 1 if self._data is not None else 0
      ...
      for v, var in enumerate(self._orderedVars):
        ...
        if isinstance(self._collector[0, v], np.ndarray):
          ...
          if len(dims) > 1:
            self.raiseAnError(
                NotImplementedError,
                'Currently cannot handle more than 1 pivot per variable')
          ...
          for index in dims:
            ...
            if index in self._alignedIndexes.keys():
              ...
              arrays[var] = self.constructNDSample(
                  data, dims=[self.sampleTag] + dims, coords=coords)
            # otherwise, we're better off making one dataarray for each rlz, then collapsing
              ...
              for r in range(len(self._collector)):
                ...
                coords = dict(
                    (idx, self._collector[r, self._orderedVars.index(idx)])
                    for idx in dims)
                self._collector[r][v] = self.constructNDSample(
                    values, dims, coords, name=str(r))
              # then collapse these entries into a single datarray
              arrays[var] = self._collapseNDtoDataArray(
                  self._collector[:, v], var, dtype=dtype)
        # if it's a dataarray, then that's old-style histories, no-can do right now
        elif isinstance(self._collector[0, v], xr.DataArray):
          self.raiseAnError(
              NotImplementedError,
              'History entries should be numpy arrays, not data arrays!')
  ...
  def _formatRealization(self, rlz):
    ...
    if self._collector is None or len(self._collector) == 0:
      for var in self._pivotParams.keys():
        ...
        # indexes have the correct dtype. In the first pass, they aren't going into the collector, but into
        # alignedIndexes.
  ...
  def _fromCSV(self, fileName, **kwargs):
    ...
    for var in self.getVars():
      if var in dims.keys():
        ...
        arrays[var] = self._collapseNDtoDataArray(
            ndat, var, labels=samples, dtype=dtype)
      else:
        # scalar example
        data = df[[var, self.sampleTag]].groupby(
            self.sampleTag).first().values[:, 0]
        dtype = self._getCompatibleType(data.item(0))
        arrays[var] = self._collapseNDtoDataArray(
            data, var, labels=samples, dtype=dtype)
    self._convertArrayListToDataset(arrays, action='extend')
  ...
  def _fromCSVXML(self, fileName):
    ...
    if dimsNode is not None:
      metadata['pivotParams'] = dict(
          (child.tag, child.text.split(',')) for child in dimsNode)
  ...
  def _fromDict(self, source, dims=None, **kwargs):
    ...
    if len(missing) > 0:
      self.raiseAnError(
          KeyError,
          'Variables are missing from "source" that are required for data object "',
    ...
    for v, var in enumerate(self._orderedVars):
      if len(source[var].shape) > 1:
        # we can't set all at once, because the user gave us an ND array instead of a np.array(dtype=object) of
        # np.array.
  ...
  def _getRealizationFromCollectorByValue(self, toMatch, tol=1e-15):
    ...
    for r, row in enumerate(self._collector[:,
                                            tuple(
                                                self._orderedVars.index(var)
                                                for var in toMatch.keys())]):
      match = True
  ...
  def _getRealizationFromDataByIndex(self, index, unpackXArray=False):
    """
      ...
      @ In, index, int, index to return
      @ In, unpackXArray, bool, optional, True if the coordinates of the xarray variables must be exposed in the
             dict (e.g. if P(t) => {P:ndarray, t:ndarray})
  ...
  def _getRealizationFromDataByValue(self,
                                     match,
                                     tol=1e-15,
                                     unpackXArray=False):
    """
  ...
  def _getRequestedElements(self, options):
    ...
    if 'what' in options.keys():
      ...
      # BY DEFAULT keep everything needed to reload this entity. Inheritors can define _neededForReload to specify
      # what that is.
      keep = set(self._inputs + self._outputs + self._metavars +
  ...
  def _loadCsvMeta(self, fileName):
    ...
    if len(meta) > 0:
      ...
      provided = set(
          meta.get('inputs', []) + meta.get('outputs', []) +
          meta.get('metavars', []))
    # otherwise, if we have no meta XML to load from, infer what we can from the CSV, which is only the available
    # variables.
    else:
    ...
    if len(missing) > 0:
      self.raiseAnError(
          IOError,
          'Not all variables requested for data object "{}" were found in csv "{}.csv"! Missing: {}'.
          format(self.name, fileName, missing))
  ...
  def _newCollector(self, width=1, length=100, dtype=None):
    ...
    """
    if dtype is None:
      # set in subclasses if different
  ...
  def _readPandasCSV(self, fname, nullOK=None):
    ...
    try:
      ...
      # no data in file
      self.raiseAWarning(
          'Tried to read data from "{}", but the file is empty!'.format(
              fname + '.csv'))
      return
    else:
    ...
    if (not nullOK) and (pd.isnull(df).values.sum() != 0):
      ...
      self.raiseAnError(IOError,
                        'Invalid data in input file: row "{}" in "{}"'.format(
                            bad + 1, fname))
    return df
  ...
  def _setScalingFactors(self, var=None):
    ...
    # TODO check against collector entries?
    assert (self._data is not None)
    for var in varList:
      ...
      try:
        ...
        self.raiseADebug(
            'Had an issue with setting scaling factors for variable "{}". No big deal.'.
            format(var))
        pass
  ...
  def _toCSV(self, fileName, start=0, **kwargs):
    ...
    self.raiseADebug(
        'Printing data to CSV: "{}"'.format(filenameLocal + '.csv'))
  ...
  def _toCSVCluster(self, fileName, start, clusterLabel, **kwargs):
    ...
    # TODO append mode if printing each step
    with open(fileName + '.csv', 'w') as writeFile:
      writeFile.writelines('{},filename\n'.format(clusterLabel))
      for ID in clusterIDs:
        writeFile.writelines('{},{}_{}.csv\n'.format(ID, fileName, ID))
      self.raiseADebug(
          'Wrote master cluster file to "{}.csv"'.format(fileName))
    # write sub files as point sets
    ordered = list(
        var
        for var in itertools.chain(self._inputs, self._outputs, self._metavars)
        if (var != clusterLabel and var in keep))
    for ID in clusterIDs:
      data = self._data.where(
          self._data[clusterLabel] == ID, drop=True).drop(clusterLabel)
  ...
  def _toCSVXML(self, fileName, **kwargs):
    ...
    with open(fileName + '.xml', 'w') as ofile:
      ...
      for name, target in meta.items():
        xml = target.writeFile(
            asString=True, startingTabs=1, addRavenNewlines=False)
  ...
  def _toNetCDF(self, fileName, **kwargs):
    ...
    self._data.attrs = dict(
        (key, pk.dumps(val)) for key, val in self._meta.items())
    self._data.to_netcdf(fileName, **kwargs)

  def _usePandasWriteCSV(self,
                         fileName,
                         data,
                         ordered,
                         keepSampleTag=False,
    ...
    if keepSampleTag:
      ...
      if isinstance(data.index, pd.MultiIndex):
        # if we have just the self.sampleTag index (we can not drop it otherwise pandas fail). We use index=False
        # (a.a.)
        localIndex = True
        ...
        if not localIndex:
          data.to_csv(
              fileName + '.csv', mode=mode, header=header, index=localIndex)
        else:
          data.to_csv(fileName + '.csv', mode=mode, header=header)
  ...
  # _useNumpyWriteCSV (below) is a secondary method to write out POINT SET CSVs. When benchmarked with Pandas, I
  # tested using
  ...
  # The per-float columns divide the time taken by (R*M) to give a fair comparison.  The summary of the
  # var versus
  # realizations per float is:
  #          ---------- R ----------------
  #   M      4             100        1000
  ...
  def _constructHierPaths(self):
    ...
    for p, path in enumerate(paths):
      rlzs = list(
          self._data.where(data['prefix'] == ID, drop=True) for ID in path)
  ...
  def _generateHierPaths(self):
    ...
    for e, ending in enumerate(endings):
      ...
      path = [ending['prefix']]
      while ending['RAVEN_parentID'] != "None" and not pd.isnull(
          ending['RAVEN_parentID']):
        _, ending = self.realization(
            matchDict={'prefix': ending['RAVEN_parentID']})
  ...
  def _getPathEndings(self):
    ...
    if self._collector is not None and len(self._collector) > 0:
      ...
      fromColl = self._collector[np.where(
          self._collector[:, self._orderedVars.index('RAVEN_isEnding')])]
    ...
    if self._data is not None and len(self._data[self.sampleTag].values) > 0:
      ...
      indexes = self._data.where(
          self._data['RAVEN_isEnding'], drop=True)[self.sampleTag].values
