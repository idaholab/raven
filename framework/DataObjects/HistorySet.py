...
class HistorySet(DataSet):
  ...
  def _toCSV(self, fileName, start=0, **kwargs):
    ...
    # If True, all the branches are going to be printed/plotted independenttly, otherwise the are going to be
    # reconstructed
    ...
    if len(ordered):
      ...
      self.raiseAWarning(
          'No output space variables have been requested for DataObject "{}"! No history files will be printed!'.
          format(self.name))
