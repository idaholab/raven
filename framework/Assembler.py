...
class Assembler(MessageHandler.MessageUser):
  """
    Assembler class is used as base class for all the objects that need, for
    initialization purposes, to get pointers (links) of other objects at the
    Simulation stage (Simulation.run() method)
  """
  ...
  def __init__(self):
    ...
    # {MainClassName(e.g.Models):[class(e.g.Models),type(e.g.ROM),objName]}
    # list. first entry boolean flag. True if the XML parser must look for objects
    # second entry tuple.first entry list of object can be retrieved, second entry
    # multiplicity (-1,-2,-n means optional (max 1 object,2 object, no number limit))
    self.assemblerObjects = {}
    # required assembler object
    self.requiredAssObject = [False, ([], [])]
    # {'class':[['class','type','name',instance]]}}
    self.assemblerDict = {}

  def whatDoINeed(self):
    """
      This method is used mainly by the Simulation class at the Step construction stage.
      It is used for inquiring the class, which is implementing the method, about the
      kind of objects the class needs to be initialize.
      @ In, None
      @ Out, needDict, dict, dictionary of objects needed (class:tuple(object type{if
                               None, Simulation does not check the type}, object name))
    """
  ...
  def generateAssembler(self, initDict):
    """
      ...
      @ In, initDict, dict, dictionary
                          ({'mainClassName(e.g., Databases):{specializedObjectName(e.g.,
                           DatabaseForSystemCodeNamedWolf):ObjectInstance}'})
      @ Out, None
    """
  ...
  def _readAssemblerObjects(self, subXmlNode, found, testObjects):
    """
      ...
      @ In, found, dict, a dict that check if all the tokens are found
      @ In, testObjects, dict, a dict containing the number of time a token has been found
      @ Out, returnObject, tuple, tuple(found, testObjects) containig in:
                      [0], found -> dict that check if all the tokens (requested) are found
                      [1], testObjects -> a dict containing the number of time token has been found
    """
  ...
  def _readMoreXML(self, xmlNode):
    """
      ...
      and initialize some variables based on the inputs got. This method is used to automatically
      generate the Assembler 'request' based on the input of the daughter class.
      @ In, xmlNode, xml.etree.ElementTree.Element, XML element node that represents the portion
                    of the input that belongs to this class
      @ Out, None
    """
  ...
  def addAssemblerObject(self, name, flag, newXmlFlg=None):
    """
      ...
      @ In, newXmlFlg, boolean, optional, if passed in, the first entry of the
                                          ...
                                          the self.requiredAssObject[0] is set to True
      @ Out, None
    """
  ...
  def retrieveObjectFromAssemblerDict(self, objectMainClass, objectName, pop=False):
    """
      ...
      @ In, objectMainClass, str, the object main Class name (e.g. Input, Model, etc.) of
                                           the object that needs to be retrieved
      @ In, pop, bool, optional, if found pop it out (i.e. remove it from the self.assemblerDict?).
                                       Default = False
