        ...
        for subNode in xmlNode:
            for token in self.requiredAssObject[1][0]:
                if subNode.tag in token:
                    found = True
                    if 'class' not in subNode.attrib.keys(): raiseAnError(IOError,self,'In '+self.type+' PostProcessor ' + self.name+ ', block ' + subNode.tag + ' does not have the attribute class!!')
        ...
        if not found:
            for tofto in self.requiredAssObject[1][0]:
                if not str(self.requiredAssObject[1][1][0]).strip().startswith('-'):
                    raiseAnError(IOError,self,'the required object ' +tofto+ ' is missed in the definition of the '+self.type+' PostProcessor!')
            ...
            for cnt,tofto in enumerate(self.requiredAssObject[1][0]):
                ...
                # optional
                    if tofto in testObjects.keys():
                        ...
                        if testObjects[tofto] != int(numerosity): raiseAnError(IOError,self,'Only '+numerosity+' '+tofto+' object/s is/are optionally required. PostProcessor '+self.name + ' got '+str(testObjects[tofto]) + '!')
                else:
                # required
                    if tofto not in testObjects.keys(): raiseAnError(IOError,self,'Required object/s "'+tofto+'" not found. PostProcessor '+self.name + '!')
                    else:
                        numerosity = numerosity.replace('n',str(testObjects[tofto]))
                        if testObjects[tofto] != int(numerosity): raiseAnError(IOError,self,'Only '+numerosity+' '+tofto+' object/s is/are required. PostProcessor '+self.name + ' got '+str(testObjects[tofto]) + '!')
