...
class Assembler(MessageHandler.MessageUser):
  ...
  def __init__(self):
    ...
    # second entry tuple.first entry list of object can be retrieved, second entry multiplicity (-1,-2,-n means
    # optional (max 1 object,2 object, no number limit))
  ...
  def whatDoINeed(self):
    """
      ...
      It is used for inquiring the class, which is implementing the method, about the kind of objects the class
      needs to
      be initialize.
      @ In, None
      @ Out, needDict, dict, dictionary of objects needed (class:tuple(object type{if None, Simulation does not
             check the type}, object name))
    """
  ...
  def _readAssemblerObjects(self, subXmlNode, found, testObjects):
    """
      ...
      @ Out, returnObject, tuple, tuple(found, testObjects) containig in [0], found -> a dictionary that check if
             all the tokens (requested) are found ;
  ...
  def _readMoreXML(self, xmlNode):
    """
      ...
      and initialize some variables based on the inputs got. This method is used to automatically generate the
      Assembler 'request'
  ...
  def addAssemblerObject(self, name, flag, newXmlFlg=None):
    """
      ...
      @ In, newXmlFlg, boolean, optional, if passed in, the first entry of the tuple self.requiredAssObject is going
             to updated with the new value
  ...
  def retrieveObjectFromAssemblerDict(self,
    ...
    """
      ...
      @ In, objectMainClass, str, the object main Class name (e.g. Input, Model, etc.) of the object that needs to
             be retrieved
