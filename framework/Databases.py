...
class DateBase(BaseType):
  ...
  def __init__(self, runInfoDict):
    ...
    # For printing verbosity labels
    self.printTag = 'DATABASE'
    # if not None, list of specific variables requested to be stored by user
    self.variables = None
...
class HDF5(DateBase):
  ...
  def returnHistory(self, options):
    """
      ...
      @ Out, tupleVar, tuple, tuple in which the first position is a numpy aray and the second is a dictionary of
             the metadata
      Note:
