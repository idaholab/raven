...
class Dymola(CodeInterfaceBase):
  ...
  def finalizeCodeOutput(self, command, output, workingDir):
    ...
    self.raiseAMessage('Reading the .mat results file from the Dymola simulation')
    ...
    try:
      ...
      self.raiseAnError(KeyError, 'File structure not supported!')
    ...
    __names = []
    # Variables and parameters to load if the user has mentionned anything
    if len(self.variablesToLoad):
      for varName in self.variablesToLoad:
        if varName not in _names:
          self.raiseAnError(KeyError, 'The variable: {} could not be found in the Dymola model, \
      ...
      self.raiseAMessage("The following variables will be loaded from the .mat results file : {}".format(__names))
    else: # Default behavior is load every parameter and variable from the model
      # TODO Can cause issue if more than 300 to save them to database: change default behavior, ask the user to s
      ...
      for varName in _names:
        ...
        columnMap[varName] = mat['dataInfo'][1][index]-1
      __names = _names
      self.raiseADebug("All variables and parameters will be loaded from the model, this can cause issues \n\
    ...
    # Format description in DataObjects/DataSet.py
    # So for a parameter i.e. a scalar it should be : {'Porosity': np.array([0.7])}
    # And for a variable i.e. a history it should be : {'...Tf[1]': np.array([36.6, 23.6, 23.7, ...])}
    response = {}
    traj = []
    ...
    for varName in __names: 
      ...
      elif dataBlockMap[varName] ==2: #Variables' histories are saved in data_2
        traj = np.array(mat['data_2'][columnMap[varName]])
      response[varName] = traj
    # Releasing memory asap for big Dymola models
    del mat
    # Fix time duplicates (re-use code from Andrea Alfonsi, Jan 2021)
    t = pd.Series(timeStepsArray.flatten())
    m = t.duplicated()
    if len(t[m]):
      # duplicated values
      tIndex = None
    ...
    response['Time'] = t.to_numpy()

    # Memory release
    del timeStepsArray
    del __names

    self.raiseAMessage("Finished reading outputs from Dymola simulation")
    return response

  def finalizeCodeOutput2(self, command, output, workingDir):
    """
      Called by RAVEN to modify output files (if needed) so that they are in a proper form.
      In this case, the default .mat output needs to be converted to .csv output, which is the
      format that RAVEN can communicate with.
    ...
    if fileInfo[1] == '1.1' and fileInfo[3] == 'binTrans':
      ...
      print('TRaj length {}'.format(numOutputPts))
      ...
      #variablesToLoad = ['Time', 'thermocline_Insulation.thermocline_fluidprops_heaters_newHC_70C.Tf[1]', 'thermocline_Insulation.geometry.Porosity']
      ...
      print(timeSeriesData1.shape)
      #print(timeSeriesData1)
      timeSeriesData2 = np.array(_timeSeriesData2)
      print('Variables matrix ....')
      print(timeSeriesData2.shape)
      #print(timeSeriesData2)
      ...
      print('New parameters matrix ....')
      print(Data1Array.shape)
