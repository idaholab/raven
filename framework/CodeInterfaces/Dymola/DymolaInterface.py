...
class Dymola(CodeInterfaceBase):
  ...
  def finalizeCodeOutput(self, output, workingDir):
    """
      ...
      @ Out, response, dict, realization containing the time stamps, the variables' histories and the parameters' values.
    """
    ...
    print('Reading the .mat results file from the Dymola simulation')
    # Later: self.raiseADebug('Reading the .mat results file from the Dymola simulation')
    ...
    if len(self.variablesToLoad):
      for varName in self.variablesToLoad:
        if varName not in _names:
          raise KeyError('The variable: {} could not be found in the Dymola model, \
      ...
      # Maybe later : self.raiseAMessage("The following variables will be loaded from the .mat results file : {}".format(__names))
