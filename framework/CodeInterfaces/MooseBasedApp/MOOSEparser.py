...
class MOOSEparser():
  ...
  def __findInXML(self, element, name):
    """
      ...
      @ Out, returnTuple, tuple, response. returnTuple[0] bool (True if found) and returnTuple[1] string (binary
             name in the element)
    """
    returnTuple = None
  ...
  def modifyOrAdd(self, modiDictionaryList, save=True):
    ...
    if save:
      ...
      returnElement = copy.deepcopy(self.root)
    else:
      # otherwise return the original modified
      returnElement = self.root
