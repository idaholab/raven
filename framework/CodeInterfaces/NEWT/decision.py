...
def __residuumSign(self):
  """
    Calculates the residuum based on the PHISICS fine-group solution
    @ In, None
    @ Out, None
  """
  refDict = {}
  ...
  values = []
  count = 0
  # energy values (eV) in the broad group structure currently sampled
  energy = [self.G1,self.G2,self.G3,self.G4,self.G5,self.G6]
  # flux in the broad  group structure currently sampled
  ...
  # constrain 1: normalized neutron flux must be within 12% of the ref value if normlized flux level > 0.5
  # and within 25% of the ref value if the flux level is < 0.5
  ...
  # constrain 2: keff relative to braod structure must be within 200 pcm compared to the fine group solution
  if keffDiff > -200.0 and keffDiff < 200.0:
    # Count the group-wise fluxes passing constrain 1
    ...
    # Exception: if a ratio is larger than 2.0 or smaller than 0.2, list element is 1, 0 otherwise. This is a failing condition regarless of the rest
    screenedRatioList = [normFluxColl[el] / normFluxBroad[el] if normFluxColl[el] > 0.1 else 1.0 for el in range(len(normFluxBroad))]
    exceededRatio = [1 if el > 2.0 or el < 0.2 else 0 for el in screenedRatioList]
    # constrain 3: half of the group-wise fluxes must pass constrain 1, and pass the exception
...
class ParseFlux:
  """
    ...
    required input:
    - PHISICS output in 252 groups (name: HTGR-test.o0)
  ...
  def __init__(self,energy,flux):
    """
      Constructor
      @ In, energy, list, energy boundaries of the broad-group structure
      @ In, flux, list, group-wise fluxes of the broad-group structure
      @ Out, None
    """
    mpi = 0
    ...
    output = os.path.join(self.workingDir,'HTGR-test.o-0') # the ref 252 is located in the root directory
  ...
  def getReactionRates(self,output,mpi,myReactDict):
    """
      Gets the reactions rates and power for each group in PHISICS
      @ In, output, string, PHISICS output file.
  ...
  def summedDictValues(self,groupOrTot, nestedDict):
    """
      ...
      @ In, groupOrTot, string, equal to 'group' or 'total'. Indicates if the group-wise RR or total RR are passed to this function
  ...
  def getFluxPerLethargy(self,energy,flux):
    """
      ...
      @ In, flux, list, group-wise flux values
      @ Out, NormFluxPerLethargy, list, normalized neutron flux per unit lerthargy
    """
    fluxPerLethargy = []
    NormFluxPerLethargy = []
  ...
  def collFlux(self,struct249,structBroad,funcToColl):
    """
      ...
      @ Out, normCollapsed, list, normalized neutron flux collapsed
    """
    totalFunc = 0
    count = 0
    energyIndex = []
  ...
  def plot(self,resultDict,nGroupsKey,bounds,pert):
    """
      ...
      @ Out, energyDupli, list, list of energy boundaries duplicated
      @ Out, fluxDupli, list, list of flux duplicated
      @ Out, fissionDupli, list, list of fission rates duplicated
      @ Out, normFluxDupli, list, list of flux (normalized) duplicated
      @ Out, energy, list, list of energy bins (eV)
      @ Out, fluxOrdered, list, list of flux ordered in decreasing energy
