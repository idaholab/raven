...
class Newt(CodeInterfaceBase):
  """
    This class is the NEWT code interface. Input required:
    - NEWT reference 252 group output to get the reference flux
    - decision.py file. Collapses the reference flux into the broad-group structure generated by RAVEN
    - template.inp (one for each lattice)
  """
  def __init__(self):
    """
      Constructor. Calls the PHISICS interface.
      @ In, None
      @ Out, None
    """
  ...
  def getPhisicsRefValues(self,rootDir):
    """
      Parses the 252-group PHISICS reference output. The NEWT 252-group and PHISICS is ran offline. The PHISICS
      output has to be completed before RAVEN was run. The reference output (numbers and instant output, and instant flux csv file)
    ...
    phisicsDataDict['instantOutput'] = self.PhisicsInterface.jobTitle+'.o'
    phisicsDataDict['workingDir'] = rootDir
    phisicsDataDict['mrtauStandAlone'] = False
    phisicsDataDict['jobTitle'] = self.PhisicsInterface.jobTitle
    ...
    phisicsDataDict['parseFlux'] = None # The flux in fine group structure is collapsed only AFTER the broad-group simulations. At this stage, only the reference fine group is parsed, but the broad calc. have not been performed yet, so impossible to use the parseFlux method in phisicsdata
  ...
  def generateCommand(self,inputFiles,executable,clargs=None,fargs=None):
    ...
    for i in range(int(self.numberOfLattice)):
      commandToRunNewt = executable + ' -N 1 -m -r -T TEMP' + self.latticeNames[i] + ' ' + self.latticeNames[i] + ' &>log_run_$PBS_JOBNAME.out'
    ...
    for i in range(int(self.numberOfLattice)):
      # The default name of the NEWT microscopic collapse library is always ft30f001
      # The AMPXconverter converts the micro lib from the SCALE6.2 format to the SCALE6.1 format (compatible with PHISICS)
    ...
    commandToRunDisadFactorScript = 'python ' + self.scaleParserFile
    ...
    commandToRunPhisics = self.phisicsExecutable + ' -i ' +inputFiles[dict['inp'.lower()]].getFilename() + ' -xs ' + inputFiles[dict['Xs-library'.lower()]].getFilename() + ' -mat ' + inputFiles[dict['Material'.lower()]].getFilename() + ' -dep ' + inputFiles[dict['Depletion_input'.lower()]].getFilename() + ' -o ' + self.instantOutput
  ...
  def getNumberOfFineGrps(self,txsecInp):
    """
      This method determines the number of fine groups used in the reference transport calculations. Possible options: 252, 238 or 56 groups
      @ In, txsecInp, string, path to txsec input
      @ Out, nGroups, integer, number groups
    """
    flag = 0
    with open(txsecInp, "rt") as lin:
      for line in lin:
        if re.search(r't-xsec', line):
          flag = 1
        if re.search(r'read comp', line):
          raise IOError('Error: cannot find the number of groups used in TXSEC')
        if flag == 1 and re.search(r'252',line):
          return 252
        elif flag == 1 and re.search(r'238',line):
          return 238
        elif flag == 1 and re.search(r'56',line):
          return 56

  def parserParser(self, scaleParser, latticeOut, subDirPath, libFile):
    """
      Parses the scaleParser (!) to pass the NEWT file names and number of lattices
      @ In, scaleParser, path, path to the scaleParser file
      @ Out, None
    """
    flag = 0
    input = open(scaleParser, "r")
    output = open(os.path.join(subDirPath,'temp.py'), "w")
    for line in input.readlines():
      if re.search(r'input flag', line):
        flag = 1
      if re.search(r'lib flag', line):
        flag = 2
      if flag == 1 and re.search(r'inputFile',line):
        # formats the list into a string ['newt0.int','newt1.inp','newt2.inp'] (indep of the number of lattice cells)
        output.write(line.replace('inputFile','inputFile = [\''+'\',\''.join(latticeOut)+'\']'))
      elif flag == 2 and re.search(r'self.outputFile',line):
        # implements the lib.xml file name
        output.write(line.replace('self.outputFile','self.outputFile = \''+libFile+'\''))
      else:
        output.write(line)
    os.rename(os.path.join(subDirPath,'temp.py'),scaleParser)
  ...
  def createNewInput(self,currentInputFiles,oriInputFiles,samplerType,**Kwargs):
    ...
    self.latticeNames = []
    latticeOut = []
    # Variables that PHISICS can perturb. The other variables are related to RELAP
    ...
    subDirPath = currentInputFiles[keyWordDict['inp']].getPath()
    nGroups = self.getNumberOfFineGrps(currentInputFiles[keyWordDict['txsec']].getAbsFile())
    # defines the arbitrary names of each lattice cells --- newt1.inp newt2.inp etc. ---
    self.scaleParserFile = currentInputFiles[keyWordDict['scaleparser']].getAbsFile()
    print (self.scaleParserFile)
    for i in range (int(self.numberOfLattice)):
      self.latticeNames.append('newt'+str(i)+'.inp')
      latticeOut.append('newt'+str(i)+'.out')
    self.parserParser(self.scaleParserFile, latticeOut, subDirPath, currentInputFiles[keyWordDict['xs-library']].getFilename())
    for paramVariable in self.distributedPerturbedVars.iterkeys():
      if paramVariable == 'group' :
        inputParser.inputParser(nGroups, subDirPath, self.latticeNames, self.numberOfLattice, **self.distributedPerturbedVars[paramVariable])
