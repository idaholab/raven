...
class GenericParser():
  ...
  def __init__(self,
               ...
               formatDelim='|'):
    """
      Accept the input file and parse it by the prefix-postfix breaks. Someday might be able to change
      prefix,postfix,defaultDelim from input file, but not yet.
      ...
      for line in lines:
        while self.prefixKey in line and self.postfixKey in line:
          ...
          if defaultDelim in var or formatDelim in var:
            ...
            defval = var[optionalPos[0] + 1:min(optionalPos[1], len(
      ...
      for inputFile in self.segments.keys():
        for place in self.varPlaces[var][
          ...
          if var in modDict.keys():
            ...
            # this gets handled in writeNewInput
            continue
          else:
  ...
  def writeNewInput(self, inFiles, origFiles):
    ...
    # FIXME the first entry? This is bad! Forces order somewhere in input file
