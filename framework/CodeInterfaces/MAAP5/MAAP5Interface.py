...
###########
    for child in xmlNode:
      ...
      if child.tag == 'stopSimulation':
        self.stop=child.text #this node defines if the MAAP5 simulation stop condition is
  ...
  def oriInput(self, oriInputFiles):
    ...
    for var in self.DETsampledVars:
      if not var in branching:
        raise IOError('Please define a branch/add a branching marker for the variable
    ...
    if 'DynamicEventTree' in self.samplerType:
      ...
      for timer in self.timer.values():
        ...
        try:
          ...
          raise IOError('Please ensure that the timer is defined into the include file and then it is contained into MAAP5 plotfil
