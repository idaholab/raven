...
class MAAP5(GenericCode):
  def _readMoreXML(self, xmlNode):
    ...
    self.include = ''
    self.tilastDict = {
    }  #{'folder_name':'tilast'} - this dictionary contains the last simulation time of each branch, this is necessary to define the correct restart time
    self.branch = {
    }  #{'folder_name':['variable branch','variable value']} where variable branch is the variable sampled for the current branch e.g. {det_1:[timeloca, 200]}
    self.values = {
...
########################
  ...
  def finalizeCodeOutput(self, command, output, workingDir):
    ...
    RAVENoutputFile = os.path.join(
        workingDir, output + ".csv"
