...
class MAAP5(GenericCode):
  def _readMoreXML(self, xmlNode):
    ...
    self.include = ''
    # {'folder_name':'tilast'} - this dictionary contains the last simulation time of each branch, this is necessary
    # to define the correct restart time
    self.tilastDict = {}
    # {'folder_name':['variable branch','variable value']} where variable branch is the variable sampled for the
    # current branch e.g. {det_1:[timeloca, 200]}
    self.branch = {}
    # {'folder_name':[['variable branch_1','variable value_1'],['variable branch_2','variable value_2']]} for each
    # DET sampled variables
    self.values = {}
    # value of the print interval
    self.printInterval = ''
    # list of MAAP5 boolean variables of interest
    self.boolOutputVariables = []
    ...
    for child in xmlNode:
      ...
      if child.tag == 'stopSimulation':
        # this node defines if the MAAP5 simulation stop condition is: 'mission_time' or the occurrence of a given
        # event e.g. 'IEVNT(691)'
        self.stop = child.text
...
###########
  ...
  def oriInput(self, oriInputFiles):
    ...
    # list of RAVEN DET sampled variables
    self.DETsampledVars = []
    # list of RAVEN HYBRID sampled variables
    self.HYBRIDsampledVars = []
    ...
    # is True, when for loop is in the block defining the variables which are HYBRID sampling
    HYBRIDVar = False
    # is True, when for loop is in the block defining the variables which are DET sampling
    DETVar = False
    # is True, when one branching condition is found
    foundDET = False
    ...
    # read the original input file in order to find DET Sampled variables and their default values, save the print
    # interval
    # open .inp file
    ...
    for line in lines:
      ...
      if line.find('$RAVEN') != -1 and DETVar:
        ...
        # var.strip() deletes whitespace
        self.DETsampledVars.append(var.strip())
    ...
    self.timer = {}
    # open include file, this file contains all the user-defined TIMER
    fileobject = open(self.include, "r")
    lines = fileobject.readlines()
    fileobject.close()
    lineNumber = 0
    branchingMarker = ''
    ...
    # TIMER condition
    found = [False] * len(self.DETsampledVars)
    # timer 'block'
    block = [False] * len(self.DETsampledVars)
    stop = False
    for line in lines:
      ...
      for cont, var in enumerate(self.DETsampledVars):
        ...
        if (str('SET TIMER') in line) and block[cont]:
          ...
          self.lineTimerComplete.append(
              'TIMER ' + str(self.timer[var])
          )  #this list contains all the TIMER associated with the branching e.g. [TIMER 100, TIMER 101, TIMER 102]
  ...
  def restart(self, currentInputFiles, parentName):
    ...
    """
    # is the correct line for the restart time definition
    correctRestart = ''
    newLine = ''
    restarFileCorrect = ''
    ...
    # open MAAP .inp for the current run
    fileobject = open(inp, "r")
    lines = fileobject.readlines()
    fileobject.close()

    #verify the restart file and restart time is defined into the input file
    # is True, if the restart time defined is correct
    restartTime = False
    # is True, if the restart file declaration is found
    foundRestart = False
    # is True, if the restart file found is correct
    correct = False
...
########################
  ...
  def finalizeCodeOutput(self, command, output, workingDir):
    ...
    """
    csvSimulationFiles = []
    # rootname of the simulation files
    realOutput = output.split("out~")[1]
    # input file of the simulation with the full path
    inp = os.path.join(workingDir, realOutput + ".inp")
    # rootname of the simulation files with the full path
    filePrefixWithPath = os.path.join(workingDir, realOutput)
    # list of MAAP output files with the evolution of continuous variables
    csvSimulationFiles = glob.glob(filePrefixWithPath + ".d" + "*.csv")
    ...
    # RAVEN will look for  output+'.csv'file but in the workingDir, so we need to append it to the filename
    RAVENoutputFile = os.path.join(workingDir, output + ".csv")
    ...
    if 'DynamicEventTree' in self.samplerType:
      ...
      for timer in self.timer.values():
        ...
        try:
          ...
          raise IOError(
              'Please ensure that the timer is defined into the include file and then it is contained into MAAP5 plotfil: ',
              timer)
      ...
      #
      # NOTE THAT THIS ERROR CAN BE WRONG SINCE IT IS POSSIBLE (BRANCHES ON DEMAND) THAT TWO BRANCHES (OR MORE)
      # HAPPEN AT THE SAME TIME! Andrea
      #
      dictTimeHappened = []
      ...
      # if any([dictTimeHappened.count(value) > 1 for value in dictTimer.values()]): raise IOError('Branch must
...
######################################################"
  ...
  def branchXml(self, tilast, Dict, inputFile, dataDict):
    ...
    @ In, tilast, string, end time of the current simulation run
      @ In, Dict, dict, dictionary containing the name and the value of the variables modified by the branch
             occurrence
    @ In, inputFile, string, name of the current input file
      @ In, dataDict, dict, dictionary containing the time evolution of the MAAP5 output variables contained in the
             csv output file
    @ Out, None
    """
    self.multiBranch = []
    ...
    if variableBranch in self.DETsampledVars and variableBranch not in self.multiBranch:
      ...
      for var in variables:
        ...
        # this function writes the xml file for DET
        detU.writeXmlForDET(filename, variableBranch, listDict, stopInfo)
    else:
      ###########
...
#      @ In, inputFile, string, name of the current input file
# @ In, dataDict, dict, dictionary containing the time evolution of the MAAP5 output variables contained in the csv
# output file
#      @ Out, None
#    """
#    base=os.path.basename(inputFile).split('.')[0]#
...
#      detU.writeXmlForDET(filename,variableBranch,listDict,stopInfo) #this function writes the xml file for DET
  ...
  def dictVariables(self, currentInp):
    """
      ...
      @ Out, self.DictAllVars, dict, dictionary containing the value of all the variables that could branching and
             the value of the corresponding variables that would be modified in the branches
    """
    # open MAAP .inp
    fileobject = open(currentInp, "r")
    lines = fileobject.readlines()
    fileobject.close()
    # this dictionary will contain all the dicitonary, one for each DET sampled variable
    self.DictAllVars = {}
    for var in self.DETsampledVars:
      block = False
      DictVar = {}
      for line in lines:
        if (var in line) and ('=' in line) and not ('WHEN' or 'IF') in line:
          # conditions on var and '=' ensure that there is an assignment,while condition on 'WHEN'/'IF' excludes
          # e.g.'WHEN TIM<=AFWOFF' line
        ...
        if ('END' in line) and block:
          block = False
      self.DictAllVars["Dict{0}".format(
          var
      )] = DictVar  #with Dict{0}.format(var) dictionary referred to each single sampled variable is called DictVar (e.g., 'DictTIMELOCA', 'DictAFWOFF')
  ...
  def stopSimulation(self, currentInputFiles, Kwargs):
    """
      ...
      @ Out, Kwargs, dict,kwarded dictionary of parameters. In this dictionary there is another dictionary called
             "SampledVars"
           where RAVEN stores the variables that got sampled (e.g. Kwargs['SampledVars'] => {'var1':10,'var2':40})
    """
    for filename in currentInputFiles:
...
####################
      ...
      while len(
          parent.split('_')[-1]
      ) > 2:  #collect the name of all the parents, their corresponding timer need to be deleted from the stop condition (when the parents has already occurred)
        parent = parent[:-2]
        parents.append(parent)
