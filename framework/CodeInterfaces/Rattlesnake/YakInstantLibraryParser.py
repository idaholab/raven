...
class YakInstantLibraryParser():
  ...
  def __init__(self, inputFiles):
    ...
    self.level0Element = 'Materials'
    # level 1 element tag is always Macros
    self.level1Element = 'Macros'
    # These are some of the level 2 element tag with string vector xmlnode.text, without xml subnodes
    self.level2Element = ['material']
    ...
    # total energy groups
    self.nGroup = None
    # total energy groups defined in alias files
    self.aliasesNG = None
    ...
    for xmlFile in inputFiles:
      ...
      if macrosLib != None:
        if self.nGroup == None:
          ...
          self.nGroup = int(macrosLib.attrib['NG'])
  ...
  def _readYakXSInternal(self, library, pDict, keyDict):
    """
      ...
      @ In, keyDict, dict, dictionary to store the instatnt library node names, use to trace the cross section types
  ...
  def _readScatteringXS(self, profile, scattering, pDict):
    ...
    if has_profile:
      ...
      if scattering is not None:
        # store in 1-D array
        scatteringValue = self._stringSpacesToNumpyArray(scattering.text)
      else:
        raise IOError(
  ...
  def _checkYakXS(self, reactionDict):
    ...
    if 'TotalXS' not in reactionList:
      if 'DiffusionCoefficient' not in reactionList:
        raise IOError(
            'Total and diffusion coefficient cross sections are not found in the cross section input file, at least one of them should be provided!'
        )
      else:
  ...
  def _rebalanceXS(self, reactionDict, perturbDict, aliasType):
    """
      Recalculate some depedent xs, such as 'TotalXS', 'AbsorptionXS', 'ScatteringXS', 'NuFissionXS',
      'KappaFissionXS',
    ...
    reactionDict['RemovalXS'] = np.asarray(
        list(reactionDict['TotalXS'][g] - reactionDict['ScatteringXS'][g][g]
             for g in range(self.nGroup)))
