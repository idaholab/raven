...
class YakMultigroupLibraryParser():
  ...
  # Functions Used for Reading Yak Multigroup Cross Section Library (Also including some functions for checking and
  # recalculations)
  def __init__(self, inputFiles):
    ...
    # dictionaries for libraries of tabulated xs values
    self.libs = {}
    ...
    self.defaultKappa = 195 * 1.6 * 10**(-13)
    # alias to XML node dict
    self.aliases = {}
    self.validReactions = [
        'Total',
        'Fission',
    ...
    # root element tag is always the same for Yak XS format
    self.level0Element = 'Multigroup_Cross_Section_Libraries'
    # level 1 element tag is always Multigroup_Cross_Section_Library
    self.level1Element = 'Multigroup_Cross_Section_Library'
    # These are some of the level 2 element tag with string vector xmlnode.text, without xml subnodes
    self.level2Element = [
        'Tabulation', 'AllReactions', 'TablewiseReactions',
        'LibrarywiseReactions'
    ]
    # list of XML nodes that need to be read.
    self.toBeReadXML = []
    # dict to store library keys: {mglib_ID:{gridIndex:{IsotopeName:[reactions]}}}
    self.libsKeys = {}
    ...
    for xmlFile in inputFiles:
      ...
      if root.tag == self.level0Element:
        ...
        self.nGroup = int(root.attrib['NGroup'])
        for mgLib in root:
  ...
  def initialize(self, aliasFiles):
    ...
    for xmlFile in aliasFiles:
      ...
      if root.tag != self.level0Element:
        raise IOError('Invalid root tag: ' + root.tag + ' is provided.' +
                      ' The valid root tag should be: ' + self.level0Element)
      rootName = root.attrib['Name'].strip()
      if rootName in self.aliases.keys():
        raise IOError('Duplicated libraries name: ' + rootName +
                      ' is found in provided alias files!')
      ...
      if child.tag in self.perturbableReactions:
        ...
        if groupIndex == None:
          ...
          if len(varsList) != aliasXSGroup:
            msg = str(
                aliasXSGroup
            ) + ' variables should be provided for ' + child.tag + ' of material ' + child.attrib['mat']
  ...
  def _readYakXSInternal(self, library, pDict, keyDict):
    """
      ...
      @ In, library, xml.etree.ElementTree.Element, xml element for cross section library defined in yak cross
             section files
      @ In, pDict, dict, dictionary to store the multigroup library
      @ In, keyDict, dict, dictionary to store the multigroup library node names, use to trace the cross section
             types for given isotope at given gridIndex
      @ Out, None
    """
    ...
    if xmlNode.tag in self.level2Element:
      ...
      for child in xmlNode:
        if child != None:
          ...
          break
      if hasSubNode:
        self._readIsotopeXS(xmlNode, pDict[xmlNode.attrib['Name'].strip()],
                            keyDict[xmlNode.attrib['Name'].strip()])
    ...
    if has_profile:
      for child in node:
        if child.tag == 'Profile':
          ...
          # store in 1-D array
          scatteringValue = self._stringSpacesToNumpyArray(child.text)
      pDict['Scattering'] = np.zeros((self.nGroup * (orderScattering + 1),
                                      self.nGroup))
      ip = 0
      for l in range(orderScattering + 1):
        for g in range(self.nGroup):
          for gr in range(pDict['ScatterStart'][g + l * self.nGroup] - 1,
                          pDict['ScatterEnd'][g + l * self.nGroup]):
            pDict['Scattering'][g + l * self.nGroup][gr] = scatteringValue[ip]
            ip += 1
    else:
      # store in 1-D array
      scatteringValue = self._stringSpacesToNumpyArray(child.text)
      pDict[child.tag] = scatteringValue.reshape(
          (self.nGroup * (orderScattering + 1), self.nGroup))
  ...
  def _readAdditionalYakXS(self, xmlNode, pDict):
    ...
    if len(self.toBeReadXML) != 0:
      raise IOError('The following nodes xml' + str(self.toBeReadXML) +
  ...
  def _checkYakXS(self, pDict, keyDict):
    """
      ...
      @ In, keyDict, dict, dictionary stores the multigroup library node names, use to trace the cross section types
             for given isotope at given gridIndex
      @ Out, None
    """
      ...
      for isotopeKey, reactionList in isotopeDict.items():
        if len(reactionList) == 0:
          if 'Tablewise' in pDict[gridKey].keys():
            pDict[gridKey][isotopeKey] = copy.deepcopy(
                pDict[gridKey]['Tablewise'])
          elif 'Librarywise' in pDict[gridKey].keys():
            pDict[gridKey][isotopeKey] = copy.deepcopy(
                pDict[gridKey]['Librarywise'])
          else:
            raise IOError(
                'The isotope: ' + isotopeKey +
                ' is provided, but the required cross sections are not found in the library!'
            )
        else:
          # add the missing cross sections from the Tablewise or Librarywise dictionary. Tablewise first, and then
          # Librarywise
          if 'Tablewise' in pDict[gridKey].keys():
  ...
  def _recalculateYakXS(self, reactionDict):
    ...
    if 'nuFission' in reactionList:
      if 'Fission' not in reactionList:
        ...
        for i in range(self.nGroup):
          if reactionDict['Fission'][i] != 0:
            nu.append(
                reactionDict['nuFission'][i] / reactionDict['Fission'][i])
          else:
      ...
      if 'kappaFission' not in reactionList:
        ...
        reactionDict['kappaFission'] = self.defaultKappa * reactionDict[
            'Fission']
        ...
        for i in range(self.nGroup):
          if reactionDict['Fission'][i] != 0:
            kappa.append(
                reactionDict['kappaFission'][i] / reactionDict['Fission'][i])
          else:
    ...
    if 'Total' not in reactionList:
      if 'Transport' not in reactionList:
        raise IOError(
            'Total and Transport cross sections are not found in the cross section input file, at least one of them should be provided!'
        )
      else:
        ...
        if 'Scattering' not in reactionList:
          ...
          reactionDict['Total'] = reactionDict['Transport'] + np.sum(
              reactionDict['Scattering'][self.nGroup:2 * self.nGroup])
    else:
      if 'Transport' not in reactionList:
        ...
        if 'Scattering' not in reactionList:
          ...
          reactionDict['Transport'] = reactionDict['Total'] - np.sum(
              reactionDict['Scattering'][self.nGroup:2 * self.nGroup])

    # Metod 1: Currently, rattlesnake will not check the consistent of provided cross sections, rattlesnake will
    # only use Total,
    ...
    if 'Scattering' in reactionList:
      reactionDict[
          'Absorption'] = reactionDict['Total'] - reactionDict['TotalScattering']
    else:
      if self.nGroup == 1 and 'Absorption' in reactionList:
        reactionDict[
            'Scattering'] = reactionDict['Total'] - reactionDict['Absorption']
    ...
    if 'nuFission' in reactionList:
      reactionDict[
          'Capture'] = reactionDict['Absorption'] - reactionDict['Fission']
    else:
  ...
  def perturb(self, **Kwargs):
    ...
    for libsKey, libDict in pertFactor.items():
      ...
      for libID, gridDict in libDict.items():
        self._rebalanceXS(self.pertLib[libsKey][libID], gridDict,
                          pertFactor[libsKey][libID], aliasType)
      ...
      if type(libValue) == dict:
        ...
        for var in libValue:
          if var in self.modDict.keys():
            ...
            raise IOError('The user wants to perturb ' + var +
                          ', but this variable is not defined in the Sampler!')
        groupValues = np.asarray(groupValues)
        factors[libKey] = groupValues
        if not lib['perturbTransport'] and libKey == 'Transport':
          raise IOError(
              'Transport can not be perturbed since it does not exist in the input XS library!'
          )
        if aliasType == 'rel':
  ...
  def _rebalanceXS(self, libDict, libKeyDict, factorDict, aliasType):
    ...
    for gridKey, isotopeDict in libKeyDict.items():
      for isotopeKey, reactionList in isotopeDict.items():
        ...
        self._rebalanceYakXS(libDict[gridKey][isotopeKey],
                             factorDict[gridKey][isotopeKey], aliasType)
  ...
  def _rebalanceYakXS(self, reactionDict, perturbDict, aliasType):
    ...
    if 'Fission' in reactionDict.keys():
      ...
      reactionDict['kappaFission'] = reactionDict['Fission'] * reactionDict[
          'Kappa']
      reactionDict[
          'Absorption'] = reactionDict['Fission'] + reactionDict['Capture']
    else:
      reactionDict['Absorption'] = copy.copy(reactionDict['Capture'])
    reactionDict[
        'Total'] = reactionDict['Absorption'] + reactionDict['TotalScattering']
    if hasTotalScattering:
      #total scattering are perturbed
      #recalculate Scattering Cross Sections
      for g in range(self.nGroup):
        if aliasType == 'rel':
          reactionDict['Scattering'][0:self.nGroup, g] *= perturbDict[
              'TotalScattering'][g]
        elif aliasType == 'abs':
    ...
    reactionDict['Removal'] = np.asarray(
        list(reactionDict['Total'][g] - reactionDict['Scattering'][g][g]
             for g in range(self.nGroup)))
    #recalculate Transport cross sections
    if not reactionDict['perturbTransport']:
      if reactionDict['Scattering'].shape[0] >= self.nGroup * 2:
        reactionDict['Transport'] = reactionDict['Total'] - np.sum(
            reactionDict['Scattering'][self.nGroup:self.nGroup * 2])
      else:
        #recalculate Transport cross sections
        reactionDict['Transport'] = copy.copy(reactionDict['Total'])
          ...
          if childChild.tag == 'Value':
            ...
            for g in range(reactionDict[child.tag].shape[0]):
              msg = msg + '\n' + '            ' + ' '.join([
                  ...
                  [reactionDict['ScatterStart'][g] - 1:
                   reactionDict['ScatterEnd'][g]]
              ])
            childChild.text = msg + '\n'
            ...
            for subNode in table:
              if subNode.tag == 'Isotope':
                ...
                if mat not in self.aliases[libsKey][libID][gridIndex].keys():
                  continue
                self._replaceXMLNodeText(
                    subNode, self.pertLib[libsKey][libID][gridIndex][mat])
