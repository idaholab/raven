...
# WARNING if you import unicode_literals here, we fail tests (e.g. framework.testFactorials). This may be a
# future-proofing problem. 2015-04.
import warnings
...
class csvUtilityClass(object):
  ...
  def __init__(self,
    ...
    """
      Constructor
      @ In, listOfFiles, list, list of CSV files that need to be merged. If in one or more "filenames" the special
             symbol $*$ is present, the class will use the filename as root name and look for all the files with
             that root. For example:
                                             if  listOfFiles[1] == "aPath/outputChannel$*$":
      the code will inquire the directory "aPath" to look for all the files starting with the name "outputChannel"
      => at end we will have a list of files like "outputChannel_1.csv,outputChannel_ab.csv, etc"
      ...
      @ In, mergeSameVariables, bool, optional, do variables with the same name need to be merged together ? (aka,
             take only the values of the first occurence)?
      @ Out, None
    """
    if len(listOfFiles) == 0:
      raise IOError(
          "MergeCSV class ERROR: the number of CSV files provided is equal to 0!! it can not merge anything!"
      )
    # list of files
    self.listOfFiles = []
    # dictionary that is going to contain all the data from the multiple CSVs
    self.dataContainer = {}
    # it containes all the headers
    self.allHeaders = []
    filePathToExpand = []
    ...
    for filename in self.listOfFiles:
      ...
      if all_field_names[-1] == "":
        # it means there is a trailing "'" at the end of the file
        all_field_names.pop(-1)
      isAlreadyIn = False
  ...
  def mergeCSV(self, outputFileName, options={}):
    """
      ...
      @ In, outputFileName, string, full path of the resulting merged CSV (output file name, eg.
             /users/userName/output.csv)
      @ In, options, dict, optional, dictionary of options: { "variablesToExpandFrom":"aVariable" (a variable
             through which the "shorter" CSVs need to be expanded)
                                                              "sameKeySuffix":"integerCounter or filename (default)" (if in the CSVs that need to be merged there are
      multiple occurrences of the same key, the code will append either a letter (A,B,C,D,etc) or an integer counter
      (1,2,3,etc)
  ...
  def mergeCsvAndReturnOutput(self, options={}):
    """
      ...
      multiple occurrences of the same key, the code will append either a letter (A,B,C,D,etc) or an integer counter
      (1,2,3,etc)
      "returnAsDict":True/False, True if the merged values need to be returned as a dictionary, otherwise it returns
      a tuple
                                                            }
      @ Out, mergedReturn, dict or tuple, merged csvs values (see "returnAsDict" option above to understand what you
             get)
    """
    # set some default
        ...
        if headerCounts[head] > 1 and head not in variablesToExpandFrom:
          if not self.mergeSameVariables:
            ...
            if sameKeySuffix == "filename":
              self.dataContainer[filename]["headers"][
                  cnt] = head + "_" + os.path.split(filename)[-1].split(".")[0]
            else:
              headerAppender[variable] += 1
              self.dataContainer[filename]["headers"][cnt] = head + "_" + str(
                  headerAppender[variable])
