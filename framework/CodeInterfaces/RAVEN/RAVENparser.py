...
class RAVENparser():
  ...
  def returnOutstreamsNamesAnType(self):
    """
      ...
      @ In, None
      @ Out, outStreamsNames, dict, the dictionary of outstreams of type print
             {'outStreamName':[DataObjectName,DataObjectType]}
    """
  ...
  def modifyOrAdd(self, modiDictionary={}, save=True, allowAdd=False):
    ...
    """
    if save:
      # make a copy if save is requested
      returnElement = copy.deepcopy(self.tree)
    else:
      # otherwise return the original modified
      returnElement = self.tree
      ...
      if len(foundNodes) == 0:
        ...
        if mathUtils.isSingleValued(val):
          ...
          if len(val.shape) > 1:
            raise IOError(
                self.printTag +
                'ERROR: RAVEN interface is not prepared to handle matrix value passing yet!'
            )
