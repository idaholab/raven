...
class RAVEN(CodeInterfaceBase):
  ...
  def _readMoreXML(self, xmlNode):
    """
      ...
      read specific flags.
      In this case, this is used for locating an external python module where the variables can be modified and the
      "vector" variables
    ...
    # {modulePath : {'variables': [], 'noScalar': 0, 'scalar': 0}, etc }
    self.conversionDict = {}
    ...
    if child is not None:
      for moduleNode in child:
        ...
        if source is None:
          raise IOError(
              self.printTag +
              ' ERROR: no module "source" listed in "conversion" subnode attributes!'
          )
        # fix up the path
        ...
        if not os.path.exists(source):
          raise IOError(self.printTag +
                        ' ERROR: the conversionModule "{}" was not found!'
        ...
        if checkImport is None:
          raise IOError(self.printTag +
                        ' ERROR: the conversionModule "{}" failed on import!'
                        .format(self.extModForVarsManipulationPath))
        ...
        if not (noScalar or scalar):
          raise IOError(
              self.printTag + ' ERROR: the conversionModule "' + source +
        ...
        if varNode is None:
          raise IOError(
              self.printTag +
              ' ERROR: no node "variables" listed in "conversion|module" subnode!'
          )
        variables = [x.strip() for x in varNode.text.split(',')]
        self.conversionDict[source] = {
            'variables': variables,
            'noScalar': noScalar,
            'scalar': scalar
        }
  ...
  def __findInputFile(self, inputFiles):
    """
      ...
      @ In, inputFiles, list, List of input files (length of the list depends on the number of inputs that have been
    ...
    for index, inputFile in enumerate(inputFiles):
      if inputFile.getType().lower() == 'raven':
        ...
        if found:
          raise IOError(
              self.printTag +
              " ERROR: Currently the RAVEN interface allows only one input file (xml). ExternalXML and Merging Files will be added in the future!"
          )
        found = True
    if not found:
      raise IOError(
          self.printTag +
          ' ERROR: None of the input files are tagged with the "type" "raven" (e.g. <Input name="aName" type="raven">inputFileName.xml</Input>)'
      )
  ...
  def createNewInput(self, currentInputFiles, oriInputFiles, samplerType,
    ...
    for source, convDict in self.conversionDict.items():
      ...
      varVals = dict(
          (var, np.asarray(modifDict[var])) for var in convDict['variables'])
  ...
  def checkForOutputFailure(self, output, workingDir):
    """
      ...
      This can happen in those codes that record the failure of the job (e.g. not converged, etc.) as normal
      termination (returncode == 0)
      This method can be used, for example, to parse the outputfile looking for a special keyword that testifies
      that a particular job got failed
  ...
  def finalizeCodeOutput(self, command, output, workingDir):
    """
      ...
      @ In, output, string, the Output name root
      @ In, workingDir, string, current working dir
      @ Out, dataObjectsToReturn, dict, optional, this is a special case for RAVEN only. It returns the constructed
             dataobjects
    ...
    # The return dict for this CodeInterface is a dictionary of data objects (either one or two of them, up to one
    # each point set and history set).
    # Normally, the end result of this method is producing a CSV file with the data to load.
    # When data objects are returned, this triggers an "if" path in Models/Code.py in evaluateSample(). There's an
    # "else"
