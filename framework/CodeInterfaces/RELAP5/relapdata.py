...
class relapdata:
  ...
  def __init__(self, filen, deckNumber=-1):
    """
      ...
      @ In, deckNumber, int, optional, the deckNumber from which the outputs need to be retrieved (default is the
             last)
      @ Out, None
    """
  ...
  def getTimeDeck(self, lines, deckNumber=-1):
    """
      ...
      @ In, deckNumber, int, optional, the deckNumber from which the outputs need to be retrieved (default is the
             last)
      @ Out, times, dict, dict containing the information
             {'deckNumber':{'time':float,'sliceCoordinates':tuple(startLine,EndLine)}}. Dict of final times and
             corresponding deck start and end line number
    """
    times = {}
    ...
    for cnt, line in enumerate(lines):
      if re.match('^\s*Final time=', line) or re.match('^\s*0Final time=',
        ...
        times[deckNum] = {
            'time': line.split()[2],
            'sliceCoordinates': (startLineNumber, endLineNumber)
        }
  ...
  def checkLine(self, lineList):
    """
      ...
      @ Out, outcome, bool, boolean variable which is True if the list contains only numbers, False if the contains
             at list a string
    """
    outcome = True
  ...
  def writeCSV(self, filen):
    ...
    for i in range(len(self.minordata.get(self.minordata.keys()[0]))):
      IOcsvfile.write(','.join(
          self.minordata.get(self.minordata.keys()[j])[i]
