...
class Relap5(CodeInterfaceBase):
  ...
  def _readMoreXML(self, xmlNode):
    """
      ...
      read specific flags.
      Only one option is possible. You can choose here, if multi-deck mode is activated, from which deck you want to
      load the results
      @ In, xmlNode, xml.etree.ElementTree.Element, Xml element node
      @ Out, None.
    """
    # default is the last deck!
    self.outputDeck = -1
  ...
  def generateCommand(self, inputFiles, executable, clargs=None, fargs=None):
    """
      ...
      @ In, fargs, dict, optional, a dictionary containing the auxiliary input file variables the user can specify
    ...
    if clargs:
      ...
      addflags = ''
    # commandToRun = executable + ' -i ' + inputFiles[index].getFilename() + ' -o ' + outputfile + '.o' + ' -r ' +
    # outputfile + '.r' + addflags
    commandToRun = executable + ' -i ' + inputFiles[index].getFilename(
    ) + ' -o ' + outputfile + '.o ' + addflags
    ...
    for keys in Kwargs['SampledVars']:
      ...
      if len(multiDeck) > 1:
        ...
        try:
          ...
          raise IOError(
              "RELAP5 interface: activated multi-deck/case approach but the deck number is not an integer (first word followed by '|' symbol). Got "
              + str(deck))
        deckActivated = True
      ...
      if deck is None:
        ...
        if deckActivated:
          raise IOError(
              "If the multi-deck/case approach gets activated, all the variables need to provide a DECK ID. E.g. deckNumber|card|word ! Wrong variable is "
              + card)
