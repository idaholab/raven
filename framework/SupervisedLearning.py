...
class superVisedLearning(utils.metaclass_insert(abc.ABCMeta)):
  ...
  def __init__(self,**kwargs):
    ...
    if 'Features' not in self.initOptionDict.keys(): self.raiseAnError(IOError,self,'Feature names not provided')
    if 'Target'   not in self.initOptionDict.keys(): self.raiseAnError(IOError,self,'Target name not provided')
    ...
    if self.features.count(self.target) > 0: self.raiseAnError(IOError,self,'The target and one of the features have the same name!')
  ...
  def train(self,tdict):
    '''
      ...
      @ Out, None
    '''
    if type(tdict) != dict: self.raiseAnError(TypeError,self,'In method "train", the training set needs to be provided through a dictionary. Type of the in-object is ' + str(type(tdict)))
    names, values  = list(tdict.keys()), list(tdict.values())
    if self.target in names: targetValues = values[names.index(self.target)]
    else                   : self.raiseAnError(IOError,self,'The output sought '+self.target+' is not in the training set')
    # check if the targetValues are consistent with the expected structure
    resp = self.checkArrayConsistency(targetValues)
    if not resp[0]: self.raiseAnError(IOError,self,'In training set for target '+self.target+':'+resp[1])
    # construct the evaluation matrixes
    featureValues = np.zeros(shape=(targetValues.size,len(self.features)))
    for cnt, feat in enumerate(self.features):
      if feat not in names: self.raiseAnError(IOError,self,'The feature sought '+feat+' is not in the training set')
      else:
        resp = self.checkArrayConsistency(values[names.index(feat)])
        if not resp[0]: self.raiseAnError(IOError,self,'In training set for feature '+feat+':'+resp[1])
        if values[names.index(feat)].size != featureValues[:,0].size: self.raiseAnError(IOError,self,'In training set, the number of values provided for feature '+feat+' are != number of target outcomes!')
  ...
  def confidence(self,edict):
    ...
    '''
    if type(edict) != dict: self.raiseAnError(IOError,self,'method "confidence". The inquiring set needs to be provided through a dictionary. Type of the in-object is ' + str(type(edict)))
    names, values   = list(edict.keys()), list(edict.values())
    for index in range(len(values)):
      resp = self.checkArrayConsistency(values[index])
      if not resp[0]: self.raiseAnError(IOError,self,'In evaluate request for feature '+names[index]+':'+resp[1])
    featureValues = np.zeros(shape=(values[0].size,len(self.features)))
    for cnt, feat in enumerate(self.features):
      if feat not in names: self.raiseAnError(IOError,self,'The feature sought '+feat+' is not in the evaluate set')
      else:
        resp = self.checkArrayConsistency(values[names.index(feat)])
        if not resp[0]: self.raiseAnError(IOError,self,'In training set for feature '+feat+':'+resp[1])
  ...
  def evaluate(self,edict):
    ...
    '''
    if type(edict) != dict: self.raiseAnError(IOError,self,'method "evaluate". The evaluate request/s need/s to be provided through a dictionary. Type of the in-object is ' + str(type(edict)))
    names, values  = list(edict.keys()), list(edict.values())
    for index in range(len(values)):
      resp = self.checkArrayConsistency(values[index])
      if not resp[0]: self.raiseAnError(IOError,self,'In evaluate request for feature '+names[index]+':'+resp[1])
...
class NDinterpolatorRom(superVisedLearning):
  ...
  def __confidenceLocal__(self,featureVals):
    self.raiseAnError(NotImplementedError,self,'NDinterpRom   : __confidenceLocal__ method must be implemented!')
  ...
  def __evaluateLocal__(self,featureVals):
    ...
    for n_sample in range(featureVals.shape[0]):
      ...
      self.raiseAMessage(self,'NDinterpRom   : Prediction by ' + self.__class__.ROMtype + '. Predicted value is ' + str(prediction[n_sample]))
    return prediction
  ...
  def __returnCurrentSettingLocal__(self):
    self.raiseAnError(NotImplementedError,self,'NDinterpRom   : __returnCurrentSettingLocal__ method must be implemented!')
...
class GaussPolynomialRom(NDinterpolatorRom):
      ...
      if key=='Interpolation':
        for var,val in val.items():
          ...
          for atrName,atrVal in val.items():
            ...
            else: self.raiseAnError(IOError,self,'Unrecognized option: '+atrName)

    if not self.indexSetType:
      self.raiseAnError(IOError,self,'No IndexSet specified!')
    if not self.maxPolyOrder:
      self.raiseAnError(IOError,self,'No maxPolyOrder specified!')
    if self.maxPolyOrder < 1:
      self.raiseAnError(IOError,self,'Polynomial order cannot be less than 1 currently.')
  ...
  def __trainLocal__(self,featureVals,targetVals):
    ...
    if len(featureVals)!=len(self.sparseGrid):
      self.raiseAnError(IOError,self,'ROM requires '+str(len(self.sparseGrid))+' points, but '+str(len(featureVals))+' provided!')
    ...
    if not np.allclose(fvs,sgs,rtol=1e-15):
      ...
      self.raiseADebug(self,msg,verbosity='silent')
      self.raiseAnError(IOError,self,'input values do not match required values!')
...
class HDMRRom(GaussPolynomialRom):
  ...
  def __evaluateLocal__(self,featureVals):
    ...
    #am I trained?
    if not self.amITrained: self.raiseAnError(IOError,self,'Cannot evaluate, as ROM is not trained!')
  ...
  def getSensitivities(self,maxLevel=None,kind='variance'):
    ...
    if maxLevel==None: maxLevel = avail
      ...
      if maxLevels>avail: raiseAnError(IOError,self,'Requested level %i for sensitivity analyis, but this composition is at most %i order!' %(maxLevel,avail) )
...
class NDmicroSphere(NDinterpolatorRom):
  ...
  def __init__(self,**kwargs):
    ...
    if not 'p' in self.initOptionDict.keys(): self.raiseAnError(IOError,self,'the <p> parameter must be provided in order to use NDmicroSphere as ROM!!!!')
    if not 'precision' in self.initOptionDict.keys(): self.raiseAnError(IOError,self,'the <precision> parameter must be provided in order to use NDmicroSphere as ROM!!!!')
...
class SciKitLearn(superVisedLearning):
  ...
  def __init__(self,**kwargs):
    ...
    if 'SKLtype' not in self.initOptionDict.keys(): self.raiseAnError(IOError,self,'to define a scikit learn ROM the SKLtype keyword is needed (from ROM '+self.name+')')
    SKLtype, SKLsubType = self.initOptionDict['SKLtype'].split('|')
    self.initOptionDict.pop('SKLtype')
    if not SKLtype in self.__class__.availImpl.keys(): self.raiseAnError(IOError,self,'not known SKLtype ' + SKLtype +'(from ROM '+self.name+')')
    if not SKLsubType in self.__class__.availImpl[SKLtype].keys(): self.raiseAnError(IOError,self,'not known SKLsubType ' + SKLsubType +'(from ROM '+self.name+')')
  ...
  def __confidenceLocal__(self,edict):
    ...
    else            : self.raiseAnError(IOError,self,'the ROM '+str(self.name)+'has not the an method to evaluate the confidence of the prediction')
  ...
  def __returnCurrentSettingLocal__(self):
    self.raiseAMessage(self,'here we need to collect some info on the ROM status','FIXME')
    localInitParam = {}
    return localInitParam
#
#
...
def returnInstance(ROMclass,caller,**kwargs):
  ...
  except KeyError: caller.raiseAnError(NameError,'SUPERVISEDLEARNING','not known '+__base+' type '+str(ROMclass))

def returnClass(ROMclass,caller):
  '''This function return an instance of the request model type'''
  try: return __interfaceDict[ROMclass]
  except KeyError: caller.raiseAnError(NameError,'SUPERVISEDLEARNING','not known '+__base+' type '+ROMclass)
