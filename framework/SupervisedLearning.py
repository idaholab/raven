...
class SciKitLearn(supervisedLearning):
  ...
  availImpl = {
  }  # dictionary of available ROMs {mainClass:{subtype:(classPointer,Output type (float or int), boolean -> External Z-normalization needed)}
...
class ARMA(supervisedLearning):
  ...
  def __generateCDF__(self, data):
    ...
    num_bins = [0] * data.shape[
        1]  # initialize num_bins, which will be calculated later by Freedman Diacoins rule
