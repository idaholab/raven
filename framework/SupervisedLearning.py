...
class supervisedLearning(
  ...
  """
  # this describe the type of information generated the possibility are 'boolean', 'integer', 'float'
  returnType = ''
  qualityEstType = [
  ...
  # the broad class of the interpolator
  ROMtype = ''
  ROMmultiTarget = False
  # is this ROM able to treat time-like (any monotonic variable) explicitly in its formulation?
  ROMtimeDependent = False
  ...
  def checkArrayConsistency(arrayIn, isDynamic=False):
    """
      ...
      @ Out, (consistent, 'error msg'), tuple, tuple[0] is a bool (True -> everything is ok, False -> something
             wrong), tuple[1], string ,the error mesg
    """
  ...
  def initialize(self, idict):
    """
      ...
      @ Out, None
    """
    # Overloaded by (at least) GaussPolynomialRom
    pass

  def train(self, tdict):
    """
      Method to perform the training of the supervisedLearning algorithm
      NB.the supervisedLearning object is committed to convert the dictionary that is passed (in), into the local
      format
  ...
  def returnCurrentSetting(self):
    ...
    """
    currParDict = dict({
        'Trained': self.amITrained
    }.items() + self.__CurrentSettingDictLocal__().items())
    return currParDict
  ...
  def __confidenceLocal__(self, featureVals):
    """
      ...
      This could be distance or probability or anything else, the type needs to be declared in the variable
      cls.qualityEstType
...
class GaussPolynomialRom(supervisedLearning):
  ...
  def __init__(self, messageHandler, **kwargs):
    ...
    """
    supervisedLearning.__init__(self, messageHandler, **kwargs)
    # only True once self.initialize has been called
    self.initialized = False
    # FIXME what's this?
    self.interpolator = None
    self.printTag = 'GAUSSgpcROM(' + '-'.join(self.target) + ')'
    # string of index set type, TensorProduct or TotalDegree or HyperbolicCross
    self.indexSetType = None
    # list of tuples, custom index set to use if CustomSet is the index set type
    self.indexSetVals = []
    # integer of relative maximum polynomial order to use in any one dimension
    self.maxPolyOrder = None
    # dict of quad,poly,weight choices keyed on varName
    self.itpDict = {}
    self.norm = None  #combined distribution normalization factors (product)
    # Quadratures.SparseGrid object, has points and weights
    self.sparseGrid = None
    # dict{varName: Distribution object}, has point conversion methods based on quadrature
    self.distDict = None
    # dict{varName: Quadrature object}, has keys for distribution's point conversion methods
    self.quads = None
    # dict{varName: OrthoPolynomial object}, has polynomials for evaluation
    self.polys = None
    # array of tuples, polynomial order combinations
    self.indexSet = None
    # dict{index set point, float}, polynomial combination coefficients for each combination
    self.polyCoeffDict = None
    self.numRuns = None  #number of runs to generate ROM; default is len(self.sparseGrid)
    # dict{varName: dict{attribName:value} }
    self.itpDict = {}
    # list of feature variables
    self.featv = None
    # list of target variables
    ...
    # type of sparse quadrature to use,default smolyak
    self.sparseGridType = 'smolyak'
    # choice of sparse quadrature construction methods
    self.sparseQuadOptions = ['smolyak', 'tensor']
  ...
  def checkForNonzeros(self, tol=1e-12):
    """
      ...
      @ Out, data, dict, {'target1':list(tuple),'target2':list(tuple)}: the indices and values of the nonzero
...
class HDMRRom(GaussPolynomialRom):
  ...
  def __init__(self, messageHandler, **kwargs):
    ...
    # true only when self.initialize has been called
    self.initialized = False
    self.printTag = 'HDMR_ROM(' + '-'.join(self.target) + ')'
    # depth of HDMR/Sobol expansion
    ...
    # mean, store to avoid recalculation
    self.mean = None
    # variance, store to avoid recalculation
    self.variance = None
    # converted true ANOVA terms, stores coefficients not polynomials
    self.anova = None
    # partial variance contributions
    self.partialVariances = None
  ...
  def __trainLocal__(self, featureVals, targetVals):
    ...
    # each entry will look like 'x1,x2':('x1','x2'), missing the reference entry
    self.terms = {(): []}
...
class pickledROM(supervisedLearning):
  ...
  def __evaluateLocal__(self, featureVals):
    ...
    self.raiseAnError(
        RuntimeError,
        'PickledROM has not been loaded from file yet!  An IO step is required to perform this action.'
    )
...
class MSR(NDinterpolatorRom):
  ...
  def __init__(self, messageHandler, **kwargs):
    ...
    # Gradient estimate methodology
    self.gradient = 'steepest'
    # Neighborhood graph used
    self.graph = 'beta skeleton'
    # beta used in the beta skeleton graph
    self.beta = 1
    #  and its relaxed version
    # k-nearest neighbor value for either
    self.knn = -1
    ...
    # Morse-smale simplification amount
    ...
    # Strategy for merging topo partitions
    self.persistence = 'difference'
    # Should the linear models be weighted
    self.weighted = False
    #  by probability information?
    # Should any normalization be
    self.normalization = None
    #  performed within the AMSC? No, this
    #  data should already be standardized
    # The method used to predict the label
    self.partitionPredictor = 'kde'
    #  of each query point (can be soft).
    # Flag: blend the predictions
    self.blending = False
    #  depending on soft label predictions
    #  or use only the most likely local
    #  model
    # What kernel should be used in the
    self.kernel = 'gaussian'
    #  kde approach
    # The bandwidth for the kde approach
    self.bandwidth = 1.
      ...
      if key.lower() == 'graph':
        ...
        try:
          ...
          if graph.endswith('beta skeleton'):
            self.raiseAnError(IOError, 'Requested invalid beta value:', val,
                              '(Allowable range: (0,2])')
          else:
        ...
        try:
          ...
          self.raiseAnError(IOError, 'Requested invalid knn value:', val,
                            '(Should be an integer value, knn <= 0 implies',
  ...
  def __evaluateLocal__(self, featureVals):
    """
      ...
      @ In, featureVals, numpy.array 2-D, features
      @ Out, returnDict, dict, dict of predicted values for each target ({'target1':numpy.array
             1-D,'target2':numpy.array 1-D}
    """
    returnDict = {}
    for index, target in enumerate(self.target):
      if self.partitionPredictor == 'kde':
...
class NDsplineRom(NDinterpolatorRom):
  ...
  def __trainLocal__(self, featureVals, targetVals):
    ...
    if not tensorGrid:
      ...
      newDiscretizations = [
          np.linspace(
              min(discretizations[d]),
              max(discretizations[d]),
              num=numDiscrPerDimension,
...
class SciKitLearn(supervisedLearning):
  ...
  # dictionary of available ROMs {mainClass:{subtype:(classPointer,Output type (float or int), boolean -> External
  # Z-normalization needed)}
  availImpl = {}
  ...
  availImpl['qda'] = {}
  if int(sklearn.__version__.split(".")[1]) > 16:
    availImpl['lda']['LDA'] = (da.LinearDiscriminantAnalysis, 'int',
  ...
  availImpl['linear_model'] = {}
  # Bayesian ARD regression.
  availImpl['linear_model']['ARDRegression'] = (linear_model.ARDRegression,
                                                'float', False)
  # Bayesian ridge regression
  availImpl['linear_model']['BayesianRidge'] = (linear_model.BayesianRidge,
                                                'float', False)
  # Linear Model trained with L1 and L2 prior as regularizer
  availImpl['linear_model']['ElasticNet'] = (linear_model.ElasticNet, 'float',
                                             False)
  # Elastic Net model with iterative fitting along a regularization path
  availImpl['linear_model']['ElasticNetCV'] = (linear_model.ElasticNetCV,
                                               'float', False)
  # Least Angle Regression model a.k.a.
  availImpl['linear_model']['Lars'] = (linear_model.Lars, 'float', False)
  # Cross-validated Least Angle Regression model
  availImpl['linear_model']['LarsCV'] = (linear_model.LarsCV, 'float', False)
  availImpl['linear_model']['Lasso'] = (
      linear_model.Lasso, 'float', False
  )  #Linear Model trained with L1 prior as regularizer (aka the Lasso)
  # Lasso linear model with iterative fitting along a regularization path
  availImpl['linear_model']['LassoCV'] = (linear_model.LassoCV, 'float', False)
  # Lasso model fit with Least Angle Regression a.k.a.
  availImpl['linear_model']['LassoLars'] = (linear_model.LassoLars, 'float',
                                            False)
  # Cross-validated Lasso, using the LARS algorithm
  availImpl['linear_model']['LassoLarsCV'] = (linear_model.LassoLarsCV,
                                              'float', False)
  # Lasso model fit with Lars using BIC or AIC for model selection
  availImpl['linear_model']['LassoLarsIC'] = (linear_model.LassoLarsIC,
                                              'float', False)
  # Ordinary least squares Linear Regression.
  availImpl['linear_model']['LinearRegression'] = (
      linear_model.LinearRegression, 'float', False)
  # Logistic Regression (aka logit, MaxEnt) classifier.
  availImpl['linear_model']['LogisticRegression'] = (
      linear_model.LogisticRegression, 'float', True)
  # Multi-task Lasso model trained with L1/L2 mixed-norm as regularizer
  availImpl['linear_model']['MultiTaskLasso'] = (linear_model.MultiTaskLasso,
                                                 'float', False)
  ...
  availImpl['linear_model']['OrthogonalMatchingPursuitCV'] = (
  ...
  availImpl['linear_model']['PassiveAggressiveClassifier'] = (
      linear_model.PassiveAggressiveClassifier, 'int', True)
  # Passive Aggressive Regressor
  availImpl['linear_model']['PassiveAggressiveRegressor'] = (
      linear_model.PassiveAggressiveRegressor, 'float', True)
  # Perceptron
  availImpl['linear_model']['Perceptron'] = (linear_model.Perceptron, 'float',
                                             True)
  # Linear least squares with l2 regularization.
  availImpl['linear_model']['Ridge'] = (linear_model.Ridge, 'float', False)
  # Classifier using Ridge regression.
  availImpl['linear_model']['RidgeClassifier'] = (linear_model.RidgeClassifier,
                                                  'float', False)
  # Ridge classifier with built-in cross-validation.
  availImpl['linear_model']['RidgeClassifierCV'] = (
      linear_model.RidgeClassifierCV, 'int', False)
  # Ridge regression with built-in cross-validation.
  availImpl['linear_model']['RidgeCV'] = (linear_model.RidgeCV, 'float', False)
  # Linear classifiers (SVM, logistic regression, a.o.) with SGD training.
  availImpl['linear_model']['SGDClassifier'] = (linear_model.SGDClassifier,
                                                'int', True)
  # Linear model fitted by minimizing a regularized empirical loss with SGD
  availImpl['linear_model']['SGDRegressor'] = (linear_model.SGDRegressor,
                                               'float', True)

  # support Vector Machines
  availImpl['svm'] = {}
  # Linear Support vector classifier
  availImpl['svm']['LinearSVC'] = (svm.LinearSVC, 'bool', True)
  # Support vector classifier
  availImpl['svm']['SVC'] = (svm.SVC, 'bool', True)
  # Nu Support vector classifier
  availImpl['svm']['NuSVC'] = (svm.NuSVC, 'bool', True)
  # Support vector regressor
  availImpl['svm']['SVR'] = (svm.SVR, 'float', True)

  # Multiclass and multilabel classification
  availImpl['multiClass'] = {}
  # One-vs-the-rest (OvR) multiclass/multilabel strategy
  availImpl['multiClass']['OneVsRestClassifier'] = (
      multiclass.OneVsRestClassifier, 'int', False)
  # One-vs-one multiclass strategy
  availImpl['multiClass']['OneVsOneClassifier'] = (
      multiclass.OneVsOneClassifier, 'int', False)
  # (Error-Correcting) Output-Code multiclass strategy
  availImpl['multiClass']['OutputCodeClassifier'] = (
      multiclass.OutputCodeClassifier, 'int', False)
  ...
  availImpl['neighbors']['KNeighborsClassifier'] = (
      neighbors.KNeighborsClassifier, 'int', True)
  ...
  availImpl['neighbors']['KNeighborsRegressor'] = (
      neighbors.KNeighborsRegressor, 'float', True)
  # Regression based on neighbors within a fixed radius.
  availImpl['neighbors']['RadiusNeighborsRegressor'] = (
      neighbors.RadiusNeighborsRegressor, 'float', True)
  # Nearest centroid classifier.
  availImpl['neighbors']['NearestCentroid'] = (neighbors.NearestCentroid,
                                               'int', True)
  # BallTree for fast generalized N-point problems
  availImpl['neighbors']['BallTree'] = (neighbors.BallTree, 'float', True)
  # KDTree for fast generalized N-point problems
  availImpl['neighbors']['KDTree'] = (neighbors.KDTree, 'float', True)

  availImpl['tree'] = {}
  # A decision tree classifier.
  availImpl['tree']['DecisionTreeClassifier'] = (tree.DecisionTreeClassifier,
                                                 'int', True)
  # A tree regressor.
  availImpl['tree']['DecisionTreeRegressor'] = (tree.DecisionTreeRegressor,
                                                'float', True)
  ...
  if int(sklearn.__version__.split(".")[1]) > 17:
    ...
    availImpl['neural_network']['MLPClassifier'] = (
        neural_network.MLPClassifier, 'int', True)
    # Multi-layer perceptron regressor.
    availImpl['neural_network']['MLPRegressor'] = (neural_network.MLPRegressor,
                                                   'float', True)
  ...
  def __trainLocal__(self, featureVals, targetVals):
    """
      ...
      @ Out, targetVals, array, shape = [n_samples,n_targets], an array of output target
        associated with the corresponding points in featureVals
    """
    # If all the target values are the same no training is needed and the moreover the self.evaluate could be
    # re-addressed to this value
    if self.intrinsicMultiTarget:
...
class ARMA(supervisedLearning):
  ...
  def __init__(self, messageHandler, **kwargs):
    ...
    if 'Fourier' not in self.initOptionDict.keys():
      ...
      self.fourierPara['basePeriod'] = [
          float(temp) for temp in self.initOptionDict['Fourier'].split(',')
      ]
  ...
  # This function is not used in this class and can be removed
  def _localNormalizeData(self, values, names, feat):
    """
      Overwrites default normalization procedure.
      @ In, values, unused
  ...
  def __trainLocal__(self, featureVals, targetVals):
    ...
    self.armaPara['rSeriesNorm'] = self.__dataConversion__(
        self.armaPara['rSeries'], obj='normalize')

    # Fit ARMA model: x_t = \sum_{i=1}^P \phi_i*x_{t-i} + \alpha_t + \sum_{j=1}^Q \theta_j*\alpha_{t-j}
    self.__trainARMA__()

    # Delete to reduce the pickle size, since from now the original data will no longer be used in the evaluation.
  ...
  def __generateCDF__(self, data):
    ...
    # initialize num_bins, which will be calculated later by Freedman Diacoins rule
    num_bins = [0] * data.shape[1]
  ...
  def __dataConversion__(self, data, obj):
    """
      ...
      @ Out, transformedData, array, shape = [n_timeStep, n_dimension], output transformed data that has
...
class PolyExponential(supervisedLearning):
  ...
  def __init__(self, messageHandler, **kwargs):
    ...
    supervisedLearning.__init__(self, messageHandler, **kwargs)
    # avail coeff regressors
    self.availCoeffRegressors = ['nearest', 'poly', 'spline']
    # Print tag
    self.printTag = 'PolyExponential'
    # Pivot parameter ID
    self.pivotParameterID = kwargs.get("pivotParameter", "time")
    # This ROM is able to manage the time-series on its own
    self._dynamicHandling = True
    # poly exponential options' container
    self.polyExpParams = {}
    # the number of exponential terms
    self.polyExpParams['expTerms'] = int(kwargs.get('numberExpTerms', 3))
    # which regressor to use for interpolating the coefficient
    self.polyExpParams['coeffRegressor'] = kwargs.get('coeffRegressor',
                                                      'spline').lower()
    # the polynomial order
    self.polyExpParams['polyOrder'] = int(kwargs.get('polyOrder', 3))
    # optimization tolerance
    self.polyExpParams['tol'] = float(kwargs.get('tol', 0.001))
    # maximum number of iterations in optimization
    self.polyExpParams['maxNumberIter'] = int(
        kwargs.get('maxNumberIter', 5000))
    # a_ij coefficients of the exponential terms {'target1':ndarray(nsamples,
    # self.polyExpParams['expTerms']),'target2',ndarray,etc}
    self.aij = None
    # b_ij coefficients of the exponent of the exponential terms {'target1':ndarray(nsamples,
    # self.polyExpParams['expTerms']),'target2',ndarray,etc}
    self.bij = None
    # the surrogate model itself {'target1':model,'target2':model, etc.}
    self.model = None
    # check if the pivotParameter is among the targetValues
    if self.pivotParameterID not in self.target:
      self.raiseAnError(IOError,
  ...
  def __computeExpTerms(self, x, y, returnPredictDiff=True):
    """
      ...
      @ Out, (fi, 1/taui, predictionErr (optional) ), tuple(numpy.ndarray, numpy.ndarray, numpy.ndarray (optional)),
             a_i and b_i and predictionErr (if returnPredictDiff=True)
    """
  ...
  def __trainLocal__(self, featureVals, targetVals):
    ...
    for target in targetIndexes:
      ...
      if self.polyExpParams['coeffRegressor'] == 'poly':
        # now that we have the coefficients, we can construct the polynomial expansion whose targets are the just
        ...
        for l, coeff in enumerate(coefficients):
          coeff_str = "    a_" + str(
              l + 1) if l < self.polyExpParams['expTerms'] else "    b_" + str(
                  (l - self.polyExpParams['expTerms']) + 1)
...
class DynamicModeDecomposition(supervisedLearning):
  ...
  def __init__(self, messageHandler, **kwargs):
    ...
    self.availDmdAlgorithms = ['dmd', 'hodmd']
    # dmd settings container
    self.dmdParams = {}
    # print tag
    self.printTag = 'DMD'
    # pivot parameter
    ...
    self.dmdParams['rankSVD'] = kwargs.get('rankSVD', None)
    # 0.0 < float < 1.0, computed rank is the number of the biggest sv needed to reach the energy identified by
    # "energyRankSVD"
    self.dmdParams['energyRankSVD'] = kwargs.get('energyRankSVD', None)
    # truncation rank for total least square
    self.dmdParams['rankTLSQ'] = kwargs.get('rankTLSQ', None)
  ...
  def __trainLocal__(self, featureVals, targetVals):
    """
      Perform training on input database stored in featureVals.
      # Not use for ARMA
      @ In, featureVals, numpy.ndarray, shape=[n_timeStep, n_dimensions], an array of input data
             training
    ...
    for target in list(set(self.target) - set([self.pivotParameterID])):
      ...
      # if number of features (i.e. samples) > number of snapshots, we apply the high order DMD or HODMD has been
      # requested
      imposedHODMD = False
  ...
  def __evaluateLocal__(self, featureVals):
    ...
    for target in list(set(self.target) - set([self.pivotParameterID])):
      ...
      weights, indexes = self.KDTreeFinder.query(
          featureVals, k=min(2**len(self.features), len(reconstructData)))
      ...
      for point in range(len(weights)):
        returnEvaluation[target] = np.sum(
            (weights[point, :] * reconstructData[indexes[point, :]].T), axis=1)
  ...
  def _localPrintXML(self, outFile, pivotVal, options={}):
    ...
    if "amplitudes" in what:
      ampsReal = " ".join([
          '%.6e' % self._amplitudes[target][indx].real
          for indx in range(len(self._amplitudes[target]))
      ])
      outFile.addScalar("amplitudes", "real", ampsReal, root=target)
      ampsImag = " ".join([
          '%.6e' % self._amplitudes[target][indx].imag
          for indx in range(len(self._amplitudes[target]))
      ])
