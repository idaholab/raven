...
class MPILegacySimulationMode(Simulation.SimulationMode):
  ...
  def __init__(self, messageHandler):
    ...
    # If true, don't split mpi processes across nodes
    self.__noSplitNode = False
    # If true, fiddle with max on Node
    self.__limitNode = False
    # Used with __noSplitNode and __limitNode to limit number on a node
    self.__maxOnNode = None
    # Used with __limitNode to prevent multiple batches from being on one node
    self.__noOverlap = False
